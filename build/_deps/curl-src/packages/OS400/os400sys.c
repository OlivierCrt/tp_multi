...
int Curl_getnameinfo_a(const struct sockaddr *sa, socklen_t salen,
                       char *nodename, socklen_t nodenamelen, char *servname,
                       socklen_t servnamelen, int flags) {
  char *enodename = NULL;
  char *eservname = NULL;
  int status;
...
static int Curl_gss_convert_in_place(OM_uint32 *minor_status,
                                     gss_buffer_t buf) {
...
OM_uint32 Curl_gss_display_status_a(OM_uint32 *minor_status,
  ...
  rc = gss_display_status(minor_status, status_value, status_type, mech_type,
                          message_context, status_string);

  if (rc != GSS_S_COMPLETE || !status_string || !status_string->length ||
      !status_string->value)
    return rc;
...
OM_uint32 Curl_gss_init_sec_context_a(
    ...
    gss_ctx_id_t *context_handle, gss_name_t target_name, gss_OID mech_type,
    ...
    gss_flags_t *ret_flags, OM_uint32 *time_rec) {
  int rc;
  gss_buffer_desc in;
  gss_buffer_t inp;
  ...
  if (rc != GSS_S_COMPLETE || !output_token || !output_token->length ||
      !output_token->value)
    return rc;
...
int Curl_ldap_search_s_a(void *ld, char *base, int scope, char *filter,
  ...
  if (status == LDAP_SUCCESS)
    status = ldap_search_s(ld, ebase ? ebase : "", scope,
                           efilter ? efilter : "(objectclass=*)", eattrs,
                           attrsonly, res);
  ...
  if (srclen >= offsetof(struct sockaddr_storage, ss_family) +
    ...
    case AF_UNIX:
      ...
      if (dstsize > 0 && srclen > 0) {
        srclen = QadrtConvertE2A(dstu->sun_path, srcu->sun_path, dstsize - 1,
                                 srclen);
...
int Curl_os400_recvfrom(int sd, char *buffer, int buflen, int flags,
  ...
  rcvlen =
      recvfrom(sd, buffer, buflen, flags, (struct sockaddr *)&laddr, &laddrlen);
