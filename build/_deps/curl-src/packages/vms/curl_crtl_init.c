...
int SYS$TRNLNM(const unsigned long *attr,
               ...
               struct dsc$descriptor_s *name_dsc, const unsigned char *acmode,
               const struct itmlst_3 *item_list);
int SYS$CRELNM(const unsigned long *attr,
               const struct dsc$descriptor_s *table_dsc,
               const struct dsc$descriptor_s *name_dsc,
               const unsigned char *acmode, const struct itmlst_3 *item_list);
...
static void set_features(void) {
  ...
  status =
      sys_trnlnm("GNV$UNIX_SHELL", unix_shell_name, sizeof unix_shell_name - 1);
  if (!$VMS_STATUS_SUCCESS(status)) {
    use_unix_settings = 0;
  }
