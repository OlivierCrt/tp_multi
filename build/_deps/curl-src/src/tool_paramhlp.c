...
#define MAX_PROTOSTRING                                                        \
  (64 * 11) /* Enough room for 64 10-chars proto names.                        \
             */

ParameterError proto2num(struct OperationConfig *config, const char *const *val,
                         char **ostr, const char *str) {
  char *buffer;
  const char *sep = ",";
  char *token;
  const char **protoset;
  ...
  if (!psep && **userpwd != ';') {
    ...
    if (!i && last)
      curlx_msnprintf(prompt, sizeof(prompt),
                      "Enter %s password for user '%s':", kind, *userpwd);
    else
      curlx_msnprintf(prompt, sizeof(prompt),
                      "Enter %s password for user '%s' on URL #%zu:", kind,
                      *userpwd, i + 1);
 ...
 */
static char *my_useragent(void) { return strdup(CURL_NAME "/" CURL_VERSION); }
...
ParameterError str2tls_max(long *val, const char *str) {
  static struct s_tls_max {
    ...
    long tls_max;
  } const tls_max_array[] = {{"default", CURL_SSLVERSION_MAX_DEFAULT},
