...
const struct NameValueUnsigned setopt_nv_CURLAUTH[] = {
    ...
    NV(CURLAUTH_BASIC),   NV(CURLAUTH_DIGEST),    NV(CURLAUTH_GSSNEGOTIATE),
    NV(CURLAUTH_NTLM),    NV(CURLAUTH_DIGEST_IE), NV(CURLAUTH_NTLM_WB),
    NV(CURLAUTH_ONLY),    NV(CURLAUTH_NONE),      NVEND,
};

const struct NameValue setopt_nv_CURL_HTTP_VERSION[] = {
    NV(CURL_HTTP_VERSION_NONE),  NV(CURL_HTTP_VERSION_1_0),
    NV(CURL_HTTP_VERSION_1_1),   NV(CURL_HTTP_VERSION_2_0),
    NV(CURL_HTTP_VERSION_2TLS),  NV(CURL_HTTP_VERSION_3),
    NV(CURL_HTTP_VERSION_3ONLY), NVEND,
};
  ...
  for (s = str; !result && len; s++, len--) {
    ...
    if (!result) {
      if (p && *p)
        ...
        result =
            curlx_dyn_addf(&escaped, format, (unsigned int)*(unsigned char *)s);
      }
    }
  }
...
static CURLcode libcurl_generate_slist(struct curl_slist *slist, int *slistno) {
  ...
  *slistno = ++easysrc_slist_count;
  ...
  for (; slist; slist = slist->next) {
    ...
    DATA3("slist%d = curl_slist_append(slist%d, \"%s\");", *slistno, *slistno,
          escaped);
  }
...
static CURLcode libcurl_generate_mime(CURL *curl, struct GlobalConfig *config,
...
static CURLcode libcurl_generate_mime_part(CURL *curl,
                                           ...
                                           struct tool_mime *part, int mimeno) {
  CURLcode ret = CURLE_OK;
  int submimeno = 0;
  char *escaped = NULL;
  ...
  case TOOLMIME_DATA:
    ...
    if (!ret) {
      ...
      NULL_CHECK(escaped);
      CODE2("curl_mime_data(part%d, \"%s\", CURL_ZERO_TERMINATED);", mimeno,
            escaped);
    }
    break;
...
static CURLcode libcurl_generate_mime(CURL *curl, struct GlobalConfig *config,
                                      struct tool_mime *toolmime, int *mimeno) {
  CURLcode ret = CURLE_OK;
...
CURLcode tool_setopt_mimepost(CURL *curl, struct GlobalConfig *config,
  ...
  if (!ret && config->libcurl) {
    ret =
        libcurl_generate_mime(curl, config, config->current->mimeroot, &mimeno);
...
CURLcode tool_setopt(CURL *curl, bool str, struct GlobalConfig *global,
                     struct OperationConfig *config, const char *name,
                     CURLoption tag, ...) {
  va_list arg;
  char buf[256];
