...
size_t tool_header_cb(char *ptr, size_t size, size_t nmemb, void *userdata) {
  ...
  if (hdrcbdata->honor_cd_filename && (cb > 20) &&
  ...
  if (hdrcbdata->config->show_headers &&
      (scheme == proto_http || scheme == proto_https || scheme == proto_rtsp ||
       scheme == proto_file)) {
...
static void write_linked_location(CURL *curl, const char *location,
  ...
  if (!strcmp("http", scheme) || !strcmp("https", scheme) ||
      !strcmp("ftp", scheme) || !strcmp("ftps", scheme)) {
    fprintf(stream, "%.*s" LINK "%s" LINKST "%.*s" LINKOFF, space_skipped,
            location, finalurl, (int)loclen - space_skipped, loc);
    goto locdone;
  }
