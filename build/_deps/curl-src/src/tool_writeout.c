...
static const struct httpmap http_version[] = {
    {"0", CURL_HTTP_VERSION_NONE},  {"1", CURL_HTTP_VERSION_1_0},
    {"1.1", CURL_HTTP_VERSION_1_1}, {"2", CURL_HTTP_VERSION_2},
    {"3", CURL_HTTP_VERSION_3},     {NULL, 0} /* end of list */
};
    ...
    if ('%' == *ptr && ptr[1]) {
      if ('%' == ptr[1]) {
        ...
        if ('{' == ptr[1]) {
          ...
          for (i = 0; variables[i].name; i++) {
            if ((strlen(variables[i].name) == vlen) &&
              ...
              case VAR_HEADER_JSON:
                ...
                break;
              default:
                (void)variables[i].writefunc(stream, &variables[i], per,
                                             per_result, false);
                break;
              }
              break;
          ...
          if (!match) {
            fprintf(tool_stderr, "curl: unknown --write-out variable: '%.*s'\n",
                    (int)vlen, ptr);
          }
          ...
          if (end) {
            ...
            if (flen < sizeof(fname)) {
              ...
              fname[flen] = 0;
              stream2 =
                  fopen(fname, append ? FOPEN_APPENDTEXT : FOPEN_WRITETEXT);
              if (stream2) {
