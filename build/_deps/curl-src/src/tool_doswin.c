...
SANITIZEcode sanitize_file_name(char **const sanitized, const char *file_name,
  ...
  *sanitized = NULL;
  ...
  for (; *p; ++p) {
    ...
    if ((1 <= *p && *p <= 31) ||
        (!(flags & (SANITIZE_ALLOW_COLONS | SANITIZE_ALLOW_PATH)) &&
         *p == ':') ||
...
#ifndef MSDOS
  if ((flags & SANITIZE_ALLOW_PATH) && file_name[0] == '\\' &&
      file_name[1] == '\\') {
...
CURLcode FindWin32CACert(struct OperationConfig *config,
                         curl_sslbackend backend, const TCHAR *bundle_file) {
...
struct curl_slist *GetLoadedModulePaths(void) {
  ...
  do {
    ...
    if (!WideCharToMultiByte(CP_ACP, 0, mod.szExePath, -1, buffer,
