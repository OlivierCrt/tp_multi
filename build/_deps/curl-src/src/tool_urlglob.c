...
static CURLcode glob_set(struct URLGlob *glob, char **patternp, size_t *posp,
                         curl_off_t *amount, int globindex) {
      ...
      if (pat->content.Set.elements) {
        char **new_arr =
            realloc(pat->content.Set.elements,
                    (size_t)(pat->content.Set.size + 1) * sizeof(char *));
        if (!new_arr)
          return GLOBERROR("out of memory", 0, CURLE_OUT_OF_MEMORY);
...
static CURLcode glob_range(struct URLGlob *glob, char **patternp, size_t *posp,
                           curl_off_t *amount, int globindex) {
...
static CURLcode glob_parse(struct URLGlob *glob, char *pattern, size_t pos,
                           curl_off_t *amount) {
    ...
    if (error && glob_expand->error) {
      ...
      if (glob_expand->pos) {
        msnprintf(text, sizeof(text), "%s in URL position %zu:\n%s\n%*s^",
                  glob_expand->error, glob_expand->pos, url,
                  (int)glob_expand->pos - 1, " ");
        t = text;
      } else
        t = glob_expand->error;
...
CURLcode glob_match_url(char **result, char *filename, struct URLGlob *glob) {
  ...
  *result = NULL;
...
#if defined(MSDOS) || defined(WIN32)
  {
    char *sanitized;
    SANITIZEcode sc =
        sanitize_file_name(&sanitized, curlx_dyn_ptr(&dyn),
