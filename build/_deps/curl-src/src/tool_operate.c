...
static CURLcode single_transfer(struct GlobalConfig *global,
                                struct OperationConfig *config, CURLSH *share,
                                bool capath_from_env, bool *added);
static CURLcode create_transfer(struct GlobalConfig *global, CURLSH *share,
                                bool *added);
...
static CURLcode pre_transfer(struct GlobalConfig *global,
  ...
  if (per->uploadfile && !stdin_upload(per->uploadfile)) {
    ...
    if (stat(per->uploadfile, &fileinfo) == 0) {
      ...
      default:
        per->infd =
            open(per->uploadfile, O_RDONLY | O_BINARY, "rfm=stmlf", "ctx=stm");
      }
    }
    if (per->infd == -1)
#else
...
#ifdef __AMIGA__
static void AmigaSetComment(struct per_transfer *per, CURLcode result) {
...
static CURLcode post_per_transfer(struct GlobalConfig *global,
                                  struct per_transfer *per, CURLcode result,
                                  bool *retryp, long *delay) /* milliseconds! */
{
  struct OutStruct *outs = &per->outs;
  CURL *curl = per->curl;
  ...
  *delay = 0; /* for no retry, keep it zero */
  ...
  if (per->retry_numretries &&
      (!config->retry_maxtime ||
       (tvdiff(tvnow(), per->retrystart) < config->retry_maxtime * 1000L))) {
    enum {
      RETRY_NO,
      RETRY_ALL_ERRORS,
      RETRY_TIMEOUT,
    ...
    if (retry) {
      ...
      static const char *const m[] = {NULL,           "(retrying all errors)",
      ...
      if (outs->bytes && outs->filename && outs->stream) {
         ...
         */
        notef(config->global, "Throwing away %" CURL_FORMAT_CURL_OFF_T " bytes",
              outs->bytes);
        fflush(outs->stream);
...
static CURLcode ipfs_url_rewrite(CURLU *uh, const char *protocol, char **url,
  ...
  if (config->ipfs_gateway) {
    ...
    if (curl_url_set(ipfsurl, CURLUPART_URL, gateway,
  ...
  if (curl_url_get(ipfsurl, CURLUPART_URL, url, CURLU_URLENCODE) != CURLUE_OK) {
    goto clean;
  }
 ...
 */
static CURLcode url_proto(char **url, struct OperationConfig *config,
                          char **scheme) {
  CURLcode result = CURLE_OK;
  CURLU *uh = curl_url();
  ...
  if (uh) {
    if (*url) {
      ...
      if (!curl_url_set(uh, CURLUPART_URL, *url,
                        CURLU_GUESS_SCHEME | CURLU_NON_SUPPORT_SCHEME) &&
          !curl_url_get(uh, CURLUPART_SCHEME, &schemep, CURLU_DEFAULT_SCHEME)) {
  ...
  *added = FALSE; /* not yet */
      ...
      if (!httpgetfields) {
        ...
        if (!httpgetfields) {
          ...
          result = CURLE_OUT_OF_MEMORY;
                       ...
                       config, (config->no_body ? HTTPREQ_HEAD : HTTPREQ_GET),
                       &config->httpreq)) {
          result = CURLE_FAILED_INIT;
        }
      }
  ...
  while (config->state.urlnode) {
    ...
    if (!config->globoff && infiles && !inglob) {
      ...
      result =
          glob_url(&inglob, infiles, &state->infilenum,
      ...
      if (!state->urlnum) {
        if (!config->globoff) {
          ...
          result = glob_url(&state->urls, urlnode->url, &state->urlnum,
                            (!global->silent || global->showerror) ? tool_stderr
                                                                   : NULL);
          if (result)
            break;
          urlnum = state->urlnum;
      ...
      if (state->up < state->infilenum) {
        ...
        if (config->etag_compare_file) {
          ...
          if (!file && !config->etag_save_file) {
            errorf(global, "Failed to open %s", config->etag_compare_file);
        ...
        if (config->etag_save_file) {
          ...
          if (strcmp(config->etag_save_file, "-")) {
            ...
            if (!newfile) {
              warnf(global,
                    "Failed creating file for saving etags: \"%s\". "
                    "Skip this transfer",
        ...
        *added = TRUE;
        ...
        if (per->uploadfile && !stdin_upload(per->uploadfile)) {
           ...
           */
          result =
              add_file_name_to_url(per->curl, &per->this_url, per->uploadfile);
          if (result)
            break;
          ...
          if (!strcmp(per->uploadfile, ".")) {
            if (curlx_nonblock((curl_socket_t)per->infd, TRUE) < 0)
              warnf(global, "fcntl failed on fd=%d: %s", per->infd,
                    strerror(errno));
          }
        }
        ...
        if (feature_ssl) {
...

          {
            long mask =
                ...
                (config->ssl_revoke_best_effort ? CURLSSLOPT_REVOKE_BEST_EFFORT
                                                : 0) |
                (config->native_ca_store ? CURLSSLOPT_NATIVE_CA : 0) |
                (config->ssl_auto_client_cert ? CURLSSLOPT_AUTO_CLIENT_CERT
                                              : 0);
...
#define MAX_COOKIE_LINE 8200
        ...
        my_setopt(curl, CURLOPT_FTP_CREATE_MISSING_DIRS,
                  (long)(config->ftp_create_dirs ? CURLFTP_CREATE_DIR_RETRY
        ...
        my_setopt(curl, CURLOPT_IGNORE_CONTENT_LENGTH,
                  config->ignorecl ? 1L : 0L);

        /* curl 7.14.2 */
        my_setopt(curl, CURLOPT_FTP_SKIP_PASV_IP,
                  config->ftp_skip_ip ? 1L : 0L);
...
static CURLcode add_parallel_transfers(struct GlobalConfig *global,
                                       CURLM *multi, CURLSH *share, bool *morep,
                                       bool *addedp) {
  struct per_transfer *per;
  CURLcode result = CURLE_OK;
  CURLMcode mcode;
...
static CURLcode parallel_transfers(struct GlobalConfig *global, CURLSH *share) {
  ...
  result = add_parallel_transfers(global, multi, share, &more_transfers,
                                  &added_transfers);
  if (result) {
  ...
  while (!mcode && (still_running || more_transfers)) {
    ...
    if (!mcode) {
      ...
      if (checkmore) {
        ...
        CURLcode tres = add_parallel_transfers(
...
static CURLcode transfer_per_config(struct GlobalConfig *global,
  ...
  *added = FALSE;
  ...
  if (!config->cacert && !config->capath &&
     ...
     */
    result =
        curl_easy_getinfo(curltls, CURLINFO_TLS_SSL_PTR, &tls_backend_info);
    if (result) {
...
static CURLcode run_all_transfers(struct GlobalConfig *global, CURLSH *share,
                                  CURLcode result) {
...
CURLcode operate(struct GlobalConfig *global, int argc, argv_item_t argv[]) {
  ...
  if ((argc == 1) || (first_arg && strncmp(first_arg, "-q", 2) &&
