...
static ParameterError varfunc(struct GlobalConfig *global,
  ...
  while (*f && !err) {
    ...
    if (FUNCMATCH(f, FUNC_TRIM, FUNC_TRIM_LEN)) {
      ...
      errorf(global, "unknown variable function in '%.*s'", (int)flen, finput);
...
ParameterError varexpand(struct GlobalConfig *global, const char *line,
                         struct curlx_dynbuf *out, bool *replaced) {
  CURLcode result;
  char *envp;
  bool added = FALSE;
      ...
      if (!nlen || (nlen >= sizeof(name))) {
        ...
        if (i != nlen) {
          ...
          if (funcp) {
            ...
            ParameterError err =
                varfunc(global, value, vlen, funcp, flen, &buf);
            if (err)
              return err;
 ...
 * at a later point.
 */
static ParameterError addvariable(struct GlobalConfig *global, const char *name,
                                  size_t nlen, const char *content, size_t clen,
                                  bool contalloc) {
  struct var *p;
