...
size_t tool_mime_stdin_read(char *buffer, size_t size, size_t nitems,
                            void *arg) {
    ...
    if (!ret) {
      ...
      case TOOLMIME_STDINDATA:
        ret = curl_mime_data_cb(
            part, m->size, (curl_read_callback)tool_mime_stdin_read,
            (curl_seek_callback)tool_mime_stdin_seek, NULL, m);
        break;
  ...
  *endpos = '\0';
  ...
  if (pfilename)
    *pfilename = filename;
    ...
    warnf(config->global, "Field file name not allowed here: %s", filename);

  if (pencoder)
    *pencoder = encoder;
  else if (encoder)
    warnf(config->global, "Field encoder not allowed here: %s", encoder);

  if (pheaders)
    *pheaders = headers;
  else if (headers) {
    warnf(config->global, "Field headers not allowed here: %s", headers->data);
    curl_slist_free_all(headers);
  }
...
int formparse(struct OperationConfig *config, const char *input,
              struct tool_mime **mimeroot, struct tool_mime **mimecurrent,
              bool literal_value) {
  ...
  if (contp) {
    ...
    *contp++ = '\0';
    ...
    if (*contp == '(' && !literal_value) {
      ...
      sep = get_param_part(config, '\0', &contp, &data, &type, NULL, NULL,
                           &headers);
      if (sep < 0)
        goto fail;
      ...
      *mimecurrent = (*mimecurrent)->parent;
      ...
      do {
        ...
        ++contp;
        sep = get_param_part(config, ',', &contp, &data, &type, &filename,
                             &encoder, &headers);
        if (sep < 0) {
          goto fail;
        }
      ...
      if (*contp == '<' && !literal_value) {
        ++contp;
        sep = get_param_part(config, '\0', &contp, &data, &type, NULL, &encoder,
                             &headers);
        if (sep < 0)
          goto fail;
        ...
        if (literal_value)
          ...
          sep = get_param_part(config, '\0', &contp, &data, &type, &filename,
                               &encoder, &headers);
          if (sep < 0)
            goto fail;
        }
