...
static const char *hms_for_sec(time_t tv_sec) {
  ...
  if (tv_sec != cached_tv_sec) {
    ...
    msnprintf(hms_buf, sizeof(hms_buf), "%02d:%02d:%02d", now->tm_hour,
...
static void log_line_start(FILE *log, const char *timebuf, const char *idsbuf,
                           curl_infotype type) {
  /*
...
#define TRC_IDS_FORMAT_IDS_2                                                   \
  "[%" CURL_FORMAT_CURL_OFF_T "-%" CURL_FORMAT_CURL_OFF_T "] "
/*
** callback for CURLOPT_DEBUGFUNCTION
*/
int tool_debug_cb(CURL *handle, curl_infotype type, char *data, size_t size,
                  void *userdata) {
  ...
  if (config->tracetime) {
    tv = tvnow();
    msnprintf(timebuf, sizeof(timebuf), "%s.%06ld ", hms_for_sec(tv.tv_sec),
              (long)tv.tv_usec);
  } else
    timebuf[0] = 0;
  ...
  if (handle && config->traceids &&
    ...
    if (!curl_easy_getinfo(handle, CURLINFO_CONN_ID, &conn_id) &&
        conn_id >= 0) {
      msnprintf(idsbuf, sizeof(idsbuf), TRC_IDS_FORMAT_IDS_2, xfer_id, conn_id);
    } else {
...
static void dump(const char *timebuf, const char *idsbuf, const char *text,
  ...
  for (i = 0; i < size; i += width) {
    ...
    for (c = 0; (c < width) && (i + c < size); c++) {
      ...
      if ((tracetype == TRACE_ASCII) && (i + c + 1 < size) &&
      ...
      (void)infotype;
      fprintf(stream, "%c",
              ((ptr[i + c] >= 0x20) && (ptr[i + c] < 0x7F)) ? ptr[i + c]
                                                            : UNPRINTABLE_CHAR);
      /* check again for 0D0A, to avoid an extra \n if it's at width */
      if ((tracetype == TRACE_ASCII) && (i + c + 2 < size) &&
