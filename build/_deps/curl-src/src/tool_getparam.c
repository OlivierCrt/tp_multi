...
static
#endif
    void parse_cert_parameter(const char *cert_parameter, char **certname,
                              char **passphrase) {
...
static size_t replace_url_encoded_space_by_plus(char *url) {
  ...
  while (orig_index < orig_len) {
    if ((url[orig_index] == '%') && (url[orig_index + 1] == '2') &&
        (url[orig_index + 2] == '0')) {
      url[new_index] = '+';
      orig_index += 3;
    } else {
...
static ParameterError GetSizeParameter(struct GlobalConfig *global,
                                       const char *arg, const char *which,
...
/* --data-urlencode */
static ParameterError data_urlencode(struct GlobalConfig *global, char *nextarg,
                                     char **postp, size_t *lenp) {
...
static void sethttpver(struct GlobalConfig *global,
                       struct OperationConfig *config, long httpversion) {
  if (config->httpversion && (config->httpversion != httpversion))
...
ParameterError getparameter(const char *flag, /* f or -long-flag */
  ...
  static const char *redir_protos[] = {"http", "https", "ftp", "ftps", NULL};
#ifdef HAVE_WRITABLE_ARGV
  ...
  *usedarg = FALSE; /* default is that we don't use the arg */
  ...
  do {
    ...
    if (aliases[hit].desc >= ARG_STRING) {
      ...
      if ((aliases[hit].desc == ARG_FILENAME) && (nextarg[0] == '-') &&
          nextarg[1]) {
...
#define MAX_QUERY_LEN 100000    /* larger is not likely to ever work */
    ...
    case 'F':
      ...
      if (formparse(config, nextarg, &config->mimeroot, &config->mimecurrent,
    ...
    case 'z': /* time condition coming up */
      ...
      if (-1 == config->condtime) {
        ...
        if (!rc)
          ...
          warnf(global, "Illegal date format for -z, --time-cond (and not "
  ...
  for (i = 1, stillflags = TRUE; i < argc && !result; i++) {
    ...
    if (stillflags && ('-' == orig_opt[0])) {
      ...
      if (!strcmp("--", orig_opt))
        ...
        result = getparameter(orig_opt, nextarg, argv[i + 1], &passarg, global,
                              config);
