...
static int addFd(struct Sockets *sockets, curl_socket_t fd, const char *what) {
  ...
  if (!sockets->sockets) {
    ...
    curl_socket_t *ptr = realloc(
        sockets->sockets, sizeof(curl_socket_t) * (sockets->max_count + 20));
    if (!ptr)
 ...
 */
static int checkFdSet(CURLM *curl, struct Sockets *sockets, fd_set *fdset,
