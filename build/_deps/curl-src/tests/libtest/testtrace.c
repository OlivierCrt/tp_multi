...
int libtest_debug_cb(CURL *handle, curl_infotype type, unsigned char *data,
  ...
  if (trace_cfg->tracetime) {
    ...
    msnprintf(timebuf, sizeof(timebuf), "%02d:%02d:%02d.%06ld ", now->tm_hour,
              now->tm_min, now->tm_sec, (long)tv.tv_usec);
  }
