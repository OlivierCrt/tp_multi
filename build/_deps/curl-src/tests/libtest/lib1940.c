...
static const char *show[] = {"daTE",           "Server",   "content-type",
                             "content-length", "location", "set-cookie",
                             "silly-thing",    "fold",     NULL};
...
static void showem(CURL *easy, unsigned int type) {
  ...
  for (i = 0; show[i]; i++) {
    if (CURLHE_OK ==
        curl_easy_header(easy, show[i], 0, type, HEADER_REQUEST, &header)) {
      if (header->amount > 1) {
