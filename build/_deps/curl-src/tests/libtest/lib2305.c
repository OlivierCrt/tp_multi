...
static void websocket_close(CURL *curl) {
  ...
  fprintf(stderr, "ws: curl_ws_send returned %u, sent %u\n", (int)result,
          (int)sent);
}
...
static void websocket(CURL *curl) {
  ...
  while (i < 12291) {
    CURLcode result = curl_ws_recv(curl, buffer, sizeof(buffer), &nread, &meta);
    if (result) {
    ...
    printf("%u: nread %zu Age %u Flags %x "
           ...
           (int)i, nread, meta->age, meta->flags, meta->offset,
           meta->bytesleft);
    i += meta->len;
