...
static const struct dohresp resp[] = {
    ...
    {"\x00\x01\x00\x01\x00\x01\x00\x01\x00\x01\x00\x01", 12, DNS_TYPE_A,
     DOH_DNS_BAD_ID, NULL},
    {"\x00\x00\x00\x01\x00\x01\x00\x01\x00\x01\x00\x01", 12, DNS_TYPE_A,
     DOH_DNS_BAD_RCODE, NULL},
    ...
    {"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x00\x00\x00\x03\x66\x6f\x6f\x00"
     "\x00\x01\x00\x01",
     21, DNS_TYPE_A, DOH_DNS_OUT_OF_RANGE, NULL},
    {"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x00\x00\x00\x03\x66\x6f\x6f\x00"
     "\x00\x01\x00\x01"
     "\x04",
     18, DNS_TYPE_A, DOH_DNS_OUT_OF_RANGE, NULL},
    ...
    {"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x00\x00\x00\x04\x63\x75\x72"
     ...
     56, DNS_TYPE_A, DOH_OK, "anywhere.really "},
    ...
    {"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x00\x00\x00\x04\x61\x61\x61"
     ...
     62, DNS_TYPE_AAAA, DOH_OK, "2020:2020:0000:0000:0000:0000:0000:2020 "},

    {"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x00\x00\x00\x04\x63\x75\x72"
     "\x6c\x04\x63\x75\x72\x6c\x00\x00\x05\x00\x01\xc0\x0c\x00\x05\x00"
     "\x01\x00\x00\x00\x37\x00"
     "\x07\x03\x61\x6e\x79\xc0\x27\x00",
     46, DNS_TYPE_A, DOH_DNS_LABEL_LOOP, NULL},

    /* packet with NSCOUNT == 1 */
    {"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x01\x00\x00\x04\x61\x61\x61"
     "\x61\x07\x65\x78\x61\x6d\x70\x6c\x65\x03\x63\x6f\x6d\x00\x00\x1c"
     "\x00\x01\xc0\x0c\x00\x1c\x00\x01\x00\x00\x00\x37\x00\x10\x20\x20"
     "\x20\x20\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x20\x20" LABEL_TEST
         LABEL_HOST LABEL_NAME DNSAAAA_EPILOGUE "\x00\x00\x00\x01"
     "\00\x04\x01\x01\x01\x01", /* RDDATA */

     62 + 30, DNS_TYPE_AAAA, DOH_OK,
...
UNITTEST_START {
  ...
  for (i = 0; i < sizeof(req) / sizeof(req[0]); i++) {
    int rc =
        doh_encode(req[i].name, req[i].type, buffer, sizeof(buffer), &size);
    if (rc != req[i].rc) {
...

  {
    ...
    a = &d.addr[0];
    p = &a->ip.v4[0];
    msnprintf((char *)buffer, sizeof(buffer), "%u.%u.%u.%u", p[0], p[1], p[2],
              p[3]);
