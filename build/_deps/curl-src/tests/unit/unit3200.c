...
static CURLcode unit_stop(void) { return CURLE_OK; }
...
for (i = 0; i < NUMTESTS; i++) {
  ...
  case 2:
    ...
    line = Curl_get_line(buf, len, fp);
    fail_unless(line == NULL, "Did not detect max read on EOF (3)");
    break;
  case 3:
    line = Curl_get_line(buf, len, fp);
    fail_unless(line && !strcmp("LINE1\n", line), "First line failed (4)");
    line = Curl_get_line(buf, len, fp);
    fail_unless(line == NULL, "Did not ignore partial on EOF (4)");
    break;
  case 4:
    line = Curl_get_line(buf, len, fp);
    fail_unless(line && !strcmp("LINE1\n", line), "First line failed (5)");
    line = Curl_get_line(buf, len, fp);
    fail_unless(line && !strcmp("LINE3\n", line), "Third line failed (5)");
    line = Curl_get_line(buf, len, fp);
