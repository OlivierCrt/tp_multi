...
static void parse_success(struct tcase *t) {
  ...
  for (i = 0; t->input[i]; ++i) {
    ...
    buflen = strlen(buf);
    in_len += buflen;
    nread = Curl_h1_req_parse_read(&p, buf, buflen, t->default_scheme, 0, &err);
    if (nread < 0) {
...
static struct tcase TEST1a = {T1_INPUT, NULL, "GET", NULL, NULL, "/path", 1, 0};
static struct tcase TEST1b = {T1_INPUT, "https", "GET", "https",
                              NULL,     "/path", 1,     0};
...
static struct tcase TEST2 = {T2_INPUT, NULL, "GET", NULL, NULL, "/path", 1, 8};
...
static struct tcase TEST3a = {T3_INPUT,      NULL,       "GET", "ftp",
                              "ftp.curl.se", "/xxx?a=2", 2,     0};
...
static struct tcase TEST4a = {T4_INPUT,          NULL, "CONNECT", NULL,
                              "ftp.curl.se:123", NULL, 3,         2};
...
static struct tcase TEST5a = {T5_INPUT, NULL, "OPTIONS", NULL, NULL, "*", 2, 3};

static const char *T6_INPUT[] = {
    "PUT /path HTTP/1.1\nHost: test.curl.se\n\n123",
    NULL,
};
static struct tcase TEST6a = {T6_INPUT, NULL, "PUT", NULL, NULL, "/path", 1, 3};
