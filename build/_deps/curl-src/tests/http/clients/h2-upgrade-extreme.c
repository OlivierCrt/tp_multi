...
int main(int argc, char *argv[]) {
  ...
  do {
    if (start_count) {
      ...
      curl_msnprintf(range, sizeof(range), "%" PRIu64 "-%" PRIu64, UINT64_C(0),
                     UINT64_C(16384));
    ...
    while ((msg = curl_multi_info_read(multi, &msgs_in_queue))) {
      if (msg->msg == CURLMSG_DONE) {
        ...
        if (msg->data.result == CURLE_SEND_ERROR ||
          ...
          fprintf(stderr,
                  "transfer #%" CURL_FORMAT_CURL_OFF_T ": failed with %d\n",
                  xfer_id, msg->data.result);
          exit(1);
        } else if (status != 206) {
          fprintf(stderr,
                  "transfer #%" CURL_FORMAT_CURL_OFF_T
        ...
        fprintf(stderr,
                "transfer #%" CURL_FORMAT_CURL_OFF_T " retiring "
                "(%d now running)\n",
