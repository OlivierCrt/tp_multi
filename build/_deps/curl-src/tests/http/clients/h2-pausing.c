...
static size_t cb(void *data, size_t size, size_t nmemb, void *clientp) {
  ...
  if (curl_easy_getinfo(handle->h, CURLINFO_CONTENT_LENGTH_DOWNLOAD_T,
                        &totalsize) == CURLE_OK)
    fprintf(stderr,
            "INFO: [%d] write, Content-Length %" CURL_FORMAT_CURL_OFF_T "\n",
...
int main(int argc, char *argv[]) {
  ...
  for (rounds = 0;; rounds++) {
    ...
    if (!still_running) {
      ...
      if (!as_expected) {
        fprintf(stderr,
                "ERROR: handles not in expected state "
                "after %d rounds\n",
                rounds);
        rc = 1;
      }
        ...
        for (i = 0; i < HANDLECOUNT; i++) {
          if (msg->easy_handle == handles[i].h) {
            if (handles[i].paused != 1 || !handles[i].resumed) {
              fprintf(stderr,
                      "ERROR: [%d] done, pauses=%d, resumed=%d, "
                      "result %d - wtf?\n",
                      i, handles[i].paused, handles[i].resumed,
                      msg->data.result);
              rc = 1;
              goto out;
            }
