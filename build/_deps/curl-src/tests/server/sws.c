...
static int ProcessRequest(struct httprequest *req) {
  ...
  if (use_gopher && (req->testno == DOCNUMBER_NOTHING) &&
    ...
    if (sscanf(line, "%" REQUEST_KEYWORD_SIZE_TXT "s ", request)) {
    ...
    if (fine) {
      ...
      if (req->testno == DOCNUMBER_NOTHING) {
        ...
        if (sscanf(req->reqbuf, "CONNECT %" MAXDOCNAMELEN_TXT "s HTTP/%d.%d",
          ...
          msnprintf(logbuf, sizeof(logbuf),
                    "Received a CONNECT %s HTTP/%d.%d request", doc, prot_major,
                    prot_minor);
          logmsg("%s", logbuf);
          ...
          logmsg("Port number: %d, test case number: %ld", req->connect_port,
                 req->testno);
        }
      }
      ...
      logmsg("** Unusual request. Starts with %02x %02x %02x (%c%c%c)", l[0],
             l[1], l[2], l[0], l[1], l[2]);
    }
  }
  ...
  if (use_gopher) {
    ...
    if (ptr) {
      ...
      msnprintf(logbuf, sizeof(logbuf),
                "Requested GOPHER test number %ld part %ld", req->testno,
                req->partno);
      logmsg("%s", logbuf);
    }
  }
  ...
  if (strstr(req->reqbuf, "Authorization: Negotiate")) {
    ...
    logmsg("Negotiate: prev_testno: %d, prev_partno: %d", prev_testno,
           prev_partno);
  ...
  if (req->open && req->prot_version >= 11 &&
...
static void storerequest(const char *reqbuf, size_t totalsize) {
  ...
  msnprintf(dumpfile, sizeof(dumpfile), "%s/%s", logdir,
  ...
  if (!dump) {
    logmsg("[2] Error opening file %s error: %d %s", dumpfile, error,
           strerror(error));
  ...
  if (writeleft == 0)
    ...
    logmsg("Error writing file %s error: %d %s", dumpfile, error,
           strerror(error));
...
storerequest_cleanup:
  ...
  if (res)
    logmsg("Error closing file %s error: %d %s", dumpfile, error,
           strerror(error));
}
...
static int send_doc(curl_socket_t sock, struct httprequest *req) {
  ...
  msnprintf(responsedump, sizeof(responsedump), "%s/%s", logdir,
            is_proxy ? RESPONSE_PROXY_DUMP : RESPONSE_DUMP);
...
#define STREAMTHIS "a string to stream 01234567890\n"
  ...
  if (req->testno < 0) {
    ...
    case DOCNUMBER_WERULEZ:
      ...
      msnprintf(msgbuf, sizeof(msgbuf),
                "WE ROOLZ: %" CURL_FORMAT_CURL_OFF_T "\r\n", our_getpid());
  ...
  retry:
    ...
    if (req->writedelay) {
      ...
      if (msecs_left % MAX_SLEEP_TIME_MS)
        intervals++;
      logmsg("Pausing %d milliseconds after writing %d bytes", msecs_left,
             written);
      while ((intervals > 0) && !got_exit_signal) {
        int sleep_time =
            msecs_left > MAX_SLEEP_TIME_MS ? MAX_SLEEP_TIME_MS : msecs_left;
        intervals--;
        wait_ms(sleep_time);
  ...
  if (res)
    logmsg("Error closing file %s error: %d %s", responsedump, error,
           strerror(error));
  ...
  logmsg("Response sent (%zu bytes) and written to %s", responsesize,
         responsedump);
  free(ptr);
        ...
        if (!strcmp("wait", command)) {
          ...
          while ((quarters > 0) && !got_exit_signal) {
            ...
            if (res) {
              /* should not happen */
              error = errno;
              logmsg("wait_ms() failed with error: (%d) %s", error,
                     strerror(error));
              break;
            }
          }
...
static curl_socket_t connect_to(const char *ipaddr, unsigned short port) {
  ...
  if (CURL_SOCKET_BAD == serverfd) {
    error = SOCKERRNO;
    logmsg("Error creating socket for server connection: (%d) %s", error,
           sstrerror(error));
    return CURL_SOCKET_BAD;
  }
  ...
  if (socket_domain_is_ip()) {
    ...
    if (0 != setsockopt(serverfd, IPPROTO_TCP, TCP_NODELAY, (void *)&flag,
                        sizeof(flag)))
  ...
  if (rc) {
    error = SOCKERRNO;
    logmsg("Error connecting to server port %hu: (%d) %s", port, error,
           sstrerror(error));
    sclose(serverfd);
    return CURL_SOCKET_BAD;
  }

  logmsg("connected fine to %s%s%s:%hu, now tunnel", op_br, ipaddr, cl_br,
         port);
...
static void http_connect(curl_socket_t *infdp, curl_socket_t rootfd,
                         const char *ipaddr, unsigned short ipport,
                         int keepalive_secs) {
  ...
  while (!got_exit_signal) {
    ...
    if ((clientfd[DATA] == CURL_SOCKET_BAD) &&
        (serverfd[DATA] == CURL_SOCKET_BAD) && poll_client_rd[CTRL] &&
        poll_client_wr[CTRL] && poll_server_rd[CTRL] && poll_server_wr[CTRL]) {
      ...
      if ((clientfd[DATA] == CURL_SOCKET_BAD) &&
        ...
        if (datafd != CURL_SOCKET_BAD) {
          ...
          if (socket_domain_is_ip()) {
            /* Disable the Nagle algorithm */
            curl_socklen_t flag = 1;
            if (0 != setsockopt(datafd, IPPROTO_TCP, TCP_NODELAY, (void *)&flag,
                                sizeof(flag)))
...
static curl_socket_t accept_connection(curl_socket_t sock) {
  ...
  if (CURL_SOCKET_BAD == msgsock) {
    ...
    logmsg("MAJOR ERROR: accept() failed with error: (%d) %s", error,
           sstrerror(error));
    return CURL_SOCKET_BAD;
  }
  ...
  if (0 != curlx_nonblock(msgsock, TRUE)) {
    error = SOCKERRNO;
    logmsg("curlx_nonblock failed with error: (%d) %s", error,
           sstrerror(error));
    sclose(msgsock);
    return CURL_SOCKET_BAD;
  }

  if (0 != setsockopt(msgsock, SOL_SOCKET, SO_KEEPALIVE, (void *)&flag,
                      sizeof(flag))) {
    error = SOCKERRNO;
    logmsg("setsockopt(SO_KEEPALIVE) failed with error: (%d) %s", error,
           sstrerror(error));
    sclose(msgsock);
    return CURL_SOCKET_BAD;
  }
  ...
  */
  ...
  if (socket_domain_is_ip()) {
     ...
     */
    if (0 != setsockopt(msgsock, IPPROTO_TCP, TCP_NODELAY, (void *)&flag,
                        sizeof(flag)))
...
static int service_connection(curl_socket_t msgsock, struct httprequest *req,
                              curl_socket_t listensock, const char *connecthost,
                              int keepalive_secs) {
  if (got_exit_signal)
    return -1;
...
int main(int argc, char *argv[]) {
    ...
    if (!strcmp("--version", argv[arg])) {
      ...
      if (argc > arg) {
        ...
        if ((endptr != argv[arg] + strlen(argv[arg])) ||
            (ulnum && (ulnum > 65535UL))) {
          fprintf(stderr,
                  "sws: invalid --keepalive argument (%s), must "
  ...
  msnprintf(loglockfile, sizeof(loglockfile), "%s/%s/sws-%s%s-%s.lock", logdir,
            SERVERLOGS_LOCKDIR, protocol_type, is_proxy ? "-proxy" : "",
            socket_type);
  ...
  if (0 !=
    ...
    error = SOCKERRNO;
    logmsg("setsockopt(SO_REUSEADDR) failed with error: (%d) %s", error,
           sstrerror(error));
    goto sws_cleanup;
  }
  ...
  if (0 != rc) {
    ...
    if (socket_domain == AF_UNIX)
      logmsg("Error binding socket on path %s: (%d) %s", unix_socket, error,
             sstrerror(error));
    else
#endif
  ...
  if (!port) {
    ...
    if (getsockname(sock, &localaddr.sa, &la_size) < 0) {
      error = SOCKERRNO;
      logmsg("getsockname() failed with error: (%d) %s", error,
             sstrerror(error));
      sclose(sock);
      goto sws_cleanup;
    }
  ...
  logmsg("Running %s %s version on %s", protocol_type, socket_type,
         location_str);
...
sws_cleanup:
  ...
  if (got_exit_signal) {
    logmsg("========> %s sws (%s pid: %ld) exits with signal (%d)", socket_type,
           location_str, (long)getpid(), exit_signal);
    /*
