...
void logmsg(const char *msg, ...) {
  ...
  msnprintf(timebuf, sizeof(timebuf), "%02d:%02d:%02d.%06ld", (int)now->tm_hour,
            (int)now->tm_min, (int)now->tm_sec, (long)tv.tv_usec);
...
static const char *win32_strerror(int err, char *buf, size_t buflen) {
  if (!FormatMessageA(
          ...
          err, LANG_NEUTRAL, buf, (DWORD)buflen, NULL))
...
void set_advisor_read_lock(const char *filename) {
  ...
  if (!lockfile) {
    logmsg("Error creating lock file %s error: %d %s", filename, error,
           strerror(error));
    return;
  }
  ...
  if (res)
    logmsg("Error closing lock file %s error: %d %s", filename, error,
           strerror(error));
}
...
void clear_advisor_read_lock(const char *filename) {
  ...
  */
  ...
  if (res)
    logmsg("Error removing lock file %s error: %d %s", filename, error,
           strerror(error));
}
 ...
 */
static LRESULT CALLBACK main_window_proc(HWND hwnd, UINT uMsg, WPARAM wParam,
                                         LPARAM lParam) {
  int signum = 0;
...
#endif
{
  ...
  hidden_main_window = CreateWindowEx(
      0, TEXT("MainWClass"), TEXT("Recv WM_CLOSE msg"), WS_OVERLAPPEDWINDOW,
...
int bind_unix_socket(curl_socket_t sock, const char *unix_socket,
  ...
  if (0 != rc && SOCKERRNO == EADDRINUSE) {
    ...
    if (CURL_SOCKET_BAD == unixfd) {
      logmsg("Failed to create socket at %s: (%d) %s", unix_socket, SOCKERRNO,
             sstrerror(SOCKERRNO));
      return -1;
    }
    ...
    if (0 != rc && ECONNREFUSED != error) {
      logmsg("Failed to connect to %s: (%d) %s", unix_socket, error,
             sstrerror(error));
      return rc;
    }
