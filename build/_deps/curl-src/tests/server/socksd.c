...
static curl_socket_t socksconnect(unsigned short connectport,
  ...
  if (rc) {
    int error = SOCKERRNO;
    logmsg("Error connecting to %s:%hu: (%d) %s", connectaddr, connectport,
...
static curl_socket_t socks4(curl_socket_t fd, unsigned char *buffer,
                            ssize_t rc) {
...
static curl_socket_t sockit(curl_socket_t fd) {
  ...
  if ((buffer[SOCKS5_NMETHODS] < config.nmethods_min) ||
    ...
    logmsg("NMETHODS byte not within %d - %d ", config.nmethods_min,
           config.nmethods_max);
    return CURL_SOCKET_BAD;
  }
  ...
  */
  ...
  /* port number */
  memcpy(&response[SOCKS5_BNDADDR + len], &buffer[SOCKS5_DSTADDR + len],
         sizeof(socksport));
    ...
    for (i = 0; i < 2; i++) {
      ...
      if (cp->used) {
        if (tunnel(cp, &fds_read)) {
          logmsg("SOCKS transfer completed. Bytes: < %zu > %zu", cp->fromremote,
                 cp->fromclient);
          sclose(cp->clientfd);
          sclose(cp->remotefd);
          cp->used = FALSE;
