...
static int ProcessRequest(struct httprequest *req) {
  ...
  logmsg("ProcessRequest() called with testno %ld and line [%s]", req->testno,
         line);
...
static void storerequest(char *reqbuf, size_t totalsize) {
  ...
  if (!dump) {
    logmsg("Error opening file %s error: %d %s", dumpfile, error,
           strerror(error));
...
static int send_doc(curl_socket_t sock, struct httprequest *req) {
  ...
  msnprintf(responsedump, sizeof(responsedump), "%s/%s", logdir, RESPONSE_DUMP);
  ...
  if (req->testno < 0) {
    ...
    case DOCNUMBER_WERULEZ:
      ...
      msnprintf(msgbuf, sizeof(msgbuf),
                "RTSP_SERVER WE ROOLZ: %" CURL_FORMAT_CURL_OFF_T "\r\n",
                our_getpid());
  ...
  if (req->rtp_buffer) {
    ...
    do {
      ...
      if (num > 200)
        num = 200;
      written =
          swrite(sock, req->rtp_buffer + (req->rtp_buffersize - count), num);
      if (written < 0) {
        sendfailure = TRUE;
        break;
      }
...
int main(int argc, char *argv[]) {
  ...
  msnprintf(loglockfile, sizeof(loglockfile), "%s/%s/rtsp-%s.lock", logdir,
            SERVERLOGS_LOCKDIR, ipv_inuse);
  ...
  */
  ...
  for (;;) {
    ...
    */
     ...
     */
    flag = 1;
    if (setsockopt(msgsock, IPPROTO_TCP, TCP_NODELAY, (void *)&flag,
                   sizeof(flag)) == -1) {
