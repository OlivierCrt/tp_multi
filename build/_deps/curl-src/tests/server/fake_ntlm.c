...
#define NOTHING_LEN 9
  ...
  if (inlength) {
    outincr =
        ((inlength / 2) < (HEX_STR_LEN + 1)) ? HEX_STR_LEN + 1 : inlength / 2;
  ...
  while (fgets(buf, sizeof(buf), stdin)) {
    if (strcmp(buf, type1_input) == 0) {
      ...
      if (!stream) {
        ...
        size = 0;
        error =
            getpart(&type1_output, &size, "ntlm_auth_type1", "output", stream);
        fclose(stream);
        if (error || size == 0) {
      ...
      if (!stream) {
        ...
        size = 0;
        error =
            getpart(&type3_output, &size, "ntlm_auth_type3", "output", stream);
        fclose(stream);
        if (error || size == 0) {
