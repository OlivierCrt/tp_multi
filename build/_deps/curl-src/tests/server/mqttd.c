...
static void logprotocol(mqttdir dir, const char *prefix, size_t remlen,
                        FILE *output, unsigned char *buffer, ssize_t len) {
  char data[12000] = "";
  ssize_t i;
  unsigned char *ptr = buffer;
  char *optr = data;
...
static int suback(FILE *dump, curl_socket_t fd, unsigned short packetid) {
  unsigned char packet[] = {MQTT_MSG_SUBACK, 0x03, 0, 0, /* filled in below */
                            0x00};
  ssize_t rc;
...
static size_t decode_length(unsigned char *buf, size_t buflen,
                            size_t *lenbytes) {
  size_t len = 0;
  size_t mult = 1;
  size_t i;
...
/* return 0 on success */
static int publish(FILE *dump, curl_socket_t fd, unsigned short packetid,
...
static int fixedheader(curl_socket_t fd, unsigned char *bytep,
  ...
  do {
    ...
    if (byte == MQTT_MSG_CONNECT) {
      ...
      if ((ssize_t)payload_len != (rc - 12)) {
        logmsg("Payload length mismatch, expected %x got %x", rc - 12,
               payload_len);
        goto end;
      }
      ...
      if (topic_len != (remaining_length - 5)) {
        logmsg("Wrong topic length, got %d expected %d", topic_len,
               remaining_length - 5);
        goto end;
      }
  ...
  do {
    ...
    if (FD_ISSET(sockfd, &fds_read)) {
      ...
      if (CURL_SOCKET_BAD == newfd) {
        error = SOCKERRNO;
        logmsg("accept(%d, NULL, NULL) failed with error: (%d) %s", sockfd,
               error, sstrerror(error));
      } else {
        logmsg("====> Client connect, fd %d. Read config from %s", newfd,
               configfile);
...
static curl_socket_t sockdaemon(curl_socket_t sock,
  ...
  if (rc) {
    error = SOCKERRNO;
    logmsg("Error binding socket on port %hu: (%d) %s", *listenport, error,
           sstrerror(error));
    sclose(sock);
    return CURL_SOCKET_BAD;
  }
  ...
  if (0 != rc) {
    error = SOCKERRNO;
    logmsg("listen(%d, 5) failed with error: (%d) %s", sock, error,
           sstrerror(error));
    sclose(sock);
    return CURL_SOCKET_BAD;
  }
...
int main(int argc, char *argv[]) {
  ...
  msnprintf(loglockfile, sizeof(loglockfile), "%s/%s/mqtt-%s.lock", logdir,
