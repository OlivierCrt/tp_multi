...
static ssize_t fullwrite(int filedes, const void *buffer, size_t nbytes) {
  ...
  do {
    ssize_t wc =
        write(filedes, (const unsigned char *)buffer + nwrite, nbytes - nwrite);
...
static bool disc_handshake(void) {
  ...
  do {
    ...
    if (!read_stdin(buffer, 5))
      return FALSE;
    logmsg("Received %c%c%c%c (on stdin)", buffer[0], buffer[1], buffer[2],
           buffer[3]);
...
/*
  ...
  accept()
*/
static bool juggle(curl_socket_t *sockfdp, curl_socket_t listenfd,
                   enum sockmode *mode) {
...
int main(int argc, char *argv[]) {
  ...
  if (connectport) {
    ...
    if (rc) {
      error = SOCKERRNO;
      logmsg("Error connecting to port %hu: (%d) %s", connectport, error,
             sstrerror(error));
      write_stdout("FAIL\n", 5);
      goto sockfilt_cleanup;
    }
