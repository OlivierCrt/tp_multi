...
/* Returns timeout in ms. 0 or negative number means the timeout has already
   triggered */
timediff_t Curl_pp_state_timeout(struct Curl_easy *data, struct pingpong *pp,
                                 bool disconnecting) {
  ...
  timeout_ms =
      response_time - Curl_timediff(Curl_now(), pp->response); /* spent time */

  if (data->set.timeout && !disconnecting) {
    /* if timeout is requested, find out how much remaining time we have */
    timediff_t timeout2_ms =
        data->set.timeout -                   /* timeout time */
 ...
 * made to never block
 */
CURLcode Curl_pp_vsendf(struct Curl_easy *data, struct pingpong *pp,
 ...
 */
CURLcode Curl_pp_readresp(struct Curl_easy *data, curl_socket_t sockfd,
                          struct pingpong *pp,
  ...
  while ((pp->nread_resp < (size_t)data->set.buffer_size) &&
    ...
    if (pp->cache) {
      ...
      result = Curl_read(data, sockfd, ptr,
                         data->set.buffer_size - pp->nread_resp, &gotbytes);
#ifdef HAVE_GSSAPI
    ...
    if (!keepon)
      ...
      for (i = 0; i < gotbytes; ptr++, i++) {
        ...
        if (*ptr == '\n') {
...
#endif
            Curl_debug(data, CURLINFO_HEADER_IN, pp->linestart_resp,
                       (size_t)perline);
           ...
           * headers.
           */
          result = Curl_client_write(data, CLIENTWRITE_INFO, pp->linestart_resp,
                                     perline);
          if (result)
            return result;
...
CURLcode Curl_pp_flushsend(struct Curl_easy *data, struct pingpong *pp) {
  /* we have a piece of a command still left to send */
  ssize_t written;
  CURLcode result =
      Curl_nwrite(data, FIRSTSOCKET, pp->sendthis + pp->sendsize - pp->sendleft,
                  pp->sendleft, &written);
  if (result)
    return result;
