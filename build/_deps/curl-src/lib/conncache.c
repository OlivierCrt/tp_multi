...
static void bundle_destroy(struct connectbundle *bundle) { free(bundle); }
...
int Curl_conncache_init(struct conncache *connc, int size) {
  ...
  Curl_hash_init(&connc->hash, size, Curl_hash_str, Curl_str_key_compare,
...
static void *conncache_add_bundle(struct conncache *connc, char *key,
...
void Curl_conncache_remove_conn(struct Curl_easy *data,
  ...
  if (bundle) {
    ...
    if (connc) {
      connc->num_conn--;
      DEBUGF(
          infof(data, "The cache now contains %zu members", connc->num_conn));
    }
    if (lock) {
      CONNCACHE_UNLOCK(data);
    }
 ...
 */
bool Curl_conncache_foreach(struct Curl_easy *data, struct conncache *connc,
                            void *param,
...
bool Curl_conncache_return_conn(struct Curl_easy *data,
  ...
  size_t maxconnects = (data->multi->maxconnects < 0)
                           ? data->multi->num_easy * 4
