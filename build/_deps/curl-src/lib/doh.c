...
static const char *const errors[] = {
    "",          "Bad label",       "Out of range",     "Label loop",
    "Too small", "Out of memory",   "RDATA length",     "Malformat",
    "Bad RCODE", "Unexpected TYPE", "Unexpected CLASS", "No content",
    "Bad ID",    "Name too long"};
...
/* @unittest 1655
 */
UNITTEST DOHcode doh_encode(const char *host, DNStype dnstype,
...
static size_t doh_write_cb(const void *contents, size_t size, size_t nmemb,
                           void *userp) {
...
static CURLcode dohprobe(struct Curl_easy *data, struct dnsprobe *p,
                         DNStype dnstype, const char *host, const char *url,
  ...
  DOHcode d =
      doh_encode(host, dnstype, p->dohbuffer, sizeof(p->dohbuffer), &p->dohlen);
  if (d) {
  ...
  if (!result) {
    ...
    {
      long mask =
          (data->set.ssl.enable_beast ? CURLSSLOPT_ALLOW_BEAST : 0) |
          (data->set.ssl.no_revoke ? CURLSSLOPT_NO_REVOKE : 0) |
          (data->set.ssl.no_partialchain ? CURLSSLOPT_NO_PARTIALCHAIN : 0) |
          (data->set.ssl.revoke_best_effort ? CURLSSLOPT_REVOKE_BEST_EFFORT
                                            : 0) |
          (data->set.ssl.native_ca_store ? CURLSSLOPT_NATIVE_CA : 0) |
          (data->set.ssl.auto_client_cert ? CURLSSLOPT_AUTO_CLIENT_CERT : 0);
...
struct Curl_addrinfo *Curl_doh(struct Curl_easy *data, const char *hostname,
  ...
  *waitp = TRUE; /* this never returns synchronously */
  ...
  result =
      dohprobe(data, &dohp->probe[DOH_PROBE_SLOT_IPADDR_V4], DNS_TYPE_A,
               hostname, data->set.str[STRING_DOH], data->multi, dohp->headers);
  if (result)
    goto error;
  dohp->pending++;
...
static DOHcode store_a(const unsigned char *doh, int index,
                       struct dohentry *d) {
...
static DOHcode store_aaaa(const unsigned char *doh, int index,
                          struct dohentry *d) {
  /* silently ignore addresses over the limit */
  if (d->numaddr < DOH_MAX_ADDR) {
    struct dohaddr *a = &d->addr[d->numaddr];
...
static DOHcode store_cname(const unsigned char *doh, size_t dohlen,
...
static DOHcode rdata(const unsigned char *doh, size_t dohlen,
                     unsigned short rdlength, unsigned short type, int index,
                     struct dohentry *d) {
  /* RDATA
     - A (TYPE 1):  4 bytes
...
UNITTEST DOHcode doh_decode(const unsigned char *doh, size_t dohlen,
...
static void showdoh(struct Curl_easy *data, const struct dohentry *d) {
  ...
  for (i = 0; i < d->numaddr; i++) {
    ...
    if (a->type == DNS_TYPE_A) {
      infof(data, "DoH A: %u.%u.%u.%u", a->ip.v4[0], a->ip.v4[1], a->ip.v4[2],
            a->ip.v4[3]);
...
static struct Curl_addrinfo *doh2ai(const struct dohentry *de,
...
CURLcode Curl_doh_is_resolved(struct Curl_easy *data,
  ...
  *dnsp = NULL; /* defaults to no response */
  ...
  if (!dohp->probe[DOH_PROBE_SLOT_IPADDR_V4].easy &&
    ...
    DOHcode rc[DOH_PROBE_SLOTS] = {DOH_OK, DOH_OK};
    struct dohentry de;
    int slot;
    ...
    for (slot = 0; slot < DOH_PROBE_SLOTS; slot++) {
      ...
      rc[slot] = doh_decode(Curl_dyn_uptr(&p->serverdoh),
                            Curl_dyn_len(&p->serverdoh), p->dnstype, &de);
