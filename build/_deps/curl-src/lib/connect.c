...
timediff_t Curl_timeleft(struct Curl_easy *data, struct curltime *nowp,
                         bool duringconnect) {
...
bool Curl_addr2string(struct sockaddr *sa, curl_socklen_t salen, char *addr,
                      int *port) {
...
static int conn_is_conn(struct Curl_easy *data, struct connectdata *conn,
                        void *param) {
...
curl_socket_t Curl_getconnectinfo(struct Curl_easy *data,
  ...
  if ((data->state.lastconnect_id != -1) && (data->multi_easy || data->multi)) {
    ...
    Curl_conncache_foreach(
        ...
        : data->multi_easy ? &data->multi_easy->conn_cache
...
typedef enum { SCFST_INIT, SCFST_WAITING, SCFST_DONE } cf_connect_state;
...
static CURLcode eyeballer_new(struct eyeballer **pballer,
                              cf_ip_connect_create *cf_create,
                              const struct Curl_addrinfo *addr, int ai_family,
                              struct eyeballer *primary, timediff_t delay_ms,
                              timediff_t timeout_ms, expire_id timeout_id) {
...
static void baller_close(struct eyeballer *baller, struct Curl_easy *data) {
  if (baller && baller->cf) {
    Curl_conn_cf_discard_chain(&baller->cf, data);
  }
}

static void baller_free(struct eyeballer *baller, struct Curl_easy *data) {
  if (baller) {
 ...
 */
static void baller_initiate(struct Curl_cfilter *cf, struct Curl_easy *data,
                            struct eyeballer *baller) {
 ...
 */
static CURLcode baller_start(struct Curl_cfilter *cf, struct Curl_easy *data,
                             struct eyeballer *baller, timediff_t timeoutms) {
  baller->error = 0;
...
static CURLcode baller_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
                               struct eyeballer *baller, struct curltime *now,
                               bool *connected) {
  (void)cf;
  ...
  if (!baller->result && !*connected) {
    ...
    if (!baller->result) {
      if (*connected) {
        ...
        infof(data,
              "%s connect timeout after %" CURL_FORMAT_TIMEDIFF_T
              "ms, move on!",
 ...
 */
static CURLcode is_connected(struct Curl_cfilter *cf, struct Curl_easy *data,
                             bool *connected) {
  struct cf_he_ctx *ctx = cf->ctx;
...
evaluate:
  *connected = FALSE; /* a very negative world view is best */
  ...
  for (i = 0; i < sizeof(ctx->baller) / sizeof(ctx->baller[0]); i++) {
    ...
    CURL_TRC_CF(data, cf, "%s connect -> %d, connected=%d", baller->name,
                baller->result, *connected);
    ...
    for (i = 0; i < sizeof(ctx->baller) / sizeof(ctx->baller[0]); i++) {
      ...
      if ((baller->primary && baller->primary->is_done) ||
        ...
        if (baller->is_done) {
          ...
          CURL_TRC_CF(data, cf,
                      "%s starting (timeout=%" CURL_FORMAT_TIMEDIFF_T "ms)",
                      baller->name, baller->timeoutms);
          ++ongoing;
          ++added;
        }
  ...
  for (i = 0; i < sizeof(ctx->baller) / sizeof(ctx->baller[0]); i++) {
    ...
    CURL_TRC_CF(data, cf, "%s assess started=%d, result=%d",
                baller ? baller->name : NULL, baller ? baller->has_started : 0,
  ...
  failf(data,
        "Failed to connect to %s port %u after "
        "%" CURL_FORMAT_TIMEDIFF_T " ms: %s",
        hostname, conn->port, Curl_timediff(now, data->progress.t_startsingle),
 ...
 */
static CURLcode start_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
                              const struct Curl_dns_entry *remotehost) {
  struct cf_he_ctx *ctx = cf->ctx;
  struct connectdata *conn = cf->conn;
  ...
  if (conn->ip_version == CURL_IPRESOLVE_WHATEVER) {
    ...
    ai_family0 = remotehost->addr ? remotehost->addr->ai_family : 0;
#ifdef ENABLE_IPV6
    ai_family1 = ai_family0 == AF_INET6 ? AF_INET : AF_INET6;
#else
    ai_family1 = AF_UNSPEC;
#endif
  } else {
    /* only one IP version is allowed */
    ai_family0 = (conn->ip_version == CURL_IPRESOLVE_V4) ? AF_INET :
#ifdef ENABLE_IPV6
                                                         AF_INET6;
#else
                                                         AF_UNSPEC;
  ...
  if (result)
    return result;
  CURL_TRC_CF(data, cf, "created %s (timeout %" CURL_FORMAT_TIMEDIFF_T "ms)",
  ...
  if (addr1) {
    ...
    result = eyeballer_new(&ctx->baller[1], ctx->cf_create, addr1, ai_family1,
                           ...
                           data->set.happy_eyeballs_timeout, timeout_ms,
...
static CURLcode cf_he_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  *done = FALSE;
  ...
  case SCFST_WAITING:
    ...
    if (!result && *done) {
      ...
      Curl_conn_cf_cntrl(cf->next, data, TRUE, CF_CTRL_CONN_INFO_UPDATE, 0,
                         NULL);
...
static void cf_he_close(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct cf_he_ctx *ctx = cf->ctx;
...
static struct curltime get_max_baller_time(struct Curl_cfilter *cf,
                                           struct Curl_easy *data, int query) {
  struct cf_he_ctx *ctx = cf->ctx;
...
static CURLcode cf_he_query(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (!cf->connected) {
    ...
    case CF_QUERY_CONNECT_REPLY_MS: {
      ...
      for (i = 0; i < sizeof(ctx->baller) / sizeof(ctx->baller[0]); i++) {
        ...
        if (baller && baller->cf &&
            !baller->cf->cft->query(baller->cf, data, query, &breply_ms,
                                    NULL)) {
 ...
 *                   actual connects.
 */
static CURLcode cf_happy_eyeballs_create(
    struct Curl_cfilter **pcf, struct Curl_easy *data, struct connectdata *conn,
    cf_ip_connect_create *cf_create, const struct Curl_dns_entry *remotehost,
    int transport) {
...
static CURLcode cf_he_insert_after(struct Curl_cfilter *cf_at,
  ...
  result = cf_happy_eyeballs_create(&cf, data, cf_at->conn, cf_create,
...
static void cf_setup_close(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct cf_setup_ctx *ctx = cf->ctx;
...
static CURLcode cf_setup_add(struct Curl_easy *data, struct connectdata *conn,
                             int sockindex,
                             const struct Curl_dns_entry *remotehost,
                             int transport, int ssl_mode) {
  struct Curl_cfilter *cf;
...
CURLcode Curl_conn_setup(struct Curl_easy *data, struct connectdata *conn,
                         int sockindex, const struct Curl_dns_entry *remotehost,
                         int ssl_mode) {
  CURLcode result = CURLE_OK;
  ...
  if (!conn->cfilter[sockindex] && conn->handler->protocol == CURLPROTO_HTTPS) {
  ...
  if (!conn->cfilter[sockindex]) {
    result = cf_setup_add(data, conn, sockindex, remotehost, conn->transport,
                          ssl_mode);
    if (result)
      goto out;
  }
