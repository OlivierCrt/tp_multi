...
static void socksstate(struct socks_state *sx, struct Curl_easy *data,
  ...
  static const char *const socks_statename[] = {
      "INIT",           "SOCKS_INIT",  "SOCKS_SEND",  "SOCKS_READ_INIT",
      "SOCKS_READ",     "GSSAPI_INIT", "AUTH_INIT",   "AUTH_SEND",
      "AUTH_READ",      "REQ_INIT",    "RESOLVING",   "RESOLVED",
      "RESOLVE_REMOTE", "REQ_SEND",    "REQ_SENDING", "REQ_READ",
      "REQ_READ_MORE",  "DONE"};
#endif
...
#ifdef DEBUG_AND_VERBOSE
  infof(data, "SXSTATE: %s => %s; line %d", socks_statename[oldstate],
        socks_statename[sx->state], lineno);
#endif
}
...
static CURLproxycode socks_state_recv(struct Curl_cfilter *cf,
  ...
  nread = Curl_conn_cf_recv(cf->next, data, (char *)sx->outp, sx->outstanding,
                            &result);
  if (nread <= 0) {
    if (CURLE_AGAIN == result) {
      return CURLPX_OK;
 ...
 */
static CURLproxycode do_SOCKS4(struct Curl_cfilter *cf, struct socks_state *sx,
                               struct Curl_easy *data) {
  ...
  case CONNECT_SOCKS_INIT:
    ...
    infof(data, "SOCKS4 communication to %s:%d", sx->hostname, sx->remote_port);
  ...
  case CONNECT_REQ_INIT:
    ...
    {
      size_t packetsize =
          9 + strlen((char *)socksreq + 8); /* size including NUL */
 ...
 * destination server.
 */
static CURLproxycode do_SOCKS5(struct Curl_cfilter *cf, struct socks_state *sx,
                               struct Curl_easy *data) {
  /*
  ...
  */
  ...
  case CONNECT_SOCKS_INIT:
    if (conn->bits.httpproxy)
      infof(data, "SOCKS5: connecting to HTTP proxy %s port %d", sx->hostname,
            sx->remote_port);
  ...
  case CONNECT_RESOLVE_REMOTE:
    ...
    if (!socks5_resolve_local) {
      ...
      infof(data, "SOCKS5 connect to %s:%d (remotely resolved)", sx->hostname,
            sx->remote_port);
    }
    /* FALLTHROUGH */
  ...
  case CONNECT_REQ_READ:
    ...
    if (CURLPX_OK != presult)
      ...
      int code = socksreq[1];
      failf(data, "Can't complete SOCKS5 connection to %s. (%d)", sx->hostname,
            (unsigned char)socksreq[1]);
      if (code < 9) {
...
static CURLcode socks_proxy_cf_connect(struct Curl_cfilter *cf,
  ...
  if (sx->state == CONNECT_INIT) {
    ...
    sx->hostname = conn->bits.httpproxy           ? conn->http_proxy.host.name
                   : conn->bits.conn_to_host      ? conn->conn_to_host.name
                   : sockindex == SECONDARYSOCKET ? conn->secondaryhostname
                                                  : conn->host.name;
    sx->remote_port = conn->bits.httpproxy ? (int)conn->http_proxy.port
...
static void socks_cf_get_host(struct Curl_cfilter *cf, struct Curl_easy *data,
