...
static size_t trailers_read(char *buffer, size_t size, size_t nitems,
  ...
  size_t bytes_left =
      Curl_dyn_len(trailers_buf) - data->state.trailers_bytes_sent;
  size_t to_copy = (size * nitems < bytes_left) ? size * nitems : bytes_left;
  if (to_copy) {
    memcpy(buffer, Curl_dyn_ptr(trailers_buf) + data->state.trailers_bytes_sent,
           to_copy);
...
CURLcode Curl_fillreadbuffer(struct Curl_easy *data, size_t bytes,
  ...
  if (data->state.trailers_state == TRAILERS_INITIALIZED) {
    ...
    if (trailers_ret_code == CURL_TRAILERFUNC_OK) {
      result =
          Curl_http_compile_trailers(trailers, &data->state.trailers_buf, data);
    } else {
  ...
  if (!data->req.forbidchunk && data->req.upload_chunky) {
    ...
    if (data->state.trailers_state != TRAILERS_SENDING) {
      char hexbuffer[11] = "";
      hexlen = msnprintf(hexbuffer, sizeof(hexbuffer), "%zx%s", nread,
                         endofline_native);
      ...
      if ((nread - hexlen) == 0 && data->set.trailer_callback != NULL &&
        ...
        memcpy(data->req.upload_fromhere + nread, endofline_network,
 ...
 */
static CURLcode readwrite_data(struct Curl_easy *data, struct connectdata *conn,
                               struct SingleRequest *k, int *didwhat,
                               bool *done, bool *comeback) {
  ...
  int maxloops = 100;
  curl_off_t max_recv =
      data->set.max_recv_speed ? data->set.max_recv_speed : CURL_OFF_T_MAX;
  ...
  do {
    ...
    *didwhat |= KEEP_RECV;
    ...
    if (!k->header && (nread > 0 || is_empty_data)) {
      ...
      if (data->set.verbose) {
        if (k->badheader) {
          Curl_debug(data, CURLINFO_DATA_IN, Curl_dyn_ptr(&data->state.headerb),
      ...
      if (k->chunk) {
        ...
        if (CHUNKE_STOP == res) {
          ...
          if (conn->chunk.datasize) {
            infof(data,
                  "Leftovers after chunking: % " CURL_FORMAT_CURL_OFF_T
                  "u bytes",
                  conn->chunk.datasize);
          }
        }
          ...
          if (k->maxdownload == -1 || (curl_off_t)headlen <= k->maxdownload)
            result =
                Curl_client_write(data, CLIENTWRITE_BODY,
                                  Curl_dyn_ptr(&data->state.headerb), headlen);
          else
...
static void win_update_buffer_size(curl_socket_t sockfd) {
  ...
  ULONG ideal;
  DWORD ideallen;
  result = WSAIoctl(sockfd, SIO_IDEAL_SEND_BACKLOG_QUERY, 0, 0, &ideal,
                    sizeof(ideal), &ideallen, 0, 0);
  if (result == 0) {
    setsockopt(sockfd, SOL_SOCKET, SO_SNDBUF, (const char *)&ideal,
               sizeof(ideal));
  }
}
#else
...
static CURLcode readwrite_upload(struct Curl_easy *data,
                                 struct connectdata *conn, int *didwhat) {
  ssize_t i, si;
  ...
  *didwhat |= KEEP_SEND;
  ...
  do {
    ...
    if (0 != k->upload_present &&
        ...
        !k->upload_chunky && /*(variable sized chunked header; append not
                                safe)*/
    ...
    if (0 == k->upload_present || offset) {
      ...
      if (!k->upload_done) {
        ...
        result = Curl_fillreadbuffer(
            data, data->set.upload_buffer_size - offset, &fillcount);
        k->upload_fromhere -= offset;
        if (result)
          return result;
    ...
    result = Curl_write(data, conn->writesockfd, /* socket to send to */
    ...
    if (nbody) {
      ...
      Curl_debug(data, CURLINFO_DATA_OUT,
                 &k->upload_fromhere[bytes_written - nbody], (size_t)nbody);
 ...
 * function should get called again without select() or similar in between!
 */
CURLcode Curl_readwrite(struct connectdata *conn, struct Curl_easy *data,
                        bool *done, bool *comeback) {
  struct SingleRequest *k = &data->req;
  CURLcode result;
  struct curltime now;
  ...
  if (k->keepon) {
    ...
    if (!(data->req.no_body) && (k->size != -1) && (k->bytecount != k->size) &&
#ifdef CURL_DO_LINEEND_CONV
        ...
        */
        ...
        !k->newurl) {
      failf(data,
            "transfer closed with %" CURL_FORMAT_CURL_OFF_T
            " bytes remaining to read",
            k->size - k->bytecount);
      result = CURLE_PARTIAL_FILE;
      goto out;
    }
    if (!(data->req.no_body) && k->chunk && (conn->chunk.state != CHUNK_STOP)) {
      /*
...
CURLcode Curl_pretransfer(struct Curl_easy *data) {
  ...
  if (!data->state.url && data->set.uh) {
    ...
    uc = curl_url_get(data->set.uh, CURLUPART_URL,
                      &data->set.str[STRING_SET_URL], 0);
    if (uc) {
      failf(data, "No URL set");
      return CURLE_URL_MALFORMAT;
    }
  ...
  if (!result) {
    ...
    if (data->state.wildcardmatch) {
      ...
      wc = data->wildcard;
      if ((wc->state < CURLWC_INIT) || (wc->state >= CURLWC_CLEAN)) {
        if (wc->ftpwc)
          wc->dtor(wc->ftpwc);
  ...
  if (!result)
    result =
        Curl_setstropt(&data->state.aptr.user, data->set.str[STRING_USERNAME]);
  if (!result)
                     ...
                     followtype type) /* see transfer.h */
{
  ...
  if ((type != FOLLOW_RETRY) && (data->req.httpcode != 401) &&
  ...
  case 303: /* See Other */
    ...
    if (data->state.httpreq != HTTPREQ_GET &&
      ...
      data->state.httpreq = HTTPREQ_GET;
      infof(data, "Switch to %s", data->req.no_body ? "HEAD" : "GET");
    }
    break;
...
void Curl_setup_transfer(
  ...
  if (conn->bits.multiplex || conn->httpversion >= 20 || httpsending) {
    ...
    conn->sockfd = sockindex == -1
                       ? ((writesockindex == -1 ? CURL_SOCKET_BAD
                                                : conn->sock[writesockindex]))
                       : conn->sock[sockindex];
