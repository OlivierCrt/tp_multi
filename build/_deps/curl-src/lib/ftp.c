...
static CURLcode ftp_sendquote(struct Curl_easy *data, struct connectdata *conn,
...
static void ftp_pasv_verbose(struct Curl_easy *data, struct Curl_addrinfo *ai,
...
static CURLcode ftp_state_quote(struct Curl_easy *data, bool init,
                                ftpstate instate);
static CURLcode ftp_nb_type(struct Curl_easy *data, struct connectdata *conn,
...
static CURLcode ftp_disconnect(struct Curl_easy *data, struct connectdata *conn,
...
static int ftp_getsock(struct Curl_easy *data, struct connectdata *conn,
                       curl_socket_t *socks);
static int ftp_domore_getsock(struct Curl_easy *data, struct connectdata *conn,
                              curl_socket_t *socks);
...
static CURLcode ftp_readresp(struct Curl_easy *data, curl_socket_t sockfd,
                             struct pingpong *pp, int *ftpcode, size_t *size);
...
const struct Curl_handler Curl_handler_ftps = {
    ...
    CURLPROTO_FTP,        /* family */
    PROTOPT_SSL | PROTOPT_DUAL | PROTOPT_CLOSEACTION | PROTOPT_NEEDSPWD |
        PROTOPT_NOURLQUERY | PROTOPT_WILDCARD /* flags */
};
#endif
...
static CURLcode AcceptServerConnect(struct Curl_easy *data) {
  ...
  if (data->set.fsockopt) {
    ...
    error =
        data->set.fsockopt(data->set.sockopt_client, s, CURLSOCKTYPE_ACCEPT);
...
static CURLcode InitiateTransfer(struct Curl_easy *data) {
  ...
  if (conn->proto.ftpc.state_saved == FTP_STOR) {
    ...
    /* FTP download: */
    Curl_setup_transfer(data, SECONDARYSOCKET, conn->proto.ftpc.retr_size_saved,
                        FALSE, -1);
  }
...
static CURLcode AllowServerConnect(struct Curl_easy *data, bool *connected) {
  ...
  *connected = FALSE;
  ...
  if (*connected) {
    ...
    Curl_expire(data,
                data->set.accepttimeout ? data->set.accepttimeout
...
static const char *const ftp_state_names[] = {
    "STOP",      "WAIT220",   "AUTH",      "USER",          "PASS",
    "ACCT",      "PBSZ",      "PROT",      "CCC",           "PWD",
    "SYST",      "NAMEFMT",   "QUOTE",     "RETR_PREQUOTE", "STOR_PREQUOTE",
    "POSTQUOTE", "CWD",       "MKD",       "MDTM",          "TYPE",
    "LIST_TYPE", "RETR_TYPE", "STOR_TYPE", "SIZE",          "RETR_SIZE",
    "STOR_SIZE", "REST",      "RETR_REST", "PORT",          "PRET",
    "PASV",      "LIST",      "RETR",      "STOR",          "QUIT"};
#endif
...
static void _ftp_state(struct Curl_easy *data, ftpstate newstate
  ...
  if (ftpc->state != newstate)
    infof(data, "FTP %p (line %d) state change from %s to %s", (void *)ftpc,
          lineno, ftp_state_names[ftpc->state], ftp_state_names[newstate]);
#endif
#endif
...
static CURLcode ftp_state_user(struct Curl_easy *data,
                               struct connectdata *conn) {
  CURLcode result = Curl_pp_sendf(data, &conn->proto.ftpc.pp, "USER %s",
                                  conn->user ? conn->user : "");
  if (!result) {
    struct ftp_conn *ftpc = &conn->proto.ftpc;
...
static bool match_pasv_6nums(const char *p, unsigned int *array) /* 6 numbers */
{
  int i;
  for (i = 0; i < 6; i++) {
    unsigned long num;
...
static CURLcode ftp_state_pasv_resp(struct Curl_easy *data, int ftpcode) {
  ...
  result = Curl_conn_setup(data, conn, SECONDARYSOCKET, addr,
                           conn->bits.ftp_use_data_ssl ? CURL_CF_SSL_ENABLE
                                                       : CURL_CF_SSL_DISABLE);
...
static int twodigit(const char *p) { return (p[0] - '0') * 10 + (p[1] - '0'); }
...
static CURLcode client_write_header(struct Curl_easy *data, char *buf,
                                    size_t blen) {
...
static CURLcode ftp_state_mdtm_resp(struct Curl_easy *data, int ftpcode) {
  ...
  case 213: {
    ...
    if (ftp_213_date(&data->state.buffer[4], &year, &month, &day, &hour,
                     &minute, &second)) {
      /* we have a time, reformat it */
      char timebuf[24];
      msnprintf(timebuf, sizeof(timebuf), "%04d%02d%02d %02d:%02d:%02d GMT",
    ...
    if (data->req.no_body && ftpc->file && data->set.get_filetime &&
      ...
      headerbuflen =
          msnprintf(headerbuf, sizeof(headerbuf),
                    ...
                    Curl_month[tm->tm_mon], tm->tm_year + 1900, tm->tm_hour,
                    tm->tm_min, tm->tm_sec);
...
static CURLcode ftp_state_type_resp(struct Curl_easy *data, int ftpcode,
                                    ftpstate instate) {
  CURLcode result = CURLE_OK;
  struct connectdata *conn = data->conn;
...
static CURLcode ftp_state_size_resp(struct Curl_easy *data, int ftpcode,
                                    ftpstate instate) {
  CURLcode result = CURLE_OK;
...
static CURLcode ftp_state_rest_resp(struct Curl_easy *data,
                                    struct connectdata *conn, int ftpcode,
                                    ftpstate instate) {
  CURLcode result = CURLE_OK;
  struct ftp_conn *ftpc = &conn->proto.ftpc;
...
static CURLcode ftp_state_stor_resp(struct Curl_easy *data, int ftpcode,
                                    ftpstate instate) {
  CURLcode result = CURLE_OK;
  struct connectdata *conn = data->conn;
...
static CURLcode ftp_state_get_resp(struct Curl_easy *data, int ftpcode,
                                   ftpstate instate) {
  CURLcode result = CURLE_OK;
  struct FTP *ftp = data->req.p.ftp;
  struct connectdata *conn = data->conn;
  ...
  if ((ftpcode == 150) || (ftpcode == 125)) {
    ...
    if ((instate != FTP_LIST) && !data->state.prefer_ascii &&
        !data->set.ignorecl && (ftp->downloadsize < 1)) {
      /*
...
static CURLcode ftp_state_user_resp(struct Curl_easy *data, int ftpcode) {
  CURLcode result = CURLE_OK;
  struct connectdata *conn = data->conn;
  struct ftp_conn *ftpc = &conn->proto.ftpc;
  ...
  if (ftpcode) {
    ...
    case FTP_WAIT220:
      ...
      if (data->set.use_ssl && !conn->bits.ftp_use_control_ssl) {
        ...
        result =
            Curl_pp_sendf(data, &ftpc->pp, "AUTH %s", ftpauth[ftpc->count1]);
        if (!result)
    ...
    case FTP_CWD:
      if (ftpcode / 100 != 2) {
        ...
        if (data->set.ftp_create_missing_dirs && ftpc->cwdcount &&
            !ftpc->count2) {
          /* try making it */
...
static CURLcode ftp_done(struct Curl_easy *data, CURLcode status,
    ...
    if ((data->set.ftp_filemethod == FTPFILE_NOCWD) && (rawPath[0] == '/'))
      ...
      if (!ftpc->cwdfail) {
        if (data->set.ftp_filemethod == FTPFILE_NOCWD)
          ...
          pathLen -=
              ftpc->file ? strlen(ftpc->file) : 0; /* file is url-decoded */
  ...
  if (result || premature)
    ...
    if ((-1 != data->state.infilesize) &&
        ...
        !data->set.crlf && (ftp->transfer == PPTRANSFER_BODY)) {
      failf(data,
    ...
    if ((-1 != data->req.size) && (data->req.size != data->req.bytecount) &&
      ...
      failf(data,
            ...
            data->req.bytecount);
      result = CURLE_PARTIAL_FILE;
    } else if (!ftpc->dont_check && !data->req.bytecount &&
               (data->req.size > 0)) {
    ...
    if (ftpc->wait_data_conn) {
      ...
      *completep = (int)complete;
      ...
      if (result)
        ...
        result =
            ftp_nb_type(data, conn, data->state.prefer_ascii, FTP_RETR_TYPE);
        if (result)
          return result;
      }
...
static CURLcode
ftp_perform(struct Curl_easy *data,
  ...
  for (;;) {
    ...
    case CURLWC_DOWNLOADING: {
      ...
      if (data->set.chunk_bgn) {
        long userresponse;
        Curl_set_in_callback(data, true);
        userresponse = data->set.chunk_bgn(finfo, data->set.wildcardptr,
                                           (int)wildcard->filelist.size);
        Curl_set_in_callback(data, false);
        switch (userresponse) {
    ...
    case CURLWC_SKIP: {
      ...
      Curl_llist_remove(&wildcard->filelist, wildcard->filelist.head, NULL);
      wildcard->state =
          (wildcard->filelist.size == 0) ? CURLWC_CLEAN : CURLWC_DOWNLOADING;
      continue;
    }
...
static CURLcode ftp_regular_transfer(struct Curl_easy *data,
  ...
  result = ftp_perform(data, &connected, /* have we connected after PASV/PORT */
