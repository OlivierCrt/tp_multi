...
static CURLcode ftp_pl_insert_finfo(struct Curl_easy *data,
  ...
  finfo->strings.group =
      parser->offsets.group ? str + parser->offsets.group : NULL;
  finfo->strings.perm =
      parser->offsets.perm ? str + parser->offsets.perm : NULL;
  finfo->strings.target = parser->offsets.symlink_target
                              ? str + parser->offsets.symlink_target
                              : NULL;
  finfo->strings.time = str + parser->offsets.time;
  finfo->strings.user =
      parser->offsets.user ? str + parser->offsets.user : NULL;
      ...
      case PL_UNIX_SIZE:
        ...
        case PL_UNIX_SIZE_NUMBER:
          ...
          if (c == ' ') {
            ...
            curl_off_t fsize;
            mem[parser->item_offset + parser->item_length - 1] = 0;
            if (!curlx_strtoofft(mem + parser->item_offset, &p, 10, &fsize)) {
        ...
        case PL_WINNT_DIRORSIZE_CONTENT:
          ...
          if (c == ' ') {
            ...
            if (strcmp("<DIR>", mem + parser->item_offset) == 0) {
              ...
              char *endptr;
              if (curlx_strtoofft(mem + parser->item_offset, &endptr, 10,
                                  &finfo->size)) {
                parser->error = CURLE_FTP_BAD_FILE_LIST;
                goto fail;
              }
