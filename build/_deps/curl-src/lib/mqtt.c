...
static CURLcode mqtt_done(struct Curl_easy *data, CURLcode status,
                          bool premature);
...
static int add_passwd(const char *passwd, const size_t plen, char *pkt,
                      const size_t start, int remain_pos) {
...
static int add_user(const char *username, const size_t ulen, unsigned char *pkt,
                    const size_t start, int remain_pos) {
  /* magic number that need to be set properly */
  const size_t conn_flags_pos = remain_pos + 8;
  if (ulen > 0xffff)
...
static CURLcode mqtt_connect(struct Curl_easy *data) {
  ...
  if (ulen) {
    ...
    rc = add_user(username, ulen, (unsigned char *)packet, start_user,
                  remain_pos);
    if (rc) {
...
static CURLcode mqtt_recv_atleast(struct Curl_easy *data, size_t nbytes) {
  ...
  if (rlen < nbytes) {
    ...
    result = Curl_read(data, data->conn->sock[FIRSTSOCKET], (char *)readbuf,
                       nbytes - rlen, &nread);
    if (result)
      return result;
    DEBUGASSERT(nread >= 0);
...
static CURLcode mqtt_verify_connack(struct Curl_easy *data) {
  ...
  if (ptr[0] != 0x00 || ptr[1] != 0x00) {
    failf(data, "Expected %02x%02x but got %02x%02x", 0x00, 0x00, ptr[0],
          ptr[1]);
    Curl_dyn_reset(&mq->recvbuf);
    result = CURLE_WEIRD_SERVER_REPLY;
    goto fail;
...
static CURLcode mqtt_get_topic(struct Curl_easy *data, char **topic,
                               size_t *topiclen) {
...
static CURLcode mqtt_verify_suback(struct Curl_easy *data) {
  ...
  if (((unsigned char)ptr[0]) != ((mqtt->packetid >> 8) & 0xff) ||
      ((unsigned char)ptr[1]) != (mqtt->packetid & 0xff) || ptr[2] != 0x00) {
    Curl_dyn_reset(&mq->recvbuf);
    result = CURLE_WEIRD_SERVER_REPLY;
    goto fail;
  }
...
static size_t mqtt_decode_len(unsigned char *buf, size_t buflen,
                              size_t *lenbytes) {
  size_t len = 0;
  size_t mult = 1;
  size_t i;
...
static const char *statenames[] = {
    "MQTT_FIRST",      "MQTT_REMAINING_LENGTH", "MQTT_CONNACK",
    "MQTT_SUBACK",     "MQTT_SUBACK_COMING",    "MQTT_PUBWAIT",
    "MQTT_PUB_REMAIN",
...
static void mqstate(struct Curl_easy *data, enum mqttstate state,
                    enum mqttstate nextstate) /* used if state == FIRST */
{
  struct connectdata *conn = data->conn;
  struct mqtt_conn *mqtt = &conn->proto.mqtt;
#ifdef CURLDEBUG
  infof(data, "%s (from %s) (next is %s)", statenames[state],
...
static CURLcode mqtt_read_publish(struct Curl_easy *data, bool *done) {
  ...
  case MQTT_PUBWAIT:
    ...
    if (data->set.max_filesize && (curl_off_t)remlen > data->set.max_filesize) {
