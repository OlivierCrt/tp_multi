...
static enum {
  ...
  GZIP_BAD,
  GZIP_UNDERFLOW
} check_gzip_header(unsigned char const *data, ssize_t len,
                    ssize_t *headerlen) {
  int method, flags;
...
static CURLcode brotli_unencode_write(struct Curl_easy *data,
  ...
  while ((nbytes || r == BROTLI_DECODER_RESULT_NEEDS_MORE_OUTPUT) &&
    ...
    dst = (uint8_t *)decomp;
    dstleft = DSIZ;
    r = BrotliDecoderDecompressStream(bp->br, &nbytes, &src, &dstleft, &dst,
                                      NULL);
    result =
        Curl_unencode_write(data, writer->downstream, decomp, DSIZ - dstleft);
    if (result)
      break;
    switch (r) {
...
static CURLcode zstd_unencode_write(struct Curl_easy *data,
  ...
  for (;;) {
    ...
    if (out.pos > 0) {
      result =
          Curl_unencode_write(data, writer->downstream, zp->decomp, out.pos);
      if (result)
        break;
    }
...
static const struct content_encoding *const encodings[] = {&identity_encoding,
#ifdef HAVE_LIBZ
                                                           &deflate_encoding,
                                                           &gzip_encoding,
#endif
