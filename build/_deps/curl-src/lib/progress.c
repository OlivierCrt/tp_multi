...
static void time2str(char *r, curl_off_t seconds) {
  ...
  if (h <= CURL_OFF_T_C(99)) {
    ...
    curl_off_t s =
        (seconds - (h * CURL_OFF_T_C(3600))) - (m * CURL_OFF_T_C(60));
    msnprintf(r, 9,
              "%2" CURL_FORMAT_CURL_OFF_T ":%02" CURL_FORMAT_CURL_OFF_T
    ...
    if (d <= CURL_OFF_T_C(999))
      msnprintf(r, 9,
                "%3" CURL_FORMAT_CURL_OFF_T "d %02" CURL_FORMAT_CURL_OFF_T "h",
                d, h);
    else
...
void Curl_pgrsTimeWas(struct Curl_easy *data, timerid timer,
  ...
  case TIMER_REDIRECT:
    data->progress.t_redirect =
        Curl_timediff_us(timestamp, data->progress.start);
    break;
  }
  if (delta) {
 ...
 */
timediff_t Curl_pgrsLimitWaitTime(curl_off_t cursize, curl_off_t startsize,
                                  curl_off_t limit, struct curltime start,
                                  struct curltime now) {
...
static void progress_meter(struct Curl_easy *data) {
  ...
  if (total_expected_transfer > CURL_OFF_T_C(10000))
    total_percen =
        total_transfer / (total_expected_transfer / CURL_OFF_T_C(100));
...
int Curl_pgrsUpdate(struct Curl_easy *data) {
  ...
  if (!(data->progress.flags & PGRS_HIDE)) {
    if (data->set.fxferinfo) {
      ...
      result =
          data->set.fxferinfo(data->set.progress_client, data->progress.size_dl,
