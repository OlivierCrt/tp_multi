...
/* The preferred method on Mac OS X (10.2 and later) to prevent SIGPIPEs when
   ...
   systems? */
static void nosigpipe(struct Curl_easy *data, curl_socket_t sockfd) {
  int onoff = 1;
...
static void tcpkeepalive(struct Curl_easy *data, curl_socket_t sockfd) {
  ...
  if (setsockopt(sockfd, SOL_SOCKET, SO_KEEPALIVE, (void *)&optval,
    ...
    if (WSAIoctl(sockfd, SIO_KEEPALIVE_VALS, (LPVOID)&vals, sizeof(vals), NULL,
                 0, &dummy, NULL, NULL) != 0) {
      infof(data, "Failed to set SIO_KEEPALIVE_VALS on fd %d: %d", (int)sockfd,
            WSAGetLastError());
    }
#else
#ifdef TCP_KEEPIDLE
    optval = curlx_sltosi(data->set.tcp_keepidle);
    KEEPALIVE_FACTOR(optval);
    if (setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPIDLE, (void *)&optval,
                   sizeof(optval)) < 0) {
    ...
    optval = curlx_sltosi(data->set.tcp_keepidle);
    KEEPALIVE_FACTOR(optval);
    if (setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPALIVE, (void *)&optval,
                   sizeof(optval)) < 0) {
    ...
    optval = curlx_sltosi(data->set.tcp_keepintvl);
    KEEPALIVE_FACTOR(optval);
    if (setsockopt(sockfd, IPPROTO_TCP, TCP_KEEPINTVL, (void *)&optval,
                   sizeof(optval)) < 0) {
...
static CURLcode bindlocal(struct Curl_easy *data, struct connectdata *conn,
  ...
  if (dev && (strlen(dev) < 255)) {
    ...
    if (!is_host) {
       ...
       */
      if (setsockopt(sockfd, SOL_SOCKET, SO_BINDTODEVICE, dev,
                     (curl_socklen_t)strlen(dev) + 1) == 0) {
      ...
      case IF2IP_FOUND:
         ...
         */
        infof(data, "Local Interface %s is ip %s using address family %i", dev,
              myhost, af);
        done = 1;
        break;
      }
    ...
    if (!is_interface) {
      ...
      if (h) {
        ...
        infof(data, "Name '%s' family %i resolved to '%s' family %i", dev, af,
              myhost, h->addr->ai_family);
  ...
  for (;;) {
    if (bind(sockfd, sock, sizeof_sa) >= 0) {
      ...
      if (getsockname(sockfd, (struct sockaddr *)&add, &size) < 0) {
        ...
        failf(data, "getsockname() failed with errno %d: %s", error,
  ...
  }
  {
    char buffer[STRERROR_LEN];
    data->state.os_errno = error = SOCKERRNO;
    failf(data, "bind failed with errno %d: %s", error,
          Curl_strerror(error, buffer, sizeof(buffer)));
  }
...
static ssize_t nw_in_read(void *reader_ctx, unsigned char *buf, size_t len,
                          CURLcode *err) {
  ...
  *err = CURLE_OK;
  ...
  CURL_TRC_CF(rctx->data, rctx->cf, "nw_in_read(len=%zu) -> %d, err=%d", len,
              (int)nread, *err);
  return nread;
}
  ...
  if (ctx && CURL_SOCKET_BAD != ctx->sock) {
    if (ctx->active) {
      ...
      if (ctx->sock == cf->conn->sock[cf->sockindex]) {
        CURL_TRC_CF(data, cf,
                    "cf_socket_close(%" CURL_FORMAT_SOCKET_T ", active)",
                    ctx->sock);
        socket_close(data, cf->conn, !ctx->accepted, ctx->sock);
        cf->conn->sock[cf->sockindex] = CURL_SOCKET_BAD;
      } else {
        CURL_TRC_CF(data, cf,
                    "cf_socket_close(%" CURL_FORMAT_SOCKET_T
                    ") no longer at conn->sock[], discarding",
                    ctx->sock);
      ...
      CURL_TRC_CF(data, cf,
                  "cf_socket_close(%" CURL_FORMAT_SOCKET_T ", not active)",
                  ctx->sock);
      socket_close(data, cf->conn, !ctx->accepted, ctx->sock);
      ctx->sock = CURL_SOCKET_BAD;
    }
...
static CURLcode set_local_ip(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct cf_socket_ctx *ctx = cf->ctx;
  ...
  if (!(data->conn->handler->protocol & CURLPROTO_TFTP)) {
    ...
    if (!Curl_addr2string((struct sockaddr *)&ssloc, slen, ctx->l_ip,
                          &ctx->l_port)) {
      failf(data, "ssloc inet_ntop() failed with errno %d: %s", errno,
...
static CURLcode set_remote_ip(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct cf_socket_ctx *ctx = cf->ctx;

  /* store remote address and port used in this connection attempt */
  if (!Curl_addr2string(&ctx->addr.sa_addr, ctx->addr.addrlen, ctx->r_ip,
                        &ctx->r_port)) {
    char buffer[STRERROR_LEN];

    ctx->error = errno;
    /* malformed address or bug in inet_ntop, try next address */
    failf(data, "sa_addr inet_ntop() failed with errno %d: %s", errno,
          Curl_strerror(errno, buffer, sizeof(buffer)));
    return CURLE_FAILED_INIT;
  }
  return CURLE_OK;
...
static CURLcode cf_socket_open(struct Curl_cfilter *cf,
...
#else
  is_tcp = (ctx->addr.family == AF_INET) && ctx->addr.socktype == SOCK_STREAM;
#endif
  ...
  if (data->set.fsockopt) {
    ...
    Curl_set_in_callback(data, true);
    error = data->set.fsockopt(data->set.sockopt_client, ctx->sock,
                               CURLSOCKTYPE_IPCXN);
    Curl_set_in_callback(data, false);
...
static int do_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (is_tcp_fastopen) {
    ...
    if (setsockopt(ctx->sock, IPPROTO_TCP, TCP_FASTOPEN_CONNECT,
      ...
      infof(data, "Failed to enable TCP Fast Open on fd %" CURL_FORMAT_SOCKET_T,
            ctx->sock);
...
static CURLcode cf_tcp_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
...
out:
  if (result) {
    if (ctx->error) {
      ...
      {
        char buffer[STRERROR_LEN];
        infof(data, "connect to %s port %u failed: %s", ctx->r_ip, ctx->r_port,
...
static void cf_socket_get_host(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static ssize_t cf_socket_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  *err = CURLE_OK;
  ...
  if (cf->conn->bits.tcp_fastopen) {
    nwritten =
        sendto(ctx->sock, buf, len, MSG_FASTOPEN,
               &cf->conn->remote_addr->sa_addr, cf->conn->remote_addr->addrlen);
  ...
  CURL_TRC_CF(data, cf, "send(len=%zu) -> %d, err=%d", orig_len, (int)nwritten,
              *err);
  cf->conn->sock[cf->sockindex] = fdsave;
  return nwritten;
}
...
out:
  CURL_TRC_CF(data, cf, "recv(len=%zu) -> %d, err=%d", len, (int)nread, *err);
...
static void conn_set_primary_ip(struct Curl_cfilter *cf,
  ...
  if (!(data->conn->handler->protocol & CURLPROTO_TFTP)) {
    ...
    if (getpeername(ctx->sock, (struct sockaddr *)&ssrem, &plen)) {
      int error = SOCKERRNO;
      failf(data, "getpeername() failed with errno %d: %s", error,
            Curl_strerror(error, buffer, sizeof(buffer)));
      return;
    }
    if (!Curl_addr2string((struct sockaddr *)&ssrem, plen, cf->conn->primary_ip,
                          &port)) {
      failf(data, "ssrem inet_ntop() failed with errno %d: %s", errno,
            Curl_strerror(errno, buffer, sizeof(buffer)));
      return;
    }
  }
...
static CURLcode cf_socket_cntrl(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static CURLcode cf_socket_query(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static CURLcode cf_udp_setup_quic(struct Curl_cfilter *cf,
  ...
  set_local_ip(cf, data);
  CURL_TRC_CF(data, cf,
              "%s socket %" CURL_FORMAT_SOCKET_T
              " connected: [%s:%d] -> [%s:%d]",
              (ctx->transport == TRNSPRT_QUIC) ? "QUIC" : "UDP", ctx->sock,
              ctx->l_ip, ctx->l_port, ctx->r_ip, ctx->r_port);
...
static CURLcode cf_udp_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
                               bool blocking, bool *done) {
  struct cf_socket_ctx *ctx = cf->ctx;
  CURLcode result = CURLE_COULDNT_CONNECT;
  ...
  if (ctx->sock == CURL_SOCKET_BAD) {
    ...
    if (ctx->transport == TRNSPRT_QUIC) {
      ...
      if (result)
        goto out;
      CURL_TRC_CF(data, cf,
                  "cf_udp_connect(), opened socket=%" CURL_FORMAT_SOCKET_T
                  " (%s:%d)",
...
CURLcode Curl_conn_tcp_listen_set(struct Curl_easy *data,
  ...
  ctx->connected_at = Curl_now();
  cf->connected = TRUE;
  CURL_TRC_CF(data, cf, "Curl_conn_tcp_listen_set(%" CURL_FORMAT_SOCKET_T ")",
              ctx->sock);
...
static void set_accepted_remote_ip(struct Curl_cfilter *cf,
  ...
  if (!Curl_addr2string((struct sockaddr *)&ssrem, plen, ctx->r_ip,
                        &ctx->r_port)) {
    failf(data, "ssrem inet_ntop() failed with errno %d: %s", errno,
          Curl_strerror(errno, buffer, sizeof(buffer)));
    return;
...
CURLcode Curl_conn_tcp_accepted_set(struct Curl_easy *data,
  ...
  cf->connected = TRUE;
  CURL_TRC_CF(data, cf,
              "accepted_set(sock=%" CURL_FORMAT_SOCKET_T ", remote=%s port=%d)",
...
static bool cf_is_socket(struct Curl_cfilter *cf) {
  return cf && (cf->cft == &Curl_cft_tcp || cf->cft == &Curl_cft_udp ||
                cf->cft == &Curl_cft_unix || cf->cft == &Curl_cft_tcp_accept);
}
