...
static CURLcode hsts_create(struct hsts *h, const char *hostname,
                            bool subdomains, curl_off_t expires) {
 ...
 */
static CURLcode hsts_push(struct Curl_easy *data, struct curl_index *i,
                          struct stsentry *sts, bool *stop) {
...
static CURLcode hsts_out(struct stsentry *sts, FILE *fp) {
  ...
  if (sts->expires != TIME_T_MAX) {
    ...
    fprintf(fp, "%s%s \"%d%02d%02d %02d:%02d:%02d\"\n",
            sts->includeSubDomains ? "." : "", sts->host, stamp.tm_year + 1900,
...
static CURLcode hsts_add(struct hsts *h, char *line) {
  ...
  if (2 == rc) {
    time_t expires =
        strcmp(date, UNLIMITED) ? Curl_getdate_capped(date) : TIME_T_MAX;
    CURLcode result = CURLE_OK;
    char *p = host;
    ...
    do {
      ...
      if (sc == CURLSTS_OK) {
        ...
        result = hsts_create(h, e.name,
                             ...
                             e.includeSubDomains ? TRUE : FALSE, expires);
        if (result)
          return result;
