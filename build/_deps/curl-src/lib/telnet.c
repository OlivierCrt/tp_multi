...
static CURLcode
telrcv(struct Curl_easy *data,
       const unsigned char *inbuf, /* Data received from socket */
       ssize_t count);             /* Number of bytes received */

#ifndef CURL_DISABLE_VERBOSE_STRINGS
static void printoption(struct Curl_easy *data, const char *direction, int cmd,
                        int option);
#endif

static void negotiate(struct Curl_easy *data);
static void send_negotiation(struct Curl_easy *data, int cmd, int option);
static void set_local_option(struct Curl_easy *data, int option, int newstate);
static void set_remote_option(struct Curl_easy *data, int option, int newstate);

static void printsub(struct Curl_easy *data, int direction,
                     unsigned char *pointer, size_t length);
...
static CURLcode telnet_done(struct Curl_easy *data, CURLcode, bool premature);
static CURLcode send_telnet_data(struct Curl_easy *data, char *buffer,
                                 ssize_t nread);
...
static void set_remote_option(struct Curl_easy *data, int option,
                              int newstate) {
  struct TELNET *tn = data->req.p.telnet;
...
static void printsub(struct Curl_easy *data, int direction, /* '<' or '>' */
...
static void suboption(struct Curl_easy *data) {
  ...
  case CURL_TELOPT_TTYPE:
    ...
    msnprintf((char *)temp, sizeof(temp), "%c%c%c%c%s%c%c", CURL_IAC, CURL_SB,
              CURL_TELOPT_TTYPE, CURL_TELQUAL_IS, tn->subopt_ttype, CURL_IAC,
              CURL_SE);
  ...
  case CURL_TELOPT_XDISPLOC:
    ...
    msnprintf((char *)temp, sizeof(temp), "%c%c%c%c%s%c%c", CURL_IAC, CURL_SB,
              CURL_TELOPT_XDISPLOC, CURL_TELQUAL_IS, tn->subopt_xdisploc,
              CURL_IAC, CURL_SE);
    bytes_written = swrite(conn->sock[FIRSTSOCKET], temp, len);
    if (bytes_written < 0) {
      err = SOCKERRNO;
    ...
    break;
  case CURL_TELOPT_NEW_ENVIRON:
    msnprintf((char *)temp, sizeof(temp), "%c%c%c%c", CURL_IAC, CURL_SB,
              CURL_TELOPT_NEW_ENVIRON, CURL_TELQUAL_IS);
    len = 4;
    ...
    for (v = tn->telnet_vars; v; v = v->next) {
      ...
      if (len + tmplen < (int)sizeof(temp) - 6) {
        ...
        if (!s)
          len += msnprintf((char *)&temp[len], sizeof(temp) - len, "%c%s",
                           CURL_NEW_ENV_VAR, v->data);
        else {
          size_t vlen = s - v->data;
          len += msnprintf((char *)&temp[len], sizeof(temp) - len, "%c%.*s%c%s",
                           CURL_NEW_ENV_VAR, (int)vlen, v->data,
                           CURL_NEW_ENV_VALUE, ++s);
        }
      }
    }
    msnprintf((char *)&temp[len], sizeof(temp) - len, "%c%c", CURL_IAC,
              CURL_SE);
    len += 2;
    bytes_written = swrite(conn->sock[FIRSTSOCKET], temp, len);
    if (bytes_written < 0) {
...
#define startskipping()                                                        \
  if (startwrite >= 0) {                                                       \
    result = Curl_client_write(data, CLIENTWRITE_BODY,                         \
                               (char *)&inbuf[startwrite], in - startwrite);   \
    if (result)                                                                \
      return result;                                                           \
  }                                                                            \
  startwrite = -1
...
static CURLcode telnet_done(struct Curl_easy *data, CURLcode status,
                            bool premature) {
  struct TELNET *tn = data->req.p.telnet;
  (void)status;    /* unused */
...
static CURLcode telnet_do(struct Curl_easy *data, bool *done) {
  ...
  */
  ...
  while (keepon) {
    ...
    DWORD waitret =
        WaitForMultipleObjects(obj_count, objs, FALSE, wait_timeout);
    switch (waitret) {
    ...
    case WAIT_TIMEOUT: {
      for (;;) {
        if (data->set.is_fread_set) {
          ...
          /* read from stdin */
          if (!PeekNamedPipe(stdin_handle, NULL, 0, NULL, &readfile_read,
                             NULL)) {
            keepon = FALSE;
            result = CURLE_READ_ERROR;
            break;
