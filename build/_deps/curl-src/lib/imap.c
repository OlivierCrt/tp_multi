...
static void imap_state(struct Curl_easy *data, imapstate newstate) {
  ...
  static const char *const names[] = {
      "STOP",         "SERVERGREET", "CAPABILITY",   "STARTTLS", "UPGRADETLS",
      "AUTHENTICATE", "LOGIN",       "LIST",         "SELECT",   "FETCH",
      "FETCH_FINAL",  "APPEND",      "APPEND_FINAL", "SEARCH",   "LOGOUT",
      /* LAST */
  };

  if (imapc->state != newstate)
    infof(data, "IMAP %p state change from %s to %s", (void *)imapc,
          names[imapc->state], names[newstate]);
#endif
...
static CURLcode imap_perform_login(struct Curl_easy *data,
  ...
  result =
      imap_sendf(data, "LOGIN %s %s", user ? user : "", passwd ? passwd : "");
...
static CURLcode imap_continue_authenticate(struct Curl_easy *data,
  ...
  return Curl_pp_sendf(data, &imapc->pp, "%s",
...
static CURLcode imap_perform_authentication(struct Curl_easy *data,
  ...
  if (imapc->preauth || !Curl_sasl_can_authenticate(&imapc->sasl, data)) {
    imap_state(data, IMAP_STOP);
    return result;
  }
...
static CURLcode imap_perform_fetch(struct Curl_easy *data) {
  ...
  if (imap->uid) {
    ...
    if (imap->partial)
      result = imap_sendf(data, "UID FETCH %s BODY[%s]<%s>", imap->uid,
                          imap->section ? imap->section : "", imap->partial);
    else
      result = imap_sendf(data, "UID FETCH %s BODY[%s]", imap->uid,
                          imap->section ? imap->section : "");
  } else if (imap->mindex) {
    /* Send the FETCH command */
    if (imap->partial)
      result = imap_sendf(data, "FETCH %s BODY[%s]<%s>", imap->mindex,
                          imap->section ? imap->section : "", imap->partial);
    else
      result = imap_sendf(data, "FETCH %s BODY[%s]", imap->mindex,
                          imap->section ? imap->section : "");
  } else {
...
static CURLcode imap_perform_append(struct Curl_easy *data) {
  ...
  if (data->set.mimepost.kind != MIMEKIND_NONE) {
    ...
    result = Curl_mime_prepare_headers(data, &data->set.mimepost, NULL, NULL,
                                       MIMESTRATEGY_MAIL);
  ...
  result = imap_sendf(data, "APPEND %s (\\Seen) {%" CURL_FORMAT_CURL_OFF_T "}",
...
static CURLcode imap_state_servergreet_resp(struct Curl_easy *data,
                                            int imapcode, imapstate instate) {
  struct connectdata *conn = data->conn;
...
static CURLcode imap_state_capability_resp(struct Curl_easy *data, int imapcode,
                                           imapstate instate) {
  CURLcode result = CURLE_OK;
  struct connectdata *conn = data->conn;
  struct imap_conn *imapc = &conn->proto.imapc;
...
static CURLcode imap_state_starttls_resp(struct Curl_easy *data, int imapcode,
                                         imapstate instate) {
  CURLcode result = CURLE_OK;
  struct connectdata *conn = data->conn;
...
/* For LOGIN responses */
static CURLcode imap_state_login_resp(struct Curl_easy *data, int imapcode,
                                      imapstate instate) {
  CURLcode result = CURLE_OK;
  (void)instate; /* no use for this yet */
...
static CURLcode imap_state_listsearch_resp(struct Curl_easy *data, int imapcode,
                                           imapstate instate) {
  CURLcode result = CURLE_OK;
...
static CURLcode imap_state_fetch_resp(struct Curl_easy *data,
  ...
  if (parsed) {
    ...
    if (pp->cache) {
      ...
      infof(data,
            "Written %zu bytes, %" CURL_FORMAT_CURL_OFF_TU
...
static char *imap_atom(const char *str, bool escape_only) {
  ...
  while (*str) {
    if ((*str == '\\' || *str == '"') && Curl_dyn_addn(&line, "\\", 1))
      return NULL;
...
static CURLcode imap_parse_url_options(struct connectdata *conn) {
  ...
  while (!result && ptr && *ptr) {
    ...
    if (strncasecompare(key, "AUTH=+LOGIN", 11)) {
      ...
      prefer_login = false;
      result =
          Curl_sasl_parse_url_auth_option(&imapc->sasl, value, ptr - value);
    } else {
      prefer_login = false;
...
static CURLcode imap_parse_url_path(struct Curl_easy *data) {
  ...
  if (ptr != begin) {
    ...
    result =
        Curl_urldecode(begin, end - begin, &imap->mailbox, NULL, REJECT_CTRL);
    if (result)
      return result;
  } else
    imap->mailbox = NULL;
  ...
  while (*ptr == ';') {
    ...
    result = Curl_urldecode(begin, ptr - begin, &name, NULL, REJECT_CTRL);
    if (result)
      return result;
    ...
    result = Curl_urldecode(begin, ptr - begin, &value, &valuelen, REJECT_CTRL);
    if (result) {
      free(name);
      return result;
    }
