...
static int http_getsock_do(struct Curl_easy *data, struct connectdata *conn,
                           curl_socket_t *socks);
...
static CURLcode ws_setup_conn(struct Curl_easy *data, struct connectdata *conn);
#endif
...
static CURLcode http_perhapsrewind(struct Curl_easy *data,
  ...
  if ((expectsend == -1) || (expectsend > bytessent)) {
    ...
    if ((data->state.authproxy.picked == CURLAUTH_NTLM) ||
      ...
      infof(data,
            "NTLM send, close instead of sending %" CURL_FORMAT_CURL_OFF_T
            " bytes",
    ...
    if ((data->state.authproxy.picked == CURLAUTH_NEGOTIATE) ||
      ...
      infof(data,
            "NEGOTIATE send, close instead of sending %" CURL_FORMAT_CURL_OFF_T
            " bytes",
            (curl_off_t)(expectsend - bytessent));
    }
#endif
...
CURLcode Curl_http_auth_act(struct Curl_easy *data) {
  ...
  if (conn->bits.proxy_user_passwd &&
      ((data->req.httpcode == 407) ||
       (conn->bits.authneg && data->req.httpcode < 300))) {
    pickproxy =
        pickoneauth(&data->state.authproxy, authmask & ~CURLAUTH_BEARER);
    if (!pickproxy)
      data->state.authproblem = TRUE;
  }
#endif
  ...
  if (pickhost || pickproxy) {
    ...
    if (!data->req.newurl)
      return CURLE_OUT_OF_MEMORY;
  } else if ((data->req.httpcode < 300) && (!data->state.authhost.done) &&
             conn->bits.authneg) {
...
static CURLcode output_auth_headers(struct Curl_easy *data,
  ...
  if (authstatus->picked == CURLAUTH_AWS_SIGV4) {
    ...
    auth = "Digest";
    result = Curl_output_digest(data, proxy, (const unsigned char *)request,
  ...
  if (auth) {
#ifndef CURL_DISABLE_PROXY
    infof(data, "%s auth using %s with user '%s'", proxy ? "Proxy" : "Server",
          auth,
          proxy ? (data->state.aptr.proxyuser ? data->state.aptr.proxyuser : "")
                : (data->state.aptr.user ? data->state.aptr.user : ""));
#else
    infof(data, "Server auth using %s with user '%s'", auth,
          data->state.aptr.user ? data->state.aptr.user : "");
#endif
...
#else
/* when disabled */
CURLcode Curl_http_output_auth(struct Curl_easy *data, struct connectdata *conn,
                               const char *request, Curl_HttpReq httpreq,
                               const char *path, bool proxytunnel) {
  (void)data;
  (void)conn;
  (void)request;
  (void)httpreq;
                              ...
                              const char *auth) /* the first non-space */
{
  ...
  curlnegotiate *negstate =
      proxy ? &conn->proxy_negotiate_state : &conn->http_negotiate_state;
#endif
 ...
 */
static size_t readmoredata(char *buffer, size_t size, size_t nitems,
                           void *userp) {
 ...
 * Returns CURLcode
 */
CURLcode Curl_buffer_send(struct dynbuf *in, struct Curl_easy *data,
                          struct HTTP *http,
                          /* add the number of sent bytes to this
                             counter */
                          curl_off_t *bytes_written,
                          /* how much of the buffer contains body data */
                          curl_off_t included_body_bytes, int sockindex) {
  ssize_t amount;
  CURLcode result;
  char *ptr;
  size_t size;
...
CURLcode Curl_http_done(struct Curl_easy *data, CURLcode status,
                        bool premature) {
  struct connectdata *conn = data->conn;
  struct HTTP *http = data->req.p.http;
...
static CURLcode expect100(struct Curl_easy *data, struct connectdata *conn,
                          struct dynbuf *req) {
  CURLcode result = CURLE_OK;
  ...
  if (!data->state.disableexpect && Curl_use_http_1_1plus(data, conn) &&
    ...
    if (ptr) {
      data->state.expect100header = Curl_compareheader(
          ptr, STRCONST("Expect:"), STRCONST("100-continue"));
    } else {
...
static bool hd_name_eq(const char *n1, size_t n1len, const char *n2,
                       size_t n2len) {
  if (n1len == n2len) {
...
CURLcode Curl_dynhds_add_custom(struct Curl_easy *data, bool is_connect,
  ...
  if (is_connect)
    ...
    proxy = conn->bits.httpproxy && !conn->bits.tunnel_proxy ? HEADER_PROXY
                                                             : HEADER_SERVER;
...
CURLcode Curl_add_timecondition(struct Curl_easy *data,
  ...
  msnprintf(datestr, sizeof(datestr),
            ...
            Curl_wkday[tm->tm_wday ? tm->tm_wday - 1 : 6], tm->tm_mday,
            Curl_month[tm->tm_mon], tm->tm_year + 1900, tm->tm_hour, tm->tm_min,
            tm->tm_sec);
...
CURLcode Curl_http_target(struct Curl_easy *data, struct connectdata *conn,
  ...
  if (conn->bits.httpproxy && !conn->bits.tunnel_proxy) {
    ...
    /* target or url */
    result = Curl_dyn_add(
        r, data->set.str[STRING_TARGET] ? data->set.str[STRING_TARGET] : url);
    free(url);
    if (result)
      return (result);
...
CURLcode Curl_http_body(struct Curl_easy *data, struct connectdata *conn,
  ...
  if (data->state.mimepost) {
    ...
    result = Curl_mime_prepare_headers(data, data->state.mimepost, cthdr, NULL,
                                       MIMESTRATEGY_FORM);
  ...
  if (ptr) {
    ...
    data->req.upload_chunky = Curl_compareheader(
        ptr, STRCONST("Transfer-Encoding:"), STRCONST("chunked"));
  } else {
...
#define Curl_buffer_send(a, b, c, d, e, f) CURLE_OK
  ...
  case HTTPREQ_PUT: /* Let's PUT the data to the server! */
    ...
    if ((http->postsize != -1) && !data->req.upload_chunky &&
      ...
      result = Curl_dyn_addf(
          r, "Content-Length: %" CURL_FORMAT_CURL_OFF_T "\r\n", http->postsize);
      if (result)
        return result;
    }
  ...
  case HTTPREQ_POST:
    ...
    if (data->set.postfields) {
      if (!data->state.expect100header &&
        ...
        if (!data->req.upload_chunky) {
          ...
          result =
              Curl_dyn_addn(r, data->set.postfields, (size_t)http->postsize);
    ...
    /* issue the request */
    result =
        Curl_buffer_send(r, data, data->req.p.http, &data->info.request_size,
...
CURLcode Curl_http_cookies(struct Curl_easy *data, struct connectdata *conn,
                           struct dynbuf *r) {
  CURLcode result = CURLE_OK;
    ...
    if (co) {
      ...
      while (co) {
        if (co->value) {
          ...
          result = Curl_dyn_addf(r, "%s%s=%s", count ? "; " : "", co->name,
                                 co->value);
          if (result)
            break;
          clen += add + (count ? 2 : 0);
  ...
  if (data->state.use_range) {
    ...
    if (((httpreq == HTTPREQ_GET) || (httpreq == HTTPREQ_HEAD)) &&
      ...
      if (data->set.set_resume_from < 0) {
        ...
        data->state.aptr.rangeline = aprintf(
...
CURLcode Curl_http_resume(struct Curl_easy *data, struct connectdata *conn,
      ...
      if (seekerr != CURL_SEEKFUNC_OK) {
        ...
        do {
          ...
          if ((actuallyread == 0) || (actuallyread > readthisamountnow)) {
            ...
            failf(data,
                  "Could only read %" CURL_FORMAT_CURL_OFF_T
...
CURLcode Curl_http_firstwrite(struct Curl_easy *data, struct connectdata *conn,
                              bool *done) {
  ...
  if (data->state.resume_from && !k->content_range &&
      (data->state.httpreq == HTTPREQ_GET) && !k->ignorebody) {
...
CURLcode Curl_http(struct Curl_easy *data, bool *done) {
  ...
  *done = TRUE;
...
#ifndef CURL_DISABLE_PROXY
    if (!Curl_conn_is_http2(data, conn, FIRSTSOCKET) && conn->bits.proxy &&
  ...
  p_accept =
      Curl_checkheaders(data, STRCONST("Accept")) ? NULL : "Accept: */*\r\n";
  ...
  result = Curl_dyn_addf(
      ...
      httpstring, (data->state.aptr.host ? data->state.aptr.host : ""),
      data->state.aptr.proxyuserpwd ? data->state.aptr.proxyuserpwd : "",
      ...
      (data->set.str[STRING_USERAGENT] && *data->set.str[STRING_USERAGENT] &&
       data->state.aptr.uagent)
          ? data->state.aptr.uagent
          : "",
      p_accept ? p_accept : "", data->state.aptr.te ? data->state.aptr.te : "",
      (data->set.str[STRING_ENCODING] && *data->set.str[STRING_ENCODING] &&
       data->state.aptr.accept_encoding)
          ? data->state.aptr.accept_encoding
          : "",
      (data->state.referer && data->state.aptr.ref) ? data->state.aptr.ref
                                                    : "" /* Referer: <data> */,
#ifndef CURL_DISABLE_PROXY
  ...
  if ((http->postsize > -1) && (http->postsize <= data->req.writebytecount) &&
      (http->sending != HTTPSEND_REQUEST))
 ...
 */
static statusline checkhttpprefix(struct Curl_easy *data, const char *s,
                                  size_t len) {
...
static statusline checkrtspprefix(struct Curl_easy *data, const char *s,
                                  size_t len) {
...
CURLcode Curl_http_header(struct Curl_easy *data, struct connectdata *conn,
  ...
  if (!k->http_bodyless && !data->set.ignorecl &&
      checkprefix("Content-Length:", headp)) {
    curl_off_t contentlength;
    CURLofft offt = curlx_strtoofft(headp + strlen("Content-Length:"), NULL, 10,
                                    &contentlength);
  ...
  /* check for Content-Type: header lines to get the MIME-type */
...
#ifndef CURL_DISABLE_PROXY
  else if ((conn->httpversion == 10) && conn->bits.httpproxy &&
           Curl_compareheader(headp, STRCONST("Proxy-Connection:"),
                              STRCONST("keep-alive"))) {
    /*
    ...
    infof(data, "HTTP/1.1 proxy connection set close");
           ...
           Curl_compareheader(headp, STRCONST("Connection:"),
                              STRCONST("keep-alive"))) {
    /*
  ...
  } else if (Curl_compareheader(headp, STRCONST("Connection:"),
                                STRCONST("close"))) {
    /*
    ...
    result = Curl_build_unencoding_stack(
        data, headp + strlen("Transfer-Encoding:"), TRUE);
    if (result)
      return result;
     ...
     */
    result = Curl_build_unencoding_stack(
        data, headp + strlen("Content-Encoding:"), FALSE);
    if (result)
      return result;
    ...
    (void)curlx_strtoofft(headp + strlen("Retry-After:"), NULL, 10,
                          &retry_after);
    if (!retry_after) {
    ...
    */
    ...
    /* If there is a custom-set Host: name, use it here, or else use real peer
       host name. */
    const char *host = data->state.aptr.cookiehost ? data->state.aptr.cookiehost
                                                   : conn->host.name;
    const bool secure_context =
        conn->handler->protocol & (CURLPROTO_HTTPS | CURLPROTO_WSS) ||
                strcasecompare("localhost", host) ||
    ...
    Curl_cookie_add(data, data->cookies, TRUE, FALSE,
                    headp + strlen("Set-Cookie:"), host, data->state.up.path,
                    secure_context);
    Curl_share_unlock(data, CURL_LOCK_DATA_COOKIE);
  }
#endif
    ...
    if (authp->picked == CURLAUTH_NEGOTIATE) {
      ...
      negdata->noauthpersist =
          checkprefix("false", persistentauth) ? TRUE : FALSE;
      ...
      free(persistentauth);
           ...
           checkprefix("Location:", headp) && !data->req.location) {
  ...
  /* If enabled, the header is incoming and this is over HTTPS */
    ...
    enum alpnid id = (conn->httpversion == 30)   ? ALPN_h3
                     : (conn->httpversion == 20) ? ALPN_h2
                                                 : ALPN_h1;
    result = Curl_altsvc_parse(data, data->asi, headp + strlen("Alt-Svc:"), id,
                               conn->host.name,
...
CURLcode Curl_bump_headersize(struct Curl_easy *data, size_t delta,
                              bool connect_only) {
  size_t bad = 0;
  ...
  do {
    ...
    if (!end_ptr) {
      ...
      if (!k->headerline) {
        ...
        statusline st =
            checkprotoprefix(data, conn, Curl_dyn_ptr(&data->state.headerb),
    ...
    *nread -= (ssize_t)rest_length;
    ...
    if ((0x0a == *headp) || (0x0d == *headp)) {
      ...
      if (100 <= k->httpcode && 199 >= k->httpcode) {
        ...
        case 101:
          ...
          if (k->upgr101 == UPGR101_H2) {
            ...
            result =
                Curl_http2_upgrade(data, conn, FIRSTSOCKET, k->str, *nread);
            if (result)
              return result;
            *nread = 0;
          }
...
#if defined(USE_NTLM)
      if (conn->bits.close && (((data->req.httpcode == 401) &&
       ...
       */
      writetype =
          CLIENTWRITE_HEADER | ((k->httpcode / 100 == 1) ? CLIENTWRITE_1XX : 0);

      headerlen = Curl_dyn_len(&data->state.headerb);
      result = Curl_client_write(data, writetype,
                                 Curl_dyn_ptr(&data->state.headerb), headerlen);
      if (result)
        return result;
      ...
      data->req.deductheadercount = (100 <= k->httpcode && 199 >= k->httpcode)
    ...
    if (!k->headerline++) {
      ...
      if (conn->handler->protocol & PROTO_FAMILY_HTTP) {
        ...
        if (fine_statusline) {
          ...
          */
          statusline check =
              checkhttpprefix(data, Curl_dyn_ptr(&data->state.headerb),
                              Curl_dyn_len(&data->state.headerb));
    ...
    result =
        Curl_bump_headersize(data, Curl_dyn_len(&data->state.headerb), FALSE);
    if (result)
      return result;
...
CURLcode Curl_http_req_make(struct httpreq **preq, const char *method,
                            size_t m_len, const char *scheme, size_t s_len,
...
CURLcode Curl_http_req_make2(struct httpreq **preq, const char *method,
                             size_t m_len, CURLU *url,
...
CURLcode Curl_http_req_to_h2(struct dynhds *h2_headers, struct httpreq *req,
  ...
  if (!result && scheme) {
    result = Curl_dynhds_add(h2_headers, STRCONST(HTTP_PSEUDO_SCHEME), scheme,
                             strlen(scheme));
  }
  if (!result && authority) {
  ...
  if (!result && req->path) {
    result = Curl_dynhds_add(h2_headers, STRCONST(HTTP_PSEUDO_PATH), req->path,
                             strlen(req->path));
  }
  for (i = 0; !result && i < Curl_dynhds_count(&req->headers); ++i) {
    e = Curl_dynhds_getn(&req->headers, i);
    if (!h2_non_field(e->name, e->namelen)) {
      result = Curl_dynhds_add(h2_headers, e->name, e->namelen, e->value,
                               e->valuelen);
    }
  }
...
CURLcode Curl_http_resp_make(struct http_resp **presp, int status,
