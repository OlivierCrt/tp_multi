...
static CURLcode rtp_client_write(struct Curl_easy *data, char *ptr, size_t len);
static CURLcode rtsp_parse_transport(struct Curl_easy *data, char *transport);
...
static CURLcode rtsp_done(struct Curl_easy *data, CURLcode status,
                          bool premature) {
...
static CURLcode rtsp_do(struct Curl_easy *data, bool *done) {
  ...
  *done = TRUE;
  ...
  result = Curl_dyn_addf(
      ...
      "%s" /* userpwd */
      ,
      p_transport ? p_transport : "", p_accept ? p_accept : "",
      p_accept_encoding ? p_accept_encoding : "", p_range ? p_range : "",
      p_referrer ? p_referrer : "", p_uagent ? p_uagent : "",
      p_proxyuserpwd ? p_proxyuserpwd : "", p_userpwd ? p_userpwd : "");
    ...
    if (data->state.upload) {
      ...
      postsize =
          (data->state.infilesize != -1)
              ...
              : (data->set.postfields ? (curl_off_t)strlen(data->set.postfields)
                                      : 0);
    ...
    if (putsize > 0 || postsize > 0) {
      ...
      if (rtspreq == RTSPREQ_SET_PARAMETER ||
        ...
        if (!Curl_checkheaders(data, STRCONST("Content-Type"))) {
          result = Curl_dyn_addn(&req_buffer, STRCONST("Content-Type: "
                                                       "text/parameters\r\n"));
          if (result)
            return result;
        }
  ...
  if (postsize > 0) {
    result = Curl_dyn_addn(&req_buffer, data->set.postfields, (size_t)postsize);
    if (result)
      return result;
  }
  ...
  while (rtp_dataleft > 0) {
    if (rtp[0] == '$') {
      if (rtp_dataleft > 4) {
        ...
        if (skip_size > 0) {
          DEBUGF(infof(data,
                       "Skip the malformed interleaved data %lu "
                       "bytes",
                       skip_size));
        }
        skip_size = 0;
...
static CURLcode rtp_client_write(struct Curl_easy *data, char *ptr,
                                 size_t len) {
  size_t wrote;
  ...
  if (checkprefix("CSeq:", header)) {
    ...
    if (data->set.str[STRING_RTSP_SESSION_ID]) {
      ...
      if (strlen(data->set.str[STRING_RTSP_SESSION_ID]) != idlen ||
        ...
        failf(data, "Got RTSP Session ID Line [%s], but wanted ID [%s]", start,
      ...
      if (p != endp && chan1 >= 0 && chan1 <= 255) {
        ...
        if (*endp == '-') {
          ...
          if (p == endp || chan2 < 0 || chan2 > 255) {
            infof(data,
                  "Unable to read the interleaved parameter from "
                  "Transport header: [%s]",
                  transport);
            chan2 = chan1;
          }
