...
struct HMAC_context *Curl_HMAC_init(const struct HMAC_params *hashparams,
  ...
  ctxt->hmac_hashctxt2 =
      (void *)((char *)ctxt->hmac_hashctxt1 + hashparams->hmac_ctxtsize);
...
int Curl_HMAC_update(struct HMAC_context *ctxt, const unsigned char *data,
                     unsigned int len) {
...
int Curl_HMAC_final(struct HMAC_context *ctxt, unsigned char *result) {
  ...
  if (!result)
    result =
        (unsigned char *)ctxt->hmac_hashctxt2 + ctxt->hmac_hash->hmac_ctxtsize;

  (*hashparams->hmac_hfinal)(result, ctxt->hmac_hashctxt1);
  (*hashparams->hmac_hupdate)(ctxt->hmac_hashctxt2, result,
                              hashparams->hmac_resultlen);
