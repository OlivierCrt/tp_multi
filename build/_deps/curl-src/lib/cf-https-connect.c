...
static void cf_hc_baller_reset(struct cf_hc_baller *b, struct Curl_easy *data) {
  if (b->cf) {
...
static bool cf_hc_baller_has_started(struct cf_hc_baller *b) { return !!b->cf; }

static int cf_hc_baller_reply_ms(struct cf_hc_baller *b,
                                 struct Curl_easy *data) {
  if (b->reply_ms < 0)
    b->cf->cft->query(b->cf, data, CF_QUERY_CONNECT_REPLY_MS, &b->reply_ms,
                      NULL);
  return b->reply_ms;
}
...
static void cf_hc_baller_init(struct cf_hc_baller *b, struct Curl_cfilter *cf,
                              struct Curl_easy *data, const char *name,
                              int transport) {
  ...
  cf->next = NULL;
  b->started = Curl_now();
  b->result = Curl_cf_setup_insert_after(cf, data, ctx->remotehost, transport,
                                         CURL_CF_SSL_ENABLE);
  b->cf = cf->next;
  cf->next = save;
}
...
static CURLcode baller_connected(struct Curl_cfilter *cf,
  ...
  cf->connected = TRUE;
  Curl_conn_cf_cntrl(cf->next, data, TRUE, CF_CTRL_CONN_INFO_UPDATE, 0, NULL);
  return result;
}

static bool time_to_start_h21(struct Curl_cfilter *cf, struct Curl_easy *data,
                              struct curltime now) {
  struct cf_hc_ctx *ctx = cf->ctx;
  ...
  if (elapsed_ms >= ctx->soft_eyeballs_timeout_ms) {
    if (cf_hc_baller_reply_ms(&ctx->h3_baller, data) < 0) {
      CURL_TRC_CF(data, cf,
                  "soft timeout of %dms reached, h3 has not "
...
static CURLcode cf_hc_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  *done = FALSE;
  ...
  case CF_HC_INIT:
    ...
    if (ctx->h3_baller.enabled) {
      ...
      cf_hc_baller_init(&ctx->h21_baller, cf, data, "h21", cf->conn->transport);
  ...
  case CF_HC_CONNECT:
    ...
    if ((!ctx->h3_baller.enabled || ctx->h3_baller.result) &&
      ...
      result = ctx->result = ctx->h3_baller.enabled ? ctx->h3_baller.result
                                                    : ctx->h21_baller.result;
      ctx->state = CF_HC_FAILURE;
      goto out;
    }
    result = CURLE_OK;
...
static CURLcode cf_hc_query(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static CURLcode cf_hc_create(struct Curl_cfilter **pcf, struct Curl_easy *data,
                             const struct Curl_dns_entry *remotehost,
...
CURLcode Curl_cf_https_setup(struct Curl_easy *data, struct connectdata *conn,
                             int sockindex,
                             const struct Curl_dns_entry *remotehost) {
  ...
  result =
      cf_http_connect_add(data, conn, sockindex, remotehost, try_h3, try_h21);
out:
  return result;
}
