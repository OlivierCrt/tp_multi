 ...
 * Does nothing here.
 */
int Curl_resolver_global_init(void) { return CURLE_OK; }
 ...
 * structure).
 */
void Curl_resolver_cleanup(void *resolver) { free(resolver); }
...
static bool init_resolve_thread(struct Curl_easy *data, const char *hostname,
...
static int init_thread_sync_data(struct thread_data *td, const char *hostname,
                                 int port, const struct addrinfo *hints) {
...
static CURLcode thread_wait_resolv(struct Curl_easy *data,
                                   struct Curl_dns_entry **entry, bool report) {
...
struct Curl_addrinfo *Curl_resolver_getaddrinfo(struct Curl_easy *data,
  ...
  *waitp = 0; /* default to synchronous response */
  ...
  hints.ai_family = pf;
  hints.ai_socktype =
      (data->conn->transport == TRNSPRT_TCP) ? SOCK_STREAM : SOCK_DGRAM;
