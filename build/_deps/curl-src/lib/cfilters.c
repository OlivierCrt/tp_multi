...
int Curl_cf_def_get_select_socks(struct Curl_cfilter *cf,
                                 struct Curl_easy *data, curl_socket_t *socks) {
  return cf->next ? cf->next->cft->get_select_socks(cf->next, data, socks) : 0;
}
...
ssize_t Curl_cf_def_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  return cf->next ? cf->next->cft->do_send(cf->next, data, buf, len, err)
                  : CURLE_RECV_ERROR;
}

ssize_t Curl_cf_def_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
                         char *buf, size_t len, CURLcode *err) {
  return cf->next ? cf->next->cft->do_recv(cf->next, data, buf, len, err)
                  : CURLE_SEND_ERROR;
}

bool Curl_cf_def_conn_is_alive(struct Curl_cfilter *cf, struct Curl_easy *data,
                               bool *input_pending) {
...
CURLcode Curl_cf_def_conn_keep_alive(struct Curl_cfilter *cf,
  ...
  return cf->next ? cf->next->cft->keep_alive(cf->next, data) : CURLE_OK;
}

CURLcode Curl_cf_def_query(struct Curl_cfilter *cf, struct Curl_easy *data,
...
void Curl_conn_cf_discard_all(struct Curl_easy *data, struct connectdata *conn,
                              int index) {
...
ssize_t Curl_conn_recv(struct Curl_easy *data, int num, char *buf, size_t len,
                       CURLcode *code) {
  struct Curl_cfilter *cf;
...
ssize_t Curl_conn_send(struct Curl_easy *data, int num, const void *mem,
...
CURLcode Curl_cf_create(struct Curl_cfilter **pcf,
                        const struct Curl_cftype *cft, void *ctx) {
  struct Curl_cfilter *cf;
...
void Curl_conn_cf_add(struct Curl_easy *data, struct connectdata *conn,
...
bool Curl_conn_cf_discard_sub(struct Curl_cfilter *cf,
                              ...
                              struct Curl_easy *data, bool destroy_always) {
...
CURLcode Curl_conn_cf_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
...
CURLcode Curl_conn_connect(struct Curl_easy *data, int sockindex, bool blocking,
                           bool *done) {
  struct Curl_cfilter *cf;
...
bool Curl_conn_is_multiplex(struct connectdata *conn, int sockindex) {
  ...
  for (; cf; cf = cf->next) {
    if (cf->cft->flags & CF_TYPE_MULTIPLEX)
      return TRUE;
    if (cf->cft->flags & CF_TYPE_IP_CONNECT || cf->cft->flags & CF_TYPE_SSL)
      return FALSE;
  }
  return FALSE;
}
...
CURLcode Curl_cf_def_cntrl(struct Curl_cfilter *cf, struct Curl_easy *data,
...
CURLcode Curl_conn_cf_cntrl(struct Curl_cfilter *cf, struct Curl_easy *data,
                            bool ignore_result, int event, int arg1,
                            void *arg2) {
  CURLcode result = CURLE_OK;
...
static CURLcode cf_cntrl_all(struct connectdata *conn, struct Curl_easy *data,
                             bool ignore_result, int event, int arg1,
                             void *arg2) {
  CURLcode result = CURLE_OK;
  size_t i;
  ...
  for (i = 0; i < ARRAYSIZE(conn->cfilter); ++i) {
    result = Curl_conn_cf_cntrl(conn->cfilter[i], data, ignore_result, event,
                                arg1, arg2);
    if (!ignore_result && result)
      break;
  }
...
CURLcode Curl_conn_ev_data_idle(struct Curl_easy *data) {
  return cf_cntrl_all(data->conn, data, FALSE, CF_CTRL_DATA_IDLE, 0, NULL);
}
...
CURLcode Curl_conn_ev_data_pause(struct Curl_easy *data, bool do_pause) {
  return cf_cntrl_all(data->conn, data, FALSE, CF_CTRL_DATA_PAUSE, do_pause,
                      NULL);
}
...
CURLcode Curl_conn_keep_alive(struct Curl_easy *data, struct connectdata *conn,
                              int sockindex) {
...
size_t Curl_conn_get_max_concurrent(struct Curl_easy *data,
  ...
  struct Curl_cfilter *cf = conn->cfilter[sockindex];
  result = cf ? cf->cft->query(cf, data, CF_QUERY_MAX_CONCURRENT, &n, NULL)
              : CURLE_UNKNOWN_OPTION;
