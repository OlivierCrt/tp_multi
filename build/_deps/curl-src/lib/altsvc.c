...
static struct altsvc *altsvc_createid(const char *srchost, const char *dsthost,
...
static struct altsvc *altsvc_create(char *srchost, char *dsthost, char *srcalpn,
                                    char *dstalpn, unsigned int srcport,
                                    unsigned int dstport) {
  enum alpnid dstalpnid = alpn2alpnid(dstalpn);
  enum alpnid srcalpnid = alpn2alpnid(srcalpn);
  if (!srcalpnid || !dstalpnid)
    return NULL;
  return altsvc_createid(srchost, dsthost, srcalpnid, dstalpnid, srcport,
                         dstport);
}
...
static CURLcode altsvc_add(struct altsvcinfo *asi, char *line) {
  ...
  rc = sscanf(line,
              ...
              srcalpn, srchost, &srcport, dstalpn, dsthost, &dstport, date,
              &persist, &prio);
  if (9 == rc) {
    struct altsvc *as;
...
static CURLcode altsvc_out(struct altsvc *as, FILE *fp) {
  ...
  fprintf(fp,
          ...
          "%02d:%02d:%02d\" "
          "%u %d\n",
          Curl_alpnid2str(as->src.alpnid), src6_pre, as->src.host, src6_post,
          as->src.port,

          Curl_alpnid2str(as->dst.alpnid), dst6_pre, as->dst.host, dst6_post,
          as->dst.port,

          stamp.tm_year + 1900, stamp.tm_mon + 1, stamp.tm_mday, stamp.tm_hour,
          stamp.tm_min, stamp.tm_sec, as->persist, as->prio);
  return CURLE_OK;
}
 ...
 */
CURLcode Curl_altsvc_save(struct Curl_easy *data, struct altsvcinfo *altsvc,
                          const char *file) {
  struct Curl_llist_element *e;
  struct Curl_llist_element *n;
  CURLcode result = CURLE_OK;
 ...
 */
CURLcode Curl_altsvc_parse(struct Curl_easy *data, struct altsvcinfo *asi,
                           const char *value, enum alpnid srcalpnid,
                           const char *srchost, unsigned short srcport) {
  const char *p = value;
  size_t len;
    ...
    if (*p == '=') {
      ...
      if (*p == '\"') {
        ...
        if (dstalpnid && valid) {
          ...
          as = altsvc_createid(srchost, dsthost, srcalpnid, dstalpnid, srcport,
                               dstport);
          if (as) {
 ...
 */
bool Curl_altsvc_lookup(struct altsvcinfo *asi, enum alpnid srcalpnid,
                        const char *srchost, int srcport,
                        ...
                        const int versions) /* one or more bits */
{
  ...
  for (e = asi->list.head; e; e = n) {
    ...
    if ((as->src.alpnid == srcalpnid) && hostcompare(srchost, as->src.host) &&
