...
static void zstd_version(char *buf, size_t bufsz) {
  ...
  unsigned int minor =
      (unsigned int)((zstd_version - (major * 100 * 100)) / 100);
  unsigned int patch =
      (unsigned int)(zstd_version - (major * 100 * 100) - (minor * 100));
  (void)msnprintf(buf, bufsz, "%u.%u.%u", major, minor, patch);
}
#endif
...
#endif
#ifdef USE_ARES
  msnprintf(cares_version, sizeof(cares_version), "c-ares/%s",
            ares_version(NULL));
  src[i++] = cares_version;
#endif
#ifdef USE_LIBIDN2
  msnprintf(idn_version, sizeof(idn_version), "libidn2/%s",
...
static const char
    *feature_names[sizeof(features_table) / sizeof(features_table[0])] = {NULL};
