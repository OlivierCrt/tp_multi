...
void Curl_disconnect(struct Curl_easy *data, struct connectdata *conn,
  ...
  DEBUGF(infof(data,
               "Curl_disconnect(conn #%" CURL_FORMAT_CURL_OFF_T ", dead=%d)",
...
static bool conn_maxage(struct Curl_easy *data, struct connectdata *conn,
                        struct curltime now) {
 ...
 */
static bool ConnectionExists(struct Curl_easy *data, struct connectdata *needle,
                             struct connectdata **usethis, bool *force_reuse,
                             bool *waitpipe) {
  ...
  *waitpipe = FALSE;
  ...
  if (bundle) {
    ...
    infof(
        data, "Found bundle for host: %p [%s]", (void *)bundle,
    ...
    while (curr) {
      ...
      if ((needle->handler->flags & PROTOPT_SSL) !=
        ...
        if (get_protocol_family(check->handler) != needle->handler->protocol ||
      ...
      if ((needle->handler->flags & PROTOPT_SSL)
        ...
        if ((strcasecompare(needle->handler->scheme, check->handler->scheme) ||
            ...
            (!needle->bits.conn_to_host ||
             strcasecompare(needle->conn_to_host.name,
...
void Curl_verboseconnect(struct Curl_easy *data, struct connectdata *conn) {
  if (data->set.verbose)
    infof(data, "Connected to %s (%s) port %u", CURL_CONN_HOST_DISPNAME(conn),
...
static struct connectdata *allocate_conn(struct Curl_easy *data) {
  ...
  conn->bits.proxy =
      (data->set.str[STRING_PROXY] && *data->set.str[STRING_PROXY]) ? TRUE
                                                                    : FALSE;
  conn->bits.httpproxy =
      (conn->bits.proxy && (conn->http_proxy.proxytype == CURLPROXY_HTTP ||
                            conn->http_proxy.proxytype == CURLPROXY_HTTP_1_0 ||
                            IS_HTTPS_PROXY(conn->http_proxy.proxytype)))
          ? TRUE
          : FALSE;
  conn->bits.socksproxy =
      (conn->bits.proxy && !conn->bits.httpproxy) ? TRUE : FALSE;
...
static CURLcode findprotocol(struct Curl_easy *data, struct connectdata *conn,
                             const char *protostr) {
  const struct Curl_handler *p =
      Curl_builtin_scheme(protostr, CURL_ZERO_TERMINATED);
...
static CURLcode parseurlandfillconn(struct Curl_easy *data,
  ...
  if (!use_set_uh) {
    char *newurl;
    uc = curl_url_set(
        uh, CURLUPART_URL, data->state.url,
  ...
  if (uc) {
    ...
    conn->port = conn->remote_port =
        (data->set.use_port && data->state.allow_port) ? data->set.use_port
                                                       : curlx_ultous(port);
  }
 ...
 */
static CURLcode parse_proxy(struct Curl_easy *data, struct connectdata *conn,
                            char *proxy, curl_proxytype proxytype) {
  char *portptr = NULL;
  int port = -1;
  char *proxyuser = NULL;
  ...
  sockstype = proxytype == CURLPROXY_SOCKS5_HOSTNAME ||
              proxytype == CURLPROXY_SOCKS5 || proxytype == CURLPROXY_SOCKS4A ||
              proxytype == CURLPROXY_SOCKS4;
...
static CURLcode parse_proxy_auth(struct Curl_easy *data,
  ...
  if (!result)
    result = Curl_urldecode(proxypasswd, 0, &conn->http_proxy.passwd, NULL,
                            REJECT_ZERO);
  if (!result)
    result =
        Curl_setstropt(&data->state.aptr.proxypasswd, conn->http_proxy.passwd);
  return result;
}
...
static CURLcode create_conn_helper_init_proxy(struct Curl_easy *data,
  ...
  if (Curl_check_noproxy(conn->host.name,
  ...
  if (socksproxy &&
      (!*socksproxy || (conn->handler->flags & PROTOPT_NONETWORK))) {
...
CURLcode Curl_parse_login_details(const char *login, const size_t len,
  ...
  plen = (psep ? (osep && osep > psep ? (size_t)(osep - psep)
                                      : (size_t)(login + len - psep)) -
                     1
               : 0);
  olen = (osep ? (psep && psep > osep ? (size_t)(psep - osep)
                                      : (size_t)(login + len - osep)) -
                     1
               : 0);
  ...
  *port_result = -1;
  ...
  if (*ptr == ':') {
    ...
    char *hostname_to_match =
        aprintf("%s%s%s", conn->bits.ipv6_ip ? "[" : "", conn->host.name,
                conn->bits.ipv6_ip ? "]" : "");
...
static CURLcode parse_connect_to_slist(struct Curl_easy *data,
  ...
  while (conn_to_host && !host && port == -1) {
    result =
        parse_connect_to_string(data, conn, conn_to_host->data, &host, &port);
    if (result)
      return result;
  ...
  if (data->asi && !host && (port == -1) &&
    ...
    srcalpnid = ALPN_h2;
    hit = Curl_altsvc_lookup(data->asi, srcalpnid, host,
                             conn->remote_port, /* from */
                             &as /* to */, allowed_versions);
    if (!hit)
#endif
    {
      srcalpnid = ALPN_h1;
...
#ifdef USE_UNIX_SOCKETS
static CURLcode resolve_unix(struct Curl_easy *data, struct connectdata *conn,
                             char *unix_path) {
  ...
  hostaddr->addr =
      Curl_unix2addr(unix_path, &longpath, conn->bits.abstract_unix_socket);
  if (!hostaddr->addr) {
    if (longpath)
...
#ifndef CURL_DISABLE_PROXY
static CURLcode resolve_proxy(struct Curl_easy *data, struct connectdata *conn,
                              bool *async) {
  struct Curl_dns_entry *hostaddr = NULL;
...
static CURLcode resolve_host(struct Curl_easy *data, struct connectdata *conn,
                             bool *async) {
  struct Curl_dns_entry *hostaddr = NULL;
  ...
  conn->port = conn->bits.conn_to_port ? conn->conn_to_port : conn->remote_port;
  ...
  if (rc == CURLRESOLV_PENDING)
    *async = TRUE;
    ...
    failf(data,
          "Failed to resolve host '%s' with timeout after "
...
static CURLcode resolve_fresh(struct Curl_easy *data, struct connectdata *conn,
                              bool *async) {
#ifdef USE_UNIX_SOCKETS
  ...
  if (!unix_path && conn->socks_proxy.host.name &&
      !strncmp(UNIX_SOCKET_PREFIX "/", conn->socks_proxy.host.name,
               sizeof(UNIX_SOCKET_PREFIX)))
 ...
 *************************************************************/
static CURLcode resolve_server(struct Curl_easy *data, struct connectdata *conn,
                               bool *async) {
  DEBUGASSERT(conn);
  DEBUGASSERT(data);
 ...
 * and `temp` is freed.
 */
static void reuse_conn(struct Curl_easy *data, struct connectdata *temp,
...
static CURLcode create_conn(struct Curl_easy *data,
                            struct connectdata **in_connect, bool *async) {
  CURLcode result = CURLE_OK;
  struct connectdata *conn;
...
CURLcode Curl_connect(struct Curl_easy *data, bool *asyncp,
                      bool *protocol_done) {
  CURLcode result;
  struct connectdata *conn;
...
CURLcode Curl_data_priority_add_child(struct Curl_easy *parent,
                                      struct Curl_easy *child, bool exclusive) {
