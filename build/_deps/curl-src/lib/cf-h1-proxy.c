...
struct h1_tunnel_state {
  ...
  size_t nsent;
  size_t headerlines;
  enum keeponval { KEEPON_DONE, KEEPON_CONNECT, KEEPON_IGNORE } keepon;
...
static CURLcode tunnel_reinit(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static CURLcode tunnel_init(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static void tunnel_free(struct Curl_cfilter *cf, struct Curl_easy *data) {
...
static CURLcode send_CONNECT(struct Curl_cfilter *cf, struct Curl_easy *data,
                             struct h1_tunnel_state *ts, bool *done) {
...
static CURLcode on_resp_header(struct Curl_cfilter *cf, struct Curl_easy *data,
                               struct h1_tunnel_state *ts, const char *header) {
  CURLcode result = CURLE_OK;
  ...
  if ((checkprefix("WWW-Authenticate:", header) && (401 == k->httpcode)) ||
    ...
    if (k->httpcode / 100 == 2) {
      ...
      (void)curlx_strtoofft(header + strlen("Content-Length:"), NULL, 10,
                            &ts->cl);
    }
    ...
    if (k->httpcode / 100 == 2) {
      ...
      infof(data,
            ...
            k->httpcode);
    } else if (Curl_compareheader(header, STRCONST("Transfer-Encoding:"),
                                  STRCONST("chunked"))) {
  ...
  } else if (Curl_compareheader(header, STRCONST("Proxy-Connection:"),
                                STRCONST("close")))
    ts->close_connection = TRUE;
    ...
    data->info.httpproxycode = k->httpcode =
        (header[9] - '0') * 100 + (header[10] - '0') * 10 + (header[11] - '0');
  }
  return result;
}
    ...
    if (('\r' == linep[0]) || ('\n' == linep[0])) {
      ...
      if ((407 == k->httpcode) && !data->state.authproblem) {
        ...
        if (ts->cl) {
          ...
          r = Curl_httpchunk_read(data, linep + 1, 1, &gotbytes, &extra);
          if (r == CHUNKE_STOP) {
            /* we're done reading chunks! */
            infof(data, "chunk reading DONE");
            ts->keepon = KEEPON_DONE;
...
static CURLcode CONNECT_host(struct Curl_cfilter *cf, struct Curl_easy *data,
                             char **pauthority, char **phost_header) {
  const char *hostname;
  int port;
  bool ipv6_ip;
  CURLcode result;
...
static CURLcode start_CONNECT(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (hyper_request_set_uri(req, (uint8_t *)authority, strlen(authority))) {
  ...
  result = Curl_http_output_auth(data, conn, "CONNECT", HTTPREQ_GET, authority,
                                 TRUE);
  if (result)
    goto error;
  ...
  if ((conn->http_proxy.proxytype == CURLPROXY_HTTP_1_0) &&
      (HYPERE_OK != hyper_request_set_version(req, HYPER_HTTP_VERSION_1_0))) {
...
static CURLcode H1_CONNECT(struct Curl_cfilter *cf, struct Curl_easy *data,
                           struct h1_tunnel_state *ts) {
  struct connectdata *conn = cf->conn;
  CURLcode result;
  bool done;
...
static void cf_h1_proxy_close(struct Curl_cfilter *cf, struct Curl_easy *data) {
