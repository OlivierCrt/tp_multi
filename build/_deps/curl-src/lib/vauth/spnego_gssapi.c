 ...
 */
bool Curl_auth_is_spnego_supported(void) { return TRUE; }
...
CURLcode Curl_auth_decode_spnego_message(struct Curl_easy *data,
  ...
  if (!nego->spn) {
    ...
    major_status = gss_import_name(&minor_status, &spn_token,
                                   GSS_C_NT_HOSTBASED_SERVICE, &nego->spn);
  ...
  major_status = Curl_gss_init_sec_context(
      data, &minor_status, &nego->context, nego->spn, &Curl_spnego_mech_oid,
      GSS_C_NO_CHANNEL_BINDINGS, &input_token, &output_token, TRUE, NULL);
...
CURLcode Curl_auth_create_spnego_message(struct negotiatedata *nego,
  ...
  result = Curl_base64_encode(nego->output_token.value,
                              nego->output_token.length, outptr, outlen);
