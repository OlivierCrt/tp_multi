 ...
 */
bool Curl_auth_is_gssapi_supported(void) { return TRUE; }
...
CURLcode Curl_auth_create_gssapi_user_message(
  ...
  major_status =
      Curl_gss_init_sec_context(data, &minor_status, &krb5->context, krb5->spn,
                                &Curl_krb5_mech_oid, GSS_C_NO_CHANNEL_BINDINGS,
                                &input_token, &output_token, mutual_auth, NULL);

  if (GSS_ERROR(major_status)) {
    if (output_token.value)
      gss_release_buffer(&unused_status, &output_token);

    Curl_gss_log_error(data, "gss_init_sec_context() failed: ", major_status,
                       minor_status);
...
CURLcode Curl_auth_create_gssapi_security_message(struct Curl_easy *data,
  ...
  if (GSS_ERROR(major_status)) {
    Curl_gss_log_error(data, "gss_unwrap() failed: ", major_status,
                       minor_status);
    return CURLE_BAD_CONTENT_ENCODING;
  }
  ...
  /* Encrypt the data */
  major_status = gss_wrap(&minor_status, krb5->context, 0, GSS_C_QOP_DEFAULT,
