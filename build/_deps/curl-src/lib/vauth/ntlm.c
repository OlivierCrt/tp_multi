 ...
 */
bool Curl_auth_is_ntlm_supported(void) { return TRUE; }
...
CURLcode Curl_auth_decode_ntlm_type2_message(struct Curl_easy *data,
  ...
  */
  ...
  if ((type2len < 32) || (memcmp(type2, NTLMSSP_SIGNATURE, 8) != 0) ||
 ...
 */
CURLcode
Curl_auth_create_ntlm_type1_message(struct Curl_easy *data, const char *userp,
                                    const char *passwdp, const char *service,
                                    const char *hostname, struct ntlmdata *ntlm,
                                    struct bufref *out) {
  ...
  */
  ...
  ntlmbuf = aprintf(
      ...
      LONGQUARTET(NTLMFLAG_NEGOTIATE_OEM | NTLMFLAG_REQUEST_TARGET |
                  NTLMFLAG_NEGOTIATE_NTLM_KEY | NTLMFLAG_NEGOTIATE_NTLM2_KEY |
                  NTLMFLAG_NEGOTIATE_ALWAYS_SIGN),
      SHORTPAIR(domlen), SHORTPAIR(domlen), SHORTPAIR(domoff), 0, 0,
      SHORTPAIR(hostlen), SHORTPAIR(hostlen), SHORTPAIR(hostoff), 0, 0,
      host, /* this is empty */
  ...
  DEBUG_OUT({
    fprintf(stderr,
            ...
            LONGQUARTET(NTLMFLAG_NEGOTIATE_OEM | NTLMFLAG_REQUEST_TARGET |
                        ...
                        NTLMFLAG_NEGOTIATE_ALWAYS_SIGN),
            NTLMFLAG_NEGOTIATE_OEM | NTLMFLAG_REQUEST_TARGET |
                NTLMFLAG_NEGOTIATE_NTLM_KEY | NTLMFLAG_NEGOTIATE_NTLM2_KEY |
                NTLMFLAG_NEGOTIATE_ALWAYS_SIGN);
    ntlm_print_flags(stderr, NTLMFLAG_NEGOTIATE_OEM | NTLMFLAG_REQUEST_TARGET |
                                 NTLMFLAG_NEGOTIATE_NTLM_KEY |
                                 NTLMFLAG_NEGOTIATE_NTLM2_KEY |
                                 NTLMFLAG_NEGOTIATE_ALWAYS_SIGN);
    fprintf(stderr, "\n****\n");
...
CURLcode Curl_auth_create_ntlm_type3_message(struct Curl_easy *data,
  ...
  */
  ...
  if (ntlm->flags & NTLMFLAG_NEGOTIATE_NTLM2_KEY) {
    ...
    /* LMv2 response */
    result = Curl_ntlm_core_mk_lmv2_resp(ntlmv2hash, entropy, &ntlm->nonce[0],
                                         lmresp);
    if (result)
      return result;

    /* NTLMv2 response */
    result = Curl_ntlm_core_mk_ntlmv2_resp(ntlmv2hash, entropy, ntlm,
                                           &ntlmv2resp, &ntresplen);
    if (result)
      return result;
