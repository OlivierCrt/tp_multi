 ...
 */
bool Curl_auth_is_digest_supported(void) { return TRUE; }
...
CURLcode Curl_auth_create_digest_md5_message(
  ...
  CURLcode result = auth_decode_digest_md5_message(
      chlg, nonce, sizeof(nonce), realm, sizeof(realm), algorithm,
      sizeof(algorithm), qop_options, sizeof(qop_options));
  if (result)
    return result;
  ...
  response =
      aprintf("username=\"%s\",realm=\"%s\",nonce=\"%s\","
              "cnonce=\"%s\",nc=\"%s\",digest-uri=\"%s\",response=%s,"
              "qop=%s",
              userp, realm, nonce, cnonce, nonceCount, spn, resp_hash_hex, qop);
  free(spn);
  if (!response)
    return CURLE_OUT_OF_MEMORY;
...
static CURLcode auth_create_digest_http_message(
    ...
    const unsigned char *request, const unsigned char *uripath,
  ...
  if (!digest->cnonce) {
    ...
    result =
        Curl_base64_encode(cnoncebuf, strlen(cnoncebuf), &cnonce, &cnonce_sz);
    if (result)
      return result;
  ...
  */
  ...
  hashthis =
      aprintf("%s:%s:%s", userp, digest->realm ? digest->realm : "", passwdp);
  if (!hashthis)
    return CURLE_OUT_OF_MEMORY;
  ...
  */
  ...
  if (digest->qop) {
    response = aprintf("username=\"%s\", "
                       ...
                       "qop=%s, "
                       "response=\"%s\"",
                       userp_quoted, realm_quoted, nonce_quoted, uripath,
                       digest->cnonce, digest->nc, digest->qop, request_digest);
...
CURLcode Curl_auth_create_digest_http_message(
  ...
  if (digest->algo <= ALGO_MD5SESS)
    return auth_create_digest_http_message(
        ...
        auth_digest_md5_to_ascii, Curl_md5it);
