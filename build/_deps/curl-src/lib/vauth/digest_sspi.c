...
CURLcode Curl_auth_create_digest_md5_message(
  ...
  status = s_pSecFn->AcquireCredentialsHandle(
      NULL, (TCHAR *)TEXT(SP_NAME_DIGEST), SECPKG_CRED_OUTBOUND, NULL,
  ...
  status = s_pSecFn->InitializeSecurityContext(&credentials, NULL, spn, 0, 0, 0,
                                               &chlg_desc, 0, &context,
...
CURLcode Curl_auth_create_digest_http_message(
  ...
  if (!digest->http_context) {
    ...
    status = s_pSecFn->InitializeSecurityContext(
        &credentials, NULL, spn, ISC_REQ_USE_HTTP_STYLE, 0, 0, &chlg_desc, 0,
        digest->http_context, &resp_desc, &attrs, &expiry);
