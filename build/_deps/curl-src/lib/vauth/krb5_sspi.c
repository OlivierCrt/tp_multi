...
bool Curl_auth_is_gssapi_supported(void) {
  ...
  status = s_pSecFn->QuerySecurityPackageInfo((TCHAR *)TEXT(SP_NAME_KERBEROS),
                                              &SecurityPackage);
...
CURLcode Curl_auth_create_gssapi_user_message(
    ...
    const char *service, const char *host, const bool mutual_auth,
  ...
  if (!krb5->credentials) {
    ...
    status = s_pSecFn->AcquireCredentialsHandle(
        NULL, (TCHAR *)TEXT(SP_NAME_KERBEROS), SECPKG_CRED_OUTBOUND, NULL,
        krb5->p_identity, NULL, NULL, krb5->credentials, &expiry);
    if (status != SEC_E_OK)
  ...
  status = s_pSecFn->InitializeSecurityContext(
      krb5->credentials, chlg ? krb5->context : NULL, krb5->spn,
      (mutual_auth ? ISC_REQ_MUTUAL_AUTH : 0), 0, SECURITY_NATIVE_DREP,
      chlg ? &chlg_desc : NULL, 0, &context, &resp_desc, &attrs, &expiry);
...
CURLcode Curl_auth_create_gssapi_security_message(struct Curl_easy *data,
  ...
  status = s_pSecFn->QueryContextAttributes(krb5->context, SECPKG_ATTR_SIZES,
                                            &sizes);
  ...
  appdatalen =
      wrap_buf[0].cbBuffer + wrap_buf[1].cbBuffer + wrap_buf[2].cbBuffer;
