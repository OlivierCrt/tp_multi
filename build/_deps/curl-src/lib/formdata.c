...
AddHttpPost(char *name, size_t namelength, char *value,
            curl_off_t contentslength, char *buffer, size_t bufferlength,
            char *contenttype, long flags, struct curl_slist *contentHeader,
            char *showfilename, char *userp, struct curl_httppost *parent_post,
 ...
 *
 ***************************************************************************/
static struct FormInfo *AddFormInfo(char *value, char *contenttype,
...
static CURLFORMcode FormAdd(struct curl_httppost **httppost,
  ...
  while (return_value == CURL_FORMADD_OK) {
    ...
    case CURLFORM_CONTENTLEN:
      ...
      current_form->contentslength = array_state
                                         ? (curl_off_t)(size_t)array_value
  ...
  if (CURL_FORMADD_OK == return_value) {
    ...
    for (form = first_form; form != NULL; form = form->more) {
      ...
      if (((form->flags & HTTPPOST_FILENAME) ||
          ...
          !form->contenttype) {
        char *f =
            (form->flags & HTTPPOST_BUFFER) ? form->showfilename : form->value;
        char const *type;
      ...
      if (!(form->flags &
            (HTTPPOST_FILENAME | HTTPPOST_READFILE | HTTPPOST_PTRCONTENTS |
      ...
      post = AddHttpPost(form->name, form->namelength, form->value,
                         form->contentslength, form->buffer, form->bufferlength,
                         form->contenttype, form->flags, form->contentheader,
                         form->showfilename, form->userp, post, httppost,
                         last_post);
...
CURLcode Curl_getformdata(struct Curl_easy *data, curl_mimepart *finalform,
                          struct curl_httppost *post,
        ...
        if (post->flags & (HTTPPOST_FILENAME | HTTPPOST_READFILE)) {
          if (!strcmp(file->contents, "-")) {
            ...
            result = curl_mime_data_cb(part, (curl_off_t)-1,
                                       ...
                                       fseeko_wrapper, NULL, (void *)stdin);
          } else
          ...
          if (!clen)
            clen = -1;
          result = curl_mime_data_cb(part, clen, fread_func, NULL, NULL,
                                     post->userp);
        } else {
          size_t uclen;
          if (!clen)
