...
static int mbedtls_bio_cf_write(void *bio, const unsigned char *buf,
                                size_t blen) {
...
static int mbedtls_bio_cf_read(void *bio, unsigned char *buf, size_t blen) {
  ...
  CURL_TRC_CF(data, cf, "mbedtls_bio_cf_in_read(len=%zu) -> %zd, err=%d", blen,
              nread, result);
...
static CURLcode mbed_connect_step1(struct Curl_cfilter *cf,
  ...
  if (ret) {
    ...
    failf(data, "mbedtls_ctr_drbg_seed returned (-0x%04X) %s", -ret, errorbuf);
  ...
  if (ca_info_blob && verifypeer) {
    ...
    if (ret < 0) {
      mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
      failf(data, "Error importing ca cert blob - mbedTLS: (-0x%04X) %s", -ret,
            errorbuf);
  ...
  if (ssl_config->key || ssl_config->key_blob) {
    if (ssl_config->key) {
      ...
      ret = mbedtls_pk_parse_keyfile(
          &backend->pk, ssl_config->key, ssl_config->key_passwd,
          mbedtls_ctr_drbg_random, &backend->ctr_drbg);
#else
      ...
      const unsigned char *key_data = (const unsigned char *)ssl_key_blob->data;
      ...
      if (ret) {
        mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
        failf(data, "Error parsing private key - mbedTLS: (-0x%04X) %s", -ret,
              errorbuf);
        return CURLE_SSL_CERTPROBLEM;
      }
    }
  ...
  ret = mbedtls_ssl_config_defaults(&backend->config, MBEDTLS_SSL_IS_CLIENT,
  ...
  mbedtls_ssl_conf_rng(&backend->config, mbedtls_ctr_drbg_random,
                       &backend->ctr_drbg);
  mbedtls_ssl_set_bio(&backend->ssl, cf, mbedtls_bio_cf_write,
                      mbedtls_bio_cf_read, NULL /*  rev_timeout() */);
...
static CURLcode mbed_connect_step2(struct Curl_cfilter *cf,
  ...
  const char *const pinnedpubkey =
      Curl_ssl_cf_is_proxy(cf) ? data->set.str[STRING_SSL_PINNEDPUBLICKEY_PROXY]
  ...
  if (ret == MBEDTLS_ERR_SSL_WANT_READ) {
    ...
    char errorbuf[128];
    mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
    failf(data, "ssl_handshake returned - mbedTLS: (-0x%04X) %s", -ret,
          errorbuf);
    return CURLE_SSL_CONNECT_ERROR;
  }
  ...
  if (pinnedpubkey) {
    ...
    if (mbedtls_x509_crt_parse_der(
            p, peercert->MBEDTLS_PRIVATE(raw).MBEDTLS_PRIVATE(p),
            peercert->MBEDTLS_PRIVATE(raw).MBEDTLS_PRIVATE(len))) {
#else
...
static CURLcode mbed_connect_step3(struct Curl_cfilter *cf,
                                   struct Curl_easy *data) {
  ...
  if (ssl_config->primary.sessionid) {
    ...
    retcode = Curl_ssl_addsessionid(cf, data, our_ssl_sessionid, 0, &added);
    Curl_ssl_sessionid_unlock(data);
    if (!added) {
      mbedtls_ssl_session_free(our_ssl_sessionid);
      free(our_ssl_sessionid);
...
static ssize_t mbed_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (ret < 0) {
    *curlcode =
        (ret == MBEDTLS_ERR_SSL_WANT_WRITE) ? CURLE_AGAIN : CURLE_SEND_ERROR;
    ret = -1;
  }
...
static ssize_t mbed_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (ret <= 0) {
    ...
    *curlcode =
        (ret == MBEDTLS_ERR_SSL_WANT_READ) ? CURLE_AGAIN : CURLE_RECV_ERROR;
    return -1;
  }
...
static CURLcode mbedtls_random(struct Curl_easy *data, unsigned char *entropy,
                               size_t length) {
  ...
  ret = mbedtls_ctr_drbg_seed(&ctr_drbg, mbedtls_entropy_func, &ctr_entropy,
                              NULL, 0);
  ...
  if (ret) {
    ...
    if (ret) {
      mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
      failf(data, "mbedtls_ctr_drbg_random returned (-0x%04X) %s", -ret,
            errorbuf);
    }
  }
...
static CURLcode mbed_connect_common(struct Curl_cfilter *cf,
 ...
 */
static int mbedtls_init(void) { return Curl_mbedtlsthreadlock_thread_setup(); }
...
static CURLcode mbedtls_sha256sum(const unsigned char *input, size_t inputlen,
...
const struct Curl_ssl Curl_ssl_mbedtls = {
    ...
    SSLSUPP_CA_PATH | SSLSUPP_CAINFO_BLOB | SSLSUPP_PINNEDPUBKEY |
        SSLSUPP_SSL_CTX | SSLSUPP_HTTPS_PROXY,
