...
static void pubkey_show(struct Curl_easy *data, BIO *mem, int num,
                        const char *type, const char *name, const BIGNUM *bn) {
  char *ptr;
  char namebuf[32];
...
static void X509V3_ext(struct Curl_easy *data, int certnum,
...
static int ossl_bio_cf_out_write(BIO *bio, const char *buf, int blen) {
  ...
  CURL_TRC_CF(data, cf, "ossl_bio_cf_out_write(len=%d) -> %d, err=%d", blen,
              (int)nwritten, result);
...
static int ossl_bio_cf_in_read(BIO *bio, char *buf, int blen) {
  ...
  CURL_TRC_CF(data, cf, "ossl_bio_cf_in_read(len=%d) -> %d, err=%d", blen,
              (int)nread, result);
  BIO_clear_retry_flags(bio);
  backend->io_result = result;
  if (nread < 0) {
...
static BIO_METHOD ossl_bio_cf_meth_1_0 = {BIO_TYPE_MEM,
                                          "OpenSSL CF BIO",
...
static void ossl_log_tls12_secret(const SSL *ssl, bool *keylog_done) {
  ...
  master_key_length = (int)SSL_SESSION_get_master_key(
  ...
  *keylog_done = true;
  Curl_tls_keylog_write("CLIENT_RANDOM", client_random, master_key,
                        master_key_length);
}
...
static CURLcode ossl_seed(struct Curl_easy *data) {
  ...
  do {
    ...
    for (i = 0, i_max = len / sizeof(struct curltime); i < i_max; ++i) {
      ...
      tv.tv_usec ^=
          (unsigned int)((Curl_now().tv_sec + Curl_now().tv_usec) * (i + 4))
          << 16;
...
static int SSL_CTX_use_certificate_blob(SSL_CTX *ctx,
                                        const struct curl_blob *blob, int type,
...
static int SSL_CTX_use_PrivateKey_blob(SSL_CTX *ctx,
                                       const struct curl_blob *blob, int type,
                                       const char *key_passwd) {
  int ret = 0;
  EVP_PKEY *pkey = NULL;
  ...
  if (type == SSL_FILETYPE_PEM)
    pkey =
        PEM_read_bio_PrivateKey(in, NULL, passwd_callback, (void *)key_passwd);
...
static int SSL_CTX_use_certificate_chain_blob(SSL_CTX *ctx,
...
static int cert_stuff(struct Curl_easy *data, SSL_CTX *ctx, char *cert_file,
                      const struct curl_blob *cert_blob, const char *cert_type,
                      char *key_file, const struct curl_blob *key_blob,
  ...
  if (cert_file || cert_blob || (file_type == SSL_FILETYPE_ENGINE)) {
    ...
    case SSL_FILETYPE_ASN1:
      ...
      cert_use_result =
          cert_blob ? SSL_CTX_use_certificate_blob(ctx, cert_blob, file_type,
                                                   key_passwd)
...
#if defined(USE_OPENSSL_ENGINE) && defined(ENGINE_CTRL_GET_CMD_FROM_NAME)
    {
      ...
      if (data->state.engine) {
        ...
        if (!ENGINE_ctrl(data->state.engine, ENGINE_CTRL_GET_CMD_FROM_NAME, 0,
                         (void *)cmd_name, NULL)) {
        ...
        if (!ENGINE_ctrl_cmd(data->state.engine, cmd_name, 0, &params, NULL,
                             1)) {
          failf(data,
    ...
    case SSL_FILETYPE_ASN1:
      cert_use_result =
          key_blob ? SSL_CTX_use_PrivateKey_blob(ctx, key_blob, file_type,
                                                 key_passwd)
...
#ifdef USE_OPENSSL_ENGINE
    {
      ...
      if (data->state.engine) {
        ...
        if (!ui_method) {
          failf(data,
                "unable do create " OSSL_PACKAGE " user-interface method");
          return 0;
        }
...
static CURLcode ossl_set_engine(struct Curl_easy *data, const char *engine) {
  ...
  if (!ENGINE_init(e)) {
    ...
    ENGINE_free(e);
    failf(data, "Failed to initialise SSL Engine '%s': %s", engine,
      ...
      if (SSL_shutdown(backend->handle) == 1) {
        ...
        default:
          ...
          CURL_TRC_CF(data, cf, "SSL shutdown, error: '%s', errno %d",
                      (sslerr ? ossl_strerror(sslerr, buf, sizeof(buf))
...
static bool subj_alt_hostcheck(struct Curl_easy *data,
                               const char *match_pattern, size_t matchlen,
  ...
  if (Curl_cert_hostcheck(match_pattern, matchlen, hostname, hostlen)) {
    infof(data, " subjectAltName: host \"%s\" matched cert's \"%s\"", dispname,
          match_pattern);
    return TRUE;
  }
  return FALSE;
...
static CURLcode ossl_verifyhost(struct Curl_easy *data,
                                struct connectdata *conn, X509 *server_cert,
                                const char *hostname, const char *dispname);
        ...
        case GEN_DNS: /* name/pattern comparison */
          ...
          */
          if ((altlen == strlen(altptr)) &&
              ...
              subj_alt_hostcheck(data, altptr, altlen, hostname, hostlen,
                                 dispname)) {
            dnsmatched = TRUE;
          }
          break;
...
static CURLcode verifystatus(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct ssl_connect_data *connssl = cf->ctx;
  int i, ocsp_status;
#if defined(OPENSSL_IS_AWSLC)
...
static void ossl_trace(int direction, int ssl_ver, int content_type,
  ...
  if (ssl_ver
    ...
    txt_len =
        msnprintf(ssl_buf, sizeof(ssl_buf), "%s (%s), %s, %s (%d):\n", verstr,
                  direction ? "OUT" : "IN", tls_rt_name, msg_name, msg_type);
  ...
  Curl_debug(data,
             (direction == 1) ? CURLINFO_SSL_DATA_OUT : CURLINFO_SSL_DATA_IN,
             (char *)buf, len);
  (void)ssl;
}
#endif
...
static int ossl_new_session_cb(SSL *ssl, SSL_SESSION *ssl_sessionid) {
  ...
  if (config->primary.sessionid) {
    ...
    if (!incache) {
      if (!Curl_ssl_addsessionid(cf, data, ssl_sessionid, 0 /* unknown size */,
                                 &added)) {
        if (added) {
...
static CURLcode populate_x509_store(struct Curl_cfilter *cf,
                                    struct Curl_easy *data, X509_STORE *store) {
  struct ssl_primary_config *conn_config = Curl_ssl_cf_get_primary_config(cf);
  ...
  if (verifypeer) {
    ...
    if (!ssl_cafile && !ssl_capath && !imported_native_ca &&
        !imported_ca_info_blob) {
...
static X509_STORE *get_cached_x509_store(struct Curl_cfilter *cf,
  ...
  if (multi && multi->ssl_backend_data && multi->ssl_backend_data->store &&
...
CURLcode Curl_ssl_setup_x509_store(struct Curl_cfilter *cf,
  ...
  cache_criteria_met = (data->set.general_ssl.ca_cache_timeout != 0) &&
                       conn_config->verifypeer && !conn_config->CApath &&
...
static CURLcode ossl_connect_step1(struct Curl_cfilter *cf,
  ...
  */
  ...
  if (ssl_cert || ssl_cert_blob || ssl_cert_type) {
    if (!result &&
        !cert_stuff(data, backend->ctx, ssl_cert, ssl_cert_blob, ssl_cert_type,
                    ssl_config->key, ssl_config->key_blob, ssl_config->key_type,
   ...
   */
  SSL_CTX_set_session_cache_mode(backend->ctx, SSL_SESS_CACHE_CLIENT |
  ...
  if (data->set.ssl.fsslctx) {
    ...
    result =
        (*data->set.ssl.fsslctx)(data, backend->ctx, data->set.ssl.fsslctxp);
  ...
  if (ssl_config->primary.sessionid) {
    ...
    if (!Curl_ssl_getsessionid(cf, data, &ssl_sessionid, NULL)) {
      ...
      if (!SSL_set_session(backend->handle, ssl_sessionid)) {
        Curl_ssl_sessionid_unlock(data);
        failf(
            data, "SSL: SSL_set_session failed: %s",
            ossl_strerror(ERR_get_error(), error_buffer, sizeof(error_buffer)));
        return CURLE_SSL_CONNECT_ERROR;
      }
 ...
 */
static CURLcode servercert(struct Curl_cfilter *cf, struct Curl_easy *data,
                           bool strict) {
  ...
  infof(data, "%s certificate:", Curl_ssl_cf_is_proxy(cf) ? "Proxy" : "Server");

  rc = x509_name_oneline(X509_get_subject_name(backend->server_cert), buffer,
                         sizeof(buffer));
  ...
  rc = x509_name_oneline(X509_get_issuer_name(backend->server_cert), buffer,
                         sizeof(buffer));
  if (rc) {
    if (strict)
...
static CURLcode ossl_connect_step3(struct Curl_cfilter *cf,
  ...
  result =
      servercert(cf, data, conn_config->verifypeer || conn_config->verifyhost);
...
static CURLcode ossl_connect_common(struct Curl_cfilter *cf,
  ...
  while (ssl_connect_2 == connssl->connecting_state ||
    ...
    if (!nonblocking && (connssl->connecting_state == ssl_connect_2_reading ||
                         connssl->connecting_state == ssl_connect_2_writing)) {
...
static CURLcode ossl_connect(struct Curl_cfilter *cf, struct Curl_easy *data) {
  CURLcode result;
  bool done = FALSE;
...
static ssize_t ossl_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (rc <= 0) {
    ...
    case SSL_ERROR_SYSCALL: {
      ...
      failf(data, OSSL_PACKAGE " SSL_write: %s, errno %d", error_buffer,
            sockerr);
    ...
    case SSL_ERROR_SSL: {
      ...
      struct ssl_connect_data *connssl_next =
          cf_ssl_next ? cf_ssl_next->ctx : NULL;
      sslerror = ERR_get_error();
  ...
  if (nread <= 0) {
    ...
    default:
      ...
      if ((nread < 0) || sslerror) {
        ...
        failf(data, OSSL_PACKAGE " SSL_read: %s, errno %d", error_buffer,
              sockerr);
...
static size_t ossl_version(char *buffer, size_t size) {
  ...
  return count;
#else
  return msnprintf(buffer, size, "%s/%lx.%lx.%lx", OSSL_PACKAGE,
  ...
  return msnprintf(buffer, size, "%s/%s", OSSL_PACKAGE, CURL_BORINGSSL_VERSION);
#else
  return msnprintf(buffer, size, OSSL_PACKAGE);
#endif
#elif defined(OPENSSL_IS_AWSLC)
  return msnprintf(buffer, size, "%s/%s", OSSL_PACKAGE,
  ...
  return msnprintf(buffer, size,
                   "%s/%lx.%lx.%lx%s"
#ifdef OPENSSL_FIPS
                   "-fips"
#endif
                   ,
                   OSSL_PACKAGE, (ssleay_value >> 28) & 0xf,
                   (ssleay_value >> 20) & 0xff, (ssleay_value >> 12) & 0xff,
                   sub);
#endif /* OPENSSL_IS_BORINGSSL */
}

/* can be called with data == NULL */
static CURLcode ossl_random(struct Curl_easy *data, unsigned char *entropy,
                            size_t length) {
  int rc;
  if (data) {
...
static void *ossl_get_internals(struct ssl_connect_data *connssl,
  ...
  struct ossl_ssl_backend_data *backend =
      (struct ossl_ssl_backend_data *)connssl->backend;
  DEBUGASSERT(backend);
  return info == CURLINFO_TLS_SESSION ? (void *)backend->ctx
                                      : (void *)backend->handle;
}

static void
ossl_free_multi_ssl_backend_data(struct multi_ssl_backend_data *mbackend) {
#if defined(HAVE_SSL_X509_STORE_SHARE)
  if (mbackend->store) {
    X509_STORE_free(mbackend->store);
  }
...
const struct Curl_ssl Curl_ssl_openssl = {
    ...
    SSLSUPP_CA_PATH | SSLSUPP_CAINFO_BLOB | SSLSUPP_CERTINFO |
        SSLSUPP_PINNEDPUBKEY | SSLSUPP_SSL_CTX |
#ifdef HAVE_SSL_CTX_SET_CIPHERSUITES
