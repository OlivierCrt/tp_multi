...
#ifdef USE_SSL
static const struct alpn_spec ALPN_SPEC_H10 = {{ALPN_HTTP_1_0}, 1};
static const struct alpn_spec ALPN_SPEC_H11 = {{ALPN_HTTP_1_1}, 1};
#ifdef USE_HTTP2
static const struct alpn_spec ALPN_SPEC_H2_H11 = {{ALPN_H2, ALPN_HTTP_1_1}, 2};
#endif
...
static CURLcode ssl_connect_nonblocking(struct Curl_cfilter *cf,
 ...
 */
bool Curl_ssl_getsessionid(struct Curl_cfilter *cf, struct Curl_easy *data,
                           void **ssl_sessionid,
 ...
 * later on.
 */
CURLcode Curl_ssl_addsessionid(struct Curl_cfilter *cf, struct Curl_easy *data,
                               void *ssl_sessionid, size_t idsize,
                               bool *added) {
  struct ssl_connect_data *connssl = cf->ctx;
  struct ssl_config_data *ssl_config = Curl_ssl_cf_get_config(cf, data);
  struct ssl_primary_config *conn_config = Curl_ssl_cf_get_primary_config(cf);
  ...
  for (i = 1; (i < data->set.general_ssl.max_ssl_sessions) &&
              data->state.session[i].sessionid;
       i++) {
...
CURLcode Curl_ssl_random(struct Curl_easy *data, unsigned char *entropy,
                         size_t length) {
...
static CURLcode pubkey_pem_to_der(const char *pem, unsigned char **der,
                                  size_t *der_len) {
...
CURLcode Curl_pin_peer_pubkey(struct Curl_easy *data, const char *pinnedpubkey,
  ...
  if (strncmp(pinnedpubkey, "sha256//", 8) == 0) {
    ...
    if (!sha256sumdigest)
      return CURLE_OUT_OF_MEMORY;
    encode = Curl_ssl->sha256sum(pubkey, pubkeylen, sha256sumdigest,
                                 CURL_SHA256_DIGEST_LENGTH);

    if (!encode)
      encode =
          Curl_base64_encode((char *)sha256sumdigest, CURL_SHA256_DIGEST_LENGTH,
                             &encoded, &encodedlen);
...
void Curl_none_close_all(struct Curl_easy *data UNUSED_PARAM) { (void)data; }

void Curl_none_session_free(void *ptr UNUSED_PARAM) { (void)ptr; }
...
bool Curl_none_cert_status_request(void) { return FALSE; }
...
bool Curl_none_false_start(void) { return FALSE; }
...
static ssize_t multissl_recv_plain(struct Curl_cfilter *cf,
                                   struct Curl_easy *data, char *buf,
...
static ssize_t multissl_send_plain(struct Curl_cfilter *cf,
                                   struct Curl_easy *data, const void *mem,
                                   size_t len, CURLcode *code) {
  if (multissl_setup(NULL))
    return CURLE_FAILED_INIT;
...
static CURLcode reinit_hostname(struct Curl_cfilter *cf) {
  ...
  if (ehostname &&
      (!connssl->hostname || strcmp(ehostname, connssl->hostname))) {
    free_hostname(connssl);
...
static void ssl_cf_close(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct cf_call_data save;
...
static CURLcode ssl_cf_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static ssize_t ssl_cf_send(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static ssize_t ssl_cf_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static CURLcode ssl_cf_query(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static bool cf_ssl_is_alive(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  return cf->next ? cf->next->cft->is_alive(cf->next, data, input_pending)
...
struct Curl_cftype Curl_cft_ssl_proxy = {
    "SSL-PROXY",          CF_TYPE_SSL,
    CURL_LOG_LVL_NONE,    ssl_cf_destroy,
    ssl_cf_connect,       ssl_cf_close,
    Curl_cf_def_get_host, ssl_cf_get_select_socks,
    ssl_cf_data_pending,  ssl_cf_send,
    ssl_cf_recv,          ssl_cf_cntrl,
    cf_ssl_is_alive,      Curl_cf_def_conn_keep_alive,
    Curl_cf_def_query,
};

static CURLcode cf_ssl_create(struct Curl_cfilter **pcf, struct Curl_easy *data,
  ...
  ctx = cf_ctx_new(
      data, alpn_get_spec(data->state.httpwant, conn->bits.tls_enable_alpn));
  if (!ctx) {
    result = CURLE_OUT_OF_MEMORY;
    goto out;
  }
...
CURLcode Curl_ssl_cfilter_add(struct Curl_easy *data, struct connectdata *conn,
                              int sockindex) {
...
CURLcode Curl_ssl_cfilter_remove(struct Curl_easy *data, int sockindex) {
...
#else
  return Curl_ssl_cf_is_proxy(cf) ? &cf->conn->proxy_ssl_config
                                  : &cf->conn->ssl_config;
#endif
}
...
out:
  if (!Curl_ssl_cf_is_proxy(cf))
    Curl_multiuse_state(data,
                        can_multi ? BUNDLE_MULTIPLEX : BUNDLE_NO_MULTIUSE);
  return CURLE_OK;
}
