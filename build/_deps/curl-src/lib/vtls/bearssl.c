  ...
  do {
    ...
    while (n) {
      ...
      case BR_PEM_END_OBJ:
        ...
        new_anchors =
            realloc(ca.anchors, new_anchors_len * sizeof(ca.anchors[0]));
        if (!new_anchors) {
          ca.err = CURLE_OUT_OF_MEMORY;
          goto fail;
        }
...
static const br_x509_class x509_vtable = {sizeof(struct x509_context),
                                          x509_start_chain,
                                          x509_start_cert,
                                          x509_append,
                                          x509_end_cert,
  ...
  while (true) {
    ...
    if (strncasecompare(cipher_name, "TLS_", 4)) {
      for (i = 0; i < NUM_OF_CIPHERS &&
                  !strcasecompare(cipher_name, ciphertable[i].name);
           ++i)
        ;
    } else {
      for (i = 0; i < NUM_OF_CIPHERS &&
                  !strcasecompare(cipher_name, ciphertable[i].alias_name);
           ++i)
        ;
    }
    if (i == NUM_OF_CIPHERS) {
    ...
    for (j = 0; j < selected_count && selected_ciphers[j] != ciphertable[i].num;
         j++)
      ;
    if (j < selected_count) {
...
static CURLcode bearssl_connect_step1(struct Curl_cfilter *cf,
  ...
  if (data->set.ssl.fsslctx) {
    ...
    ret = (*data->set.ssl.fsslctx)(data, &backend->ctx, data->set.ssl.fsslctxp);
...
static CURLcode bearssl_run_until(struct Curl_cfilter *cf,
                                  struct Curl_easy *data, unsigned target) {
  struct ssl_connect_data *connssl = cf->ctx;
  struct bearssl_ssl_backend_data *backend =
      (struct bearssl_ssl_backend_data *)connssl->backend;
  unsigned state;
...
static CURLcode bearssl_sha256sum(const unsigned char *input, size_t inputlen,
