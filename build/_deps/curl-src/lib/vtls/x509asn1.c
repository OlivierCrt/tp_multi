...
static const char *getASN1Element(struct Curl_asn1Element *elem,
                                  const char *beg,
                                  const char *end) WARN_UNUSED_RESULT;
 ...
 * string length.
 */
static ssize_t utf8asn1str(char **to, int type, const char *from,
                           const char *end) {
 ...
 */
static size_t encodeOID(char *buf, size_t buflen, const char *beg,
                        const char *end) {
  size_t i;
  unsigned int x;
  unsigned int y;
...
static const char *GTime2str(const char *beg, const char *end) {
  ...
  tzl = end - tzp;
  return curl_maprintf("%.4s-%.2s-%.2s %.2s:%.2s:%c%c%s%.*s%s%.*s", beg,
                       beg + 4, beg + 6, beg + 8, beg + 10, sec1, sec2,
                       fracl ? "." : "", (int)fracl, fracp, sep, (int)tzl, tzp);
}
...
static const char *UTime2str(const char *beg, const char *end) {
  ...
  return curl_maprintf("%u%.2s-%.2s-%.2s %.2s:%.2s:%.2s %.*s",
                       20 - (*beg >= '5'), beg, beg + 2, beg + 4, beg + 6,
                       beg + 8, sec, (int)tzl, tzp);
}
...
static const char *dumpAlgo(struct Curl_asn1Element *param, const char *beg,
                            const char *end) {
  struct Curl_asn1Element oid;
 ...
 * terminated value.
 */
static CURLcode ssl_push_certinfo(struct Curl_easy *data, int certnum,
...
CURLcode Curl_extract_certinfo(struct Curl_easy *data, int certnum,
  ...
  result = Curl_base64_encode(cert.certificate.beg,
                              cert.certificate.end - cert.certificate.beg, &cp1,
                              &cl1);
  if (result)
    return result;
...
CURLcode Curl_verifyhost(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (!getASN1Element(&elem, name.beg, name.end))
    ...
    else if (Curl_cert_hostcheck((const char *)dnsname, len, connssl->hostname,
                                 hostlen)) {
