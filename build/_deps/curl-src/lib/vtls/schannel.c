...
static CURLcode schannel_acquire_credential_handle(struct Curl_cfilter *cf,
  ...
  if (data->set.ssl.primary.clientcert || data->set.ssl.primary.cert_blob) {
    ...
    if (blob) {
      ...
      result = get_cert_location(cert_path, &cert_store_name, &cert_store_path,
                                 &cert_thumbprint_str);
    ...
    if ((fInCert || blob) && (data->set.ssl.cert_type) &&
      ...
      failf(data,
            "schannel: certificate format compatibility error "
            " for %s",
    ...
    if (fInCert || blob) {
      ...
      */
      ...
      if (pszPassword) {
        if (pwd_len > 0)
          str_w_len = MultiByteToWideChar(
              CP_UTF8, MB_ERR_INVALID_CHARS, data->set.ssl.key_passwd,
              (int)pwd_len, pszPassword, (int)(pwd_len + 1));
        ...
        if (curlx_verify_windows_version(6, 0, 0, PLATFORM_WINNT,
                                         VERSION_GREATER_THAN_EQUAL))
          cert_store =
              PFXImportCertStore(&datablob, pszPassword, PKCS12_NO_PERSIST_KEY);
        else
      ...
      client_certs[0] = CertFindCertificateInStore(
          cert_store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0, CERT_FIND_ANY,
          NULL, NULL);
      ...
      cert_store = CertOpenStore(
          CURL_CERT_STORE_PROV_SYSTEM, 0, (HCRYPTPROV)NULL,
      ...
      if (!CryptStringToBinary(cert_thumbprint_str, CERT_THUMBPRINT_STR_LEN,
                               CRYPT_STRING_HEX, cert_thumbprint_data,
                               &cert_thumbprint.cbData, NULL, NULL)) {
        curlx_unicodefree(cert_path);
        CertCloseStore(cert_store, 0);
        return CURLE_SSL_CERTPROBLEM;
      }
  ...
  backend->cred =
      (struct Curl_schannel_cred *)calloc(1, sizeof(struct Curl_schannel_cred));
  if (!backend->cred) {
  ...
  if (!conn_config->cipher_list &&
    ...
    if (ciphers13) {
      ...
      */
      ...
      while (startCur && (0 != *startCur) && (algCount < remaining_ciphers)) {
        ...
        if (disable_aes_gcm_sha384 && !strcmp("TLS_AES_256_GCM_SHA384", tmp)) {
    ...
    if (disable_aes_gcm_sha384 || disable_aes_gcm_sha256) {
      ...
      */
      ...
      if (disable_aes_gcm_sha384 != disable_aes_gcm_sha256) {
        ...
        crypto_settings[crypto_settings_idx].strCngAlgId.Length =
            sizeof(disable_aes_gcm_sha384 ? BCRYPT_SHA384_ALGORITHM
                                          : BCRYPT_SHA256_ALGORITHM);
        crypto_settings[crypto_settings_idx].strCngAlgId.MaximumLength =
            sizeof(disable_aes_gcm_sha384 ? BCRYPT_SHA384_ALGORITHM
                                          : BCRYPT_SHA256_ALGORITHM);
        crypto_settings[crypto_settings_idx].strCngAlgId.Buffer =
            (PWSTR)(disable_aes_gcm_sha384 ? BCRYPT_SHA384_ALGORITHM
                                           : BCRYPT_SHA256_ALGORITHM);
    ...
    */
    ...
    sspi_status = s_pSecFn->AcquireCredentialsHandle(
        ...
        NULL, NULL, &backend->cred->cred_handle, &backend->cred->time_stamp);
  } else {
    ...
    if (ciphers) {
      if ((enabled_protocols & SP_PROT_TLS1_3_CLIENT)) {
        infof(data,
              "schannel: WARNING: This version of Schannel may "
      ...
      if (conn_config->cipher_list13) {
        failf(data,
              "schannel: This version of Schannel does not support "
    ...
    sspi_status = s_pSecFn->AcquireCredentialsHandle(
        NULL, (TCHAR *)UNISP_NAME, SECPKG_CRED_OUTBOUND, NULL, &schannel_cred,
        NULL, NULL, &backend->cred->cred_handle, &backend->cred->time_stamp);
  }
...
#error "compiler too old to support requisite manual cert verify for Win CE"
  ...
  if (backend->use_alpn) {
    ...
    *extension_len = *list_len + (unsigned short)sizeof(unsigned int) +
  ...
  backend->ctxt =
      (struct Curl_schannel_ctxt *)calloc(1, sizeof(struct Curl_schannel_ctxt));
  if (!backend->ctxt) {
    failf(data, "schannel: unable to allocate memory");
    return CURLE_OUT_OF_MEMORY;
  }
  ...
  */
  sspi_status = s_pSecFn->InitializeSecurityContext(
      ...
      backend->req_flags, 0, 0, (backend->use_alpn ? &inbuf_desc : NULL), 0,
      &backend->ctxt->ctxt_handle, &outbuf_desc, &backend->ret_flags,
  ...
  written = Curl_conn_cf_send(cf->next, data, outbuf.pvBuffer, outbuf.cbBuffer,
                              &result);
...
static CURLcode schannel_connect_step2(struct Curl_cfilter *cf,
  ...
  if (backend->encdata_length - backend->encdata_offset <
    ...
    size_t reallocated_length =
        backend->encdata_offset + CURL_SCHANNEL_BUFFER_FREE_SIZE;
  ...
  for (;;) {
    if (doread) {
      ...
      nread = Curl_conn_cf_recv(
          cf->next, data,
          (char *)(backend->encdata_buffer + backend->encdata_offset),
          backend->encdata_length - backend->encdata_offset, &result);
    ...
    sspi_status = s_pSecFn->InitializeSecurityContext(
        ...
        backend->cred->sni_hostname, backend->req_flags, 0, 0, &inbuf_desc, 0,
        NULL, &outbuf_desc, &backend->ret_flags, &backend->ctxt->time_stamp);
    ...
    if (sspi_status == SEC_I_CONTINUE_NEEDED || sspi_status == SEC_E_OK) {
      for (i = 0; i < 3; i++) {
        ...
        if (outbuf[i].BufferType == SECBUFFER_TOKEN && outbuf[i].cbBuffer > 0) {
          ...
          written = Curl_conn_cf_send(cf->next, data, outbuf[i].pvBuffer,
                                      outbuf[i].cbBuffer, &result);
...
static bool add_cert_to_certinfo(const CERT_CONTEXT *ccert_context,
  ...
  if (valid_cert_encoding(ccert_context)) {
    ...
    int insert_index =
        reverse_order ? (args->certs_count - 1) - args->idx : args->idx;
...
static CURLcode schannel_connect_step3(struct Curl_cfilter *cf,
  ...
  if (backend->use_alpn) {
    sspi_status = s_pSecFn->QueryContextAttributes(
        &backend->ctxt->ctxt_handle, SECPKG_ATTR_APPLICATION_PROTOCOL,
        &alpn_result);
  ...
  if (ssl_config->primary.sessionid) {
    ...
    if (!incache) {
      result = Curl_ssl_addsessionid(cf, data, backend->cred,
                                     sizeof(struct Curl_schannel_cred), &added);
      if (result) {
        Curl_ssl_sessionid_unlock(data);
  ...
  if (data->set.ssl.certinfo) {
    int certs_count = 0;
    sspi_status = s_pSecFn->QueryContextAttributes(
        &backend->ctxt->ctxt_handle, SECPKG_ATTR_REMOTE_CERT_CONTEXT,
        &ccert_context);
...
static CURLcode schannel_connect_common(struct Curl_cfilter *cf,
                                        struct Curl_easy *data,
...
static ssize_t schannel_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  data_len =
      backend->stream_sizes.cbHeader + len + backend->stream_sizes.cbTrailer;
  ...
  InitSecBuffer(&outbuf[0], SECBUFFER_STREAM_HEADER, ptr,
                backend->stream_sizes.cbHeader);
  ...
  sspi_status =
      s_pSecFn->EncryptMessage(&backend->ctxt->ctxt_handle, 0, &outbuf_desc, 0);
  ...
  if (sspi_status == SEC_E_OK) {
    ...
    */
    ...
    while (len > (size_t)written) {
      ...
      this_write = Curl_conn_cf_send(cf->next, data, ptr + written,
                                     len - written, &result);
      if (result == CURLE_AGAIN)
        continue;
...
static ssize_t schannel_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  */
    ...
    nread = Curl_conn_cf_recv(
        cf->next, data,
        (char *)(backend->encdata_buffer + backend->encdata_offset), size, err);
    if (*err) {
      nread = -1;
      if (*err == CURLE_AGAIN)
        DEBUGF(infof(data, "schannel: recv returned CURLE_AGAIN"));
    ...
    if (sspi_status == SEC_E_OK || sspi_status == SEC_I_RENEGOTIATE ||
      ...
      if (inbuf[1].BufferType == SECBUFFER_DATA) {
        ...
        if (backend->decdata_length - backend->decdata_offset < size ||
          ...
          reallocated_buffer =
              realloc(backend->decdata_buffer, reallocated_length);
          if (!reallocated_buffer) {
            *err = CURLE_OUT_OF_MEMORY;
            failf(data, "schannel: unable to re-allocate memory");
            goto cleanup;
...
cleanup:
  ...
  */
  if (len && !backend->decdata_offset && backend->recv_connection_closed &&
    ...
    bool isWin2k =
        curlx_verify_windows_version(5, 0, 0, PLATFORM_WINNT, VERSION_EQUAL);
...
static int schannel_shutdown(struct Curl_cfilter *cf, struct Curl_easy *data) {
  ...
  if (backend->cred && backend->ctxt) {
    ...
    sspi_status =
        s_pSecFn->ApplyControlToken(&backend->ctxt->ctxt_handle, &BuffDesc);
    ...
    sspi_status = s_pSecFn->InitializeSecurityContext(
        &backend->cred->cred_handle, &backend->ctxt->ctxt_handle,
        backend->cred->sni_hostname, backend->req_flags, 0, 0, NULL, 0,
        &backend->ctxt->ctxt_handle, &outbuf_desc, &backend->ret_flags,
        &backend->ctxt->time_stamp);

    if ((sspi_status == SEC_E_OK) || (sspi_status == SEC_I_CONTEXT_EXPIRED)) {
      /* send close message which is in output buffer */
      ssize_t written = Curl_conn_cf_send(cf->next, data, outbuf.pvBuffer,
                                          outbuf.cbBuffer, &result);
      s_pSecFn->FreeContextBuffer(outbuf.pvBuffer);
      if ((result != CURLE_OK) || (outbuf.cbBuffer != (size_t)written)) {
        infof(data,
...
static void schannel_cleanup(void) { Curl_sspi_global_cleanup(); }
...
static void schannel_checksum(const unsigned char *input, size_t inputlen,
                              unsigned char *checksum, size_t checksumlen,
                              DWORD provType, const unsigned int algId) {
  HCRYPTPROV hProv = 0;
  HCRYPTHASH hHash = 0;
  DWORD cbHashSize = 0;
...
static CURLcode schannel_sha256sum(const unsigned char *input, size_t inputlen,
                                   unsigned char *sha256sum, size_t sha256len) {
  schannel_checksum(input, inputlen, sha256sum, sha256len, PROV_RSA_AES,
                    CALG_SHA_256);
  return CURLE_OK;
}
...
const struct Curl_ssl Curl_ssl_schannel = {
    ...
    SSLSUPP_CERTINFO |
        ...
        SSLSUPP_CAINFO_BLOB |
#endif
        SSLSUPP_PINNEDPUBKEY | SSLSUPP_TLS13_CIPHERSUITES | SSLSUPP_HTTPS_PROXY,
