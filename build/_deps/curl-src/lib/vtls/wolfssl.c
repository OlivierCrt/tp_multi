...
static int wolfssl_tls13_secret_callback(SSL *ssl, int id,
...
static long wolfssl_bio_cf_ctrl(WOLFSSL_BIO *bio, int cmd, long num,
                                void *ptr) {
...
static int wolfssl_bio_cf_out_write(WOLFSSL_BIO *bio, const char *buf,
                                    int blen) {
  ...
  CURL_TRC_CF(data, cf, "bio_write(len=%d) -> %zd, %d", blen, nwritten, result);
 ...
 */
static CURLcode wolfssl_connect_step1(struct Curl_cfilter *cf,
...
#define use_sni(x) Curl_nop_stmt
  ...
  if (ssl_cafile || ssl_capath) {
    int rc = wolfSSL_CTX_load_verify_locations_ex(
        backend->ctx, ssl_cafile, ssl_capath, WOLFSSL_LOAD_FLAG_IGNORE_ERR);
    if (SSL_SUCCESS != rc) {
  ...
  if (ssl_config->primary.clientcert && ssl_config->key) {
    ...
    if (wolfSSL_CTX_use_certificate_file(
            backend->ctx, ssl_config->primary.clientcert, file_type) != 1) {
  ...
  wolfSSL_CTX_set_verify(
      backend->ctx, conn_config->verifypeer ? SSL_VERIFY_PEER : SSL_VERIFY_NONE,
      NULL);
...
#if defined(HAVE_SECRET_CALLBACK) && defined(WOLFSSL_TLS13)
    wolfSSL_set_tls13_secret_cb(backend->handle, wolfssl_tls13_secret_callback,
                                NULL);
#endif
  }
#endif /* OPENSSL_EXTRA */
...
static CURLcode wolfssl_connect_step2(struct Curl_cfilter *cf,
                                      struct Curl_easy *data) {
  int ret = -1;
  struct ssl_connect_data *connssl = cf->ctx;
  struct wolfssl_ssl_backend_data *backend =
  ...
  if (Curl_tls_keylog_enabled()) {
     ...
     */
    if (ret == SSL_SUCCESS || (!wolfSSL_want_read(backend->handle) &&
...
static CURLcode wolfssl_connect_step3(struct Curl_cfilter *cf,
                                      struct Curl_easy *data) {
  CURLcode result = CURLE_OK;
  struct ssl_connect_data *connssl = cf->ctx;
  struct wolfssl_ssl_backend_data *backend =
...
static ssize_t wolfssl_send(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static ssize_t wolfssl_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
                            char *buf, size_t blen, CURLcode *curlcode) {
  struct ssl_connect_data *connssl = cf->ctx;
  struct wolfssl_ssl_backend_data *backend =
      (struct wolfssl_ssl_backend_data *)connssl->backend;
  char error_buffer[WOLFSSL_MAX_ERROR_SZ];
...
static void wolfssl_session_free(void *ptr) { wolfSSL_SESSION_free(ptr); }
 ...
 */
static int wolfssl_shutdown(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct ssl_connect_data *ctx = cf->ctx;
  struct wolfssl_ssl_backend_data *backend;
  int retval = 0;
...
static CURLcode wolfssl_connect_common(struct Curl_cfilter *cf,
...
static CURLcode wolfssl_random(struct Curl_easy *data, unsigned char *entropy,
                               size_t length) {
  WC_RNG rng;
  (void)data;
  if (wc_InitRng(&rng))
...
const struct Curl_ssl Curl_ssl_wolfssl = {
    ...
    SSLSUPP_PINNEDPUBKEY |
        ...
        SSLSUPP_HTTPS_PROXY |
#endif
        SSLSUPP_CA_PATH | SSLSUPP_CAINFO_BLOB | SSLSUPP_SSL_CTX,
