...
static int is_cr_or_lf(char c) { return c == '\r' || c == '\n'; }
...
static CURLcode add_certs_data_to_store(HCERTSTORE trust_store,
  ...
  while (more_certs && (current_ca_file_ptr < ca_buffer_limit)) {
    const char *begin_cert_ptr =
        c_memmem(current_ca_file_ptr, ca_buffer_limit - current_ca_file_ptr,
                 BEGIN_CERT, begin_cert_len);
    if (!begin_cert_ptr || !is_cr_or_lf(begin_cert_ptr[begin_cert_len])) {
      more_certs = 0;
    } else {
      const char *end_cert_ptr =
          c_memmem(begin_cert_ptr, ca_buffer_limit - begin_cert_ptr, END_CERT,
                   end_cert_len);
      if (!end_cert_ptr) {
        failf(data, "schannel: CA file '%s' is not correctly formatted",
              ca_file_text);
        ...
        if (!CryptQueryObject(CERT_QUERY_OBJECT_BLOB, &cert_blob,
                              CERT_QUERY_CONTENT_FLAG_CERT,
                              CERT_QUERY_FORMAT_FLAG_ALL, 0, NULL,
                              &actual_content_type, NULL, NULL, NULL,
...
static CURLcode add_certs_file_to_store(HCERTSTORE trust_store,
   ...
   * small ( < 1 MiB ).
   */
  ca_file_handle = CreateFile(ca_file_tstr, GENERIC_READ, FILE_SHARE_READ, NULL,
  ...
  result = add_certs_data_to_store(trust_store, ca_file_buffer, ca_file_bufsize,
                                   ca_file, data);
...
static DWORD cert_get_name_string(struct Curl_easy *data,
                                  CERT_CONTEXT *cert_context, LPTSTR host_names,
                                  DWORD length) {
  ...
  if (curlx_verify_windows_version(6, 2, 0, PLATFORM_WINNT,
    ...
    actual_length = CertGetNameString(cert_context, CERT_NAME_DNS_TYPE,
                                      name_flags, NULL, host_names, length);
  ...
  extension = CertFindExtension(szOID_SUBJECT_ALT_NAME2, cert_info->cExtension,
  ...
  ret_val = CryptDecodeObjectEx(
      X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, szOID_SUBJECT_ALT_NAME2,
...
CURLcode Curl_verify_host(struct Curl_cfilter *cf, struct Curl_easy *data) {
  ...
  if (len == 0) {
    failf(data, "schannel: CertGetNameString() returned no "
  ...
  /* Sanity check */
  if (actual_len != len) {
    failf(data, "schannel: CertGetNameString() returned certificate "
...
CURLcode Curl_verify_certificate(struct Curl_cfilter *cf,
  ...
  if (result == CURLE_OK &&
    ...
    if (curlx_verify_windows_version(6, 1, 0, PLATFORM_WINNT,
      ...
      trust_store = CertOpenStore(CERT_STORE_PROV_MEMORY, 0, (HCRYPTPROV)NULL,
      ...
      if (!trust_store) {
        ...
        if (ca_info_blob) {
          result = add_certs_data_to_store(
              trust_store, (const char *)ca_info_blob->data, ca_info_blob->len,
              "(memory blob)", data);
        } else {
          result =
              add_certs_file_to_store(trust_store, conn_config->CAfile, data);
        }
      }
    }
    ...
    if (result == CURLE_OK) {
      ...
      if (!create_engine_result) {
        char buffer[STRERROR_LEN];
        failf(data, "schannel: failed to create certificate chain engine: %s",
              Curl_winapi_strerror(GetLastError(), buffer, sizeof(buffer)));
        result = CURLE_SSL_CACERT_BADFILE;
      }
    }
  ...
  if (result == CURLE_OK) {
    ...
    if (!CertGetCertificateChain(
            cert_chain_engine, pCertContextServer, NULL,
            pCertContextServer->hCertStore, &ChainPara,
            (ssl_config->no_revoke ? 0 : CERT_CHAIN_REVOCATION_CHECK_CHAIN),
            NULL, &pChainContext)) {
      char buffer[STRERROR_LEN];
