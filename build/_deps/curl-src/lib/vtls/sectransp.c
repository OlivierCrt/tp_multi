 ...
 */
#define CIPHER_DEF(num, alias, weak) {#num, alias, num, weak}
...
static const struct st_cipher ciphertable[] = {
    ...
    CIPHER_DEF(TLS_ECDHE_PSK_WITH_AES_128_CBC_SHA, /* 0xC035 */
               "ECDHE-PSK-AES128-CBC-SHA", CIPHER_STRONG_ENOUGH),
    CIPHER_DEF(TLS_ECDHE_PSK_WITH_AES_256_CBC_SHA, /* 0xC036 */
               "ECDHE-PSK-AES256-CBC-SHA", CIPHER_STRONG_ENOUGH),
    ...
    CIPHER_DEF(TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256, /* 0xCCA8 */
               "ECDHE-RSA-CHACHA20-POLY1305", CIPHER_STRONG_ENOUGH),
    CIPHER_DEF(TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256, /* 0xCCA9 */
               "ECDHE-ECDSA-CHACHA20-POLY1305", CIPHER_STRONG_ENOUGH),
#endif /* CURL_BUILD_MAC_10_13 || CURL_BUILD_IOS_11 */
...
static OSStatus sectransp_bio_cf_in_read(SSLConnectionRef connection, void *buf,
                                         size_t *dataLength) /* IN/OUT */
{
  ...
  CURL_TRC_CF(data, cf, "bio_read(len=%zu) -> %zd, result=%d", *dataLength,
              nread, result);
  if (nread < 0) {
    switch (result) {
    case CURLE_OK:
                                           ...
                                           size_t *dataLength) /* IN/OUT */
{
  ...
  CURL_TRC_CF(data, cf, "bio_send(len=%zu) -> %zd, result=%d", *dataLength,
              nwritten, result);
  if (nwritten <= 0) {
...
static CURLcode CopyCertSubject(struct Curl_easy *data, SecCertificateRef cert,
                                char **certp) {
...
static OSStatus CopyIdentityWithLabelOldSchool(char *label,
  ...
  /* Start searching: */
  status = SecKeychainSearchCreateFromAttributes(NULL, kSecCertificateItemClass,
                                                 &attr_list, &search);
  if (status == noErr) {
...
static OSStatus CopyIdentityWithLabel(char *label,
  ...
  if (SecItemCopyMatching && kSecClassIdentity) {
    ...
    query_dict = CFDictionaryCreate(
        NULL, (const void **)keys, (const void **)values, 5L,
...
static OSStatus CopyIdentityFromPKCS12File(const char *cPath,
  ...
  CFStringRef password =
      cPassword
          ? CFStringCreateWithCString(NULL, cPassword, kCFStringEncodingUTF8)
          : NULL;
  ...
  if (blob) {
    ...
    pkcs_url = CFURLCreateFromFileSystemRepresentation(
        NULL, (const UInt8 *)cPath, strlen(cPath), false);
    resource_imported = CFURLCreateDataAndPropertiesFromResource(
  ...
  if (resource_imported) {
    ...
    CFDictionaryRef options = CFDictionaryCreate(
        NULL, cKeys, cValues, password ? 1L : 0L, NULL, NULL);
    ...
    status = SecItemImport(pkcs_data, NULL, &inputFormat, &inputType, 0,
                           &keyParams, NULL, &items);
#endif
...
static CURLcode set_ssl_version_min_max(struct Curl_cfilter *cf,
  ...
  if (SSLSetProtocolVersionMax) {
    ...
    result =
        sectransp_version_from_curl(&darwin_ver_max, ssl_version_max >> 16);
    if (result) {
    ...
    long i = ssl_version;
    (void)SSLSetProtocolVersionEnabled(backend->ssl_ctx, kSSLProtocolAll,
                                       false);
    for (; i <= (ssl_version_max >> 16); i++) {
      ...
      case CURL_SSLVERSION_TLSv1_0:
        ...
        break;
      case CURL_SSLVERSION_TLSv1_1:
        (void)SSLSetProtocolVersionEnabled(backend->ssl_ctx, kTLSProtocol11,
                                           true);
        break;
      case CURL_SSLVERSION_TLSv1_2:
        (void)SSLSetProtocolVersionEnabled(backend->ssl_ctx, kTLSProtocol12,
                                           true);
        break;
      case CURL_SSLVERSION_TLSv1_3:
...
static CURLcode sectransp_set_selected_ciphers(struct Curl_easy *data,
  ...
  while (*cipher_start && ciphers_count < NUM_OF_CIPHERS) {
    ...
    if (!cipher_found) {
      ...
      failf(data,
            "SSL: cipher string \"%s\" contains unsupported cipher name"
            " starting position %zd and ending position %zd",
            ciphers, cipher_start - ciphers, cipher_end - ciphers);
      return CURLE_SSL_CIPHER;
    }
    if (*cipher_end) {
...
static CURLcode sectransp_connect_step1(struct Curl_cfilter *cf,
  ...
  if (connssl->alpn) {
    if (__builtin_available(macOS 10.13.4, iOS 11, tvOS 11, *)) {
      ...
      if (err != noErr)
        infof(data, "WARNING: failed to set ALPN protocols; OSStatus %d", err);
      CFRelease(alpnArr);
  ...
  if (ssl_cert || ssl_cert_blob) {
    ...
    if ((err != noErr) && (is_cert_file || is_cert_data)) {
      ...
      err = CopyIdentityFromPKCS12File(ssl_cert, ssl_cert_blob,
                                       ssl_config->key_passwd, &cert_and_key);
    }
  ...
  */
...
#endif /* CURL_BUILD_MAC */
    bool break_on_auth = !conn_config->verifypeer || ssl_cafile || ssl_cablob;
  ...
  if (conn_config->verifyhost) {
    ...
    if (err != noErr) {
      failf(data, "SSL: SSLSetPeerDomainName() failed: OSStatus %d", err);
      return CURLE_SSL_CONNECT_ERROR;
    }
  ...
  if (err != noErr) {
    failf(data,
          "SSL: Unable to set ciphers for SSL/TLS handshake. "
          "Error code: %d",
          err);
    return CURLE_SSL_CIPHER;
  }
  ...
  if (ssl_config->primary.sessionid) {
    ...
    /* If there isn't one, then let's make one up! This has to be done prior
      ...
      CURLcode result;
      ssl_sessionid =
          aprintf("%s:%d:%d:%s:%d", ssl_cafile ? ssl_cafile : "(blob memory)",
      ...
      result = Curl_ssl_addsessionid(cf, data, ssl_sessionid, ssl_sessionid_len,
                                     NULL);
      Curl_ssl_sessionid_unlock(data);
      if (result) {
  ...
  err = SSLSetIOFuncs(backend->ssl_ctx, sectransp_bio_cf_in_read,
...
static CURLcode verify_cert_buf(struct Curl_cfilter *cf, struct Curl_easy *data,
...
#ifdef SECTRANSP_PINNEDPUBKEY
static CURLcode pkp_pin_peer_pubkey(struct Curl_easy *data, SSLContextRef ctx,
                                    const char *pinnedpubkey) { /* Scratch */
  ...
  do {
...

    {
      OSStatus success;
      success =
          SecItemExport(keyRef, kSecFormatOpenSSL, 0, NULL, &publicKeyBits);
      CFRelease(keyRef);
    ...
    result =
        Curl_pin_peer_pubkey(data, pinnedpubkey, realpubkey, realpubkeylen);
  ...
  if (err != noErr) {
    ...
    case -9841:
      if ((conn_config->CAfile || conn_config->ca_info_blob) &&
        ...
        CURLcode result =
            verify_cert(cf, data, conn_config->CAfile,
                        conn_config->ca_info_blob, backend->ssl_ctx);
        if (result)
          return result;
      }
    ...
    if (data->set.str[STRING_SSL_PINNEDPUBLICKEY]) {
      CURLcode result = pkp_pin_peer_pubkey(
          data, backend->ssl_ctx, data->set.str[STRING_SSL_PINNEDPUBLICKEY]);
      if (result) {
...
static CURLcode add_cert_to_certinfo(struct Curl_easy *data,
...
static CURLcode collect_server_cert_single(struct Curl_cfilter *cf,
                                           struct Curl_easy *data,
...
static CURLcode collect_server_cert(struct Curl_cfilter *cf,
  ...
  struct ssl_config_data *ssl_config = Curl_ssl_cf_get_config(cf, data);
  CURLcode result =
      ssl_config->certinfo ? CURLE_PEER_FAILED_VERIFICATION : CURLE_OK;
  struct ssl_connect_data *connssl = cf->ctx;
  struct st_ssl_backend_data *backend =
      (struct st_ssl_backend_data *)connssl->backend;
...
#pragma unused(server_certs)
    ...
    if (err == noErr && server_certs) {
      ...
      for (i = 0L; !result && (i < count); i++) {
        server_cert =
            (SecCertificateRef)CFArrayGetValueAtIndex(server_certs, i);
        result = collect_server_cert_single(cf, data, server_cert, i);
      }
...
static CURLcode sectransp_connect_common(struct Curl_cfilter *cf,
                                         struct Curl_easy *data,
  ...
  while (ssl_connect_2 == connssl->connecting_state ||
    ...
    if (connssl->connecting_state == ssl_connect_2_reading ||
      ...
      curl_socket_t writefd = ssl_connect_2_writing == connssl->connecting_state
                                  ? sockfd
                                  : CURL_SOCKET_BAD;
...
static int sectransp_shutdown(struct Curl_cfilter *cf, struct Curl_easy *data) {
  ...
  what =
      SOCKET_READABLE(Curl_conn_cf_get_socket(cf, data), SSL_SHUTDOWN_TIMEOUT);
...
static ssize_t sectransp_send(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static ssize_t sectransp_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
...
const struct Curl_ssl Curl_ssl_sectransp = {
    ...
    SSLSUPP_CAINFO_BLOB | SSLSUPP_CERTINFO |
#ifdef SECTRANSP_PINNEDPUBKEY
