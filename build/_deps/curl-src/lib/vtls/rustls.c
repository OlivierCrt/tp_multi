...
static CURLcode cr_connect(struct Curl_cfilter *cf UNUSED_PARAM,
...
static int read_cb(void *userdata, uint8_t *buf, uintptr_t len,
  ...
  CURLcode result;
  int ret = 0;
  ssize_t nread = Curl_conn_cf_recv(io_ctx->cf->next, io_ctx->data, (char *)buf,
                                    len, &result);
  if (nread < 0) {
    nread = 0;
...
static ssize_t tls_recv_more(struct Curl_cfilter *cf, struct Curl_easy *data,
                             CURLcode *err) {
  ...
  if (rresult != RUSTLS_RESULT_OK) {
    ...
    failf(data, "rustls_connection_process_new_packets: %.*s", errorlen,
          errorbuf);
 ...
 * output buffer.
 */
static ssize_t cr_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  while (plain_bytes_copied < plainlen) {
    ...
    rresult =
        rustls_connection_read(rconn, (uint8_t *)plainbuf + plain_bytes_copied,
                               plainlen - plain_bytes_copied, &n);
...
out:
  CURL_TRC_CF(data, cf, "cf_recv(len=%zu) -> %zd, %d", plainlen, nread, *err);
  return nread;
}
 ...
 */
static ssize_t cr_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (plainlen > 0) {
    rresult = rustls_connection_write(rconn, plainbuf, plainlen, &plainwritten);
    if (rresult != RUSTLS_RESULT_OK) {
      rustls_error(rresult, errorbuf, sizeof(errorbuf), &errorlen);
  ...
  while (rustls_connection_wants_write(rconn)) {
    io_error =
        rustls_connection_write_tls(rconn, write_cb, &io_ctx, &tlswritten);
    if (io_error == EAGAIN || io_error == EWOULDBLOCK) {
...
static CURLcode cr_init_backend(struct Curl_cfilter *cf, struct Curl_easy *data,
                                struct rustls_ssl_backend_data *const backend) {
  ...
  if (!verifypeer) {
    ...
    result = rustls_client_config_builder_load_roots_from_file(config_builder,
                                                               ssl_cafile);
    if (result != RUSTLS_RESULT_OK) {
      failf(data, "rustls: failed to load trusted certificates");
      rustls_client_config_free(
...
static CURLcode cr_connect_nonblocking(struct Curl_cfilter *cf,
  ...
  if (ssl_connection_none == connssl->state) {
    result = cr_init_backend(
        cf, data, (struct rustls_ssl_backend_data *)connssl->backend);
    if (result != CURLE_OK) {
      return result;
    }
...
static int cr_get_select_socks(struct Curl_cfilter *cf, struct Curl_easy *data,
                               curl_socket_t *socks) {
  struct ssl_connect_data *const connssl = cf->ctx;
  curl_socket_t sockfd = Curl_conn_cf_get_socket(cf, data);
  struct rustls_ssl_backend_data *const backend =
...
static void *cr_get_internals(struct ssl_connect_data *connssl,
...
const struct Curl_ssl Curl_ssl_rustls = {
    ...
    SSLSUPP_CAINFO_BLOB | /* supports */
        SSLSUPP_TLS13_CIPHERSUITES | SSLSUPP_HTTPS_PROXY,
