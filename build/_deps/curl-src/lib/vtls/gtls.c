...
static void showtime(struct Curl_easy *data, const char *text, time_t stamp) {
  ...
  msnprintf(str, sizeof(str), "  %s: %s, %02d %s %4d %02d:%02d:%02d GMT", text,
...
static gnutls_datum_t load_file(const char *file) {
  ...
  if (fseek(f, 0, SEEK_END) != 0 || (filelen = ftell(f)) < 0 ||
      fseek(f, 0, SEEK_SET) != 0 || !(ptr = malloc((size_t)filelen)))
    goto out;
...
static CURLcode handshake(struct Curl_cfilter *cf, struct Curl_easy *data,
                          bool duringconnect, bool nonblocking) {
  struct ssl_connect_data *connssl = cf->ctx;
  struct gtls_ssl_backend_data *backend =
      (struct gtls_ssl_backend_data *)connssl->backend;
...
CURLcode gtls_client_init(struct Curl_easy *data,
                          ...
                          const char *hostname, struct gtls_instance *gtls,
                          long *pverifyresult) {
  ...
  *pverifyresult = 0;
  ...
  if (config->CAfile) {
    ...
    rc = gnutls_certificate_set_x509_trust_file(gtls->cred, config->CAfile,
                                                GNUTLS_X509_FMT_PEM);
    if (rc < 0) {
      infof(data, "error reading ca cert file %s (%s)", config->CAfile,
            gnutls_strerror(rc));
  ...
  if (config->CApath) {
    ...
    rc = gnutls_certificate_set_x509_trust_dir(gtls->cred, config->CApath,
                                               GNUTLS_X509_FMT_PEM);
    if (rc < 0) {
      infof(data, "error reading ca cert file %s (%s)", config->CApath,
            gnutls_strerror(rc));
      if (config->verifypeer) {
        *pverifyresult = rc;
        return CURLE_SSL_CACERT_BADFILE;
  ...
  if (config->CRLfile) {
    ...
    rc = gnutls_certificate_set_x509_crl_file(gtls->cred, config->CRLfile,
                                              GNUTLS_X509_FMT_PEM);
    if (rc < 0) {
      failf(data, "error reading crl file %s (%s)", config->CRLfile,
            gnutls_strerror(rc));
  ...
  if (rc != GNUTLS_E_SUCCESS) {
    failf(data, "Error %d setting GnuTLS cipher list starting with %s", rc,
          err);
    return CURLE_SSL_CONNECT_ERROR;
  }
  ...
  if (config->clientcert) {
    if (ssl_config->key_passwd) {
      ...
      rc = gnutls_certificate_set_x509_key_file2(
          gtls->cred, config->clientcert,
          ssl_config->key ? ssl_config->key : config->clientcert,
          do_file_type(ssl_config->cert_type), ssl_config->key_passwd,
          supported_key_encryption_algorithms);
      if (rc != GNUTLS_E_SUCCESS) {
        failf(data, "error reading X.509 potentially-encrypted key file: %s",
              gnutls_strerror(rc));
        return CURLE_SSL_CONNECT_ERROR;
      }
    } else {
  ...
  if (config->verifystatus) {
    rc = gnutls_ocsp_status_request_enable_client(gtls->session, NULL, 0, NULL);
    if (rc != GNUTLS_E_SUCCESS) {
...
static CURLcode gtls_connect_step1(struct Curl_cfilter *cf,
  ...
  result = gtls_client_init(data, conn_config, ssl_config, connssl->hostname,
  ...
  if (conn_config->sessionid) {
    ...
    if (!Curl_ssl_getsessionid(cf, data, &ssl_sessionid, &ssl_idsize)) {
      ...
      gnutls_session_set_data(backend->gtls.session, ssl_sessionid, ssl_idsize);
...
CURLcode Curl_gtls_verifyserver(struct Curl_easy *data,
  ...
  infof(data, "SSL connection using %s / %s", gnutls_protocol_get_name(version),
        ptr);
  ...
  if (!chainp) {
    if (config->verifypeer || config->verifyhost || config->issuercert) {
#ifdef USE_GNUTLS_SRP
...
#define use_addr in_addr
  ...
  if (!rc) {
    if (config->verifyhost) {
      ...
      infof(data, "  common name: %s (does not match '%s')", certname,
            dispname);
  } else
...
static CURLcode gtls_verifyserver(struct Curl_cfilter *cf,
  ...
  result =
      Curl_gtls_verifyserver(data, session, conn_config, ssl_config,
                             connssl->hostname, connssl->dispname, pinned_key);
  if (result)
    goto out;
...
static CURLcode gtls_connect(struct Curl_cfilter *cf, struct Curl_easy *data) {
  CURLcode result;
  bool done = FALSE;
...
static ssize_t gtls_send(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static void gtls_close(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct ssl_connect_data *connssl = cf->ctx;
  struct gtls_ssl_backend_data *backend =
      (struct gtls_ssl_backend_data *)connssl->backend;
 ...
 */
static int gtls_shutdown(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct ssl_connect_data *connssl = cf->ctx;
  struct gtls_ssl_backend_data *backend =
      (struct gtls_ssl_backend_data *)connssl->backend;
  int retval = 0;
  ...
  if (backend->gtls.session) {
    ...
    while (!done) {
      ...
      if (what > 0) {
        ...
        result = gnutls_record_recv(backend->gtls.session, buf, sizeof(buf));
        switch (result) {
        case 0:
...
static ssize_t gtls_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static void gtls_session_free(void *ptr) { free(ptr); }
...
/* data might be NULL! */
static CURLcode gtls_random(struct Curl_easy *data, unsigned char *entropy,
                            size_t length) {
  int rc;
  (void)data;
...
static bool gtls_cert_status_request(void) { return TRUE; }
...
const struct Curl_ssl Curl_ssl_gnutls = {
    ...
    SSLSUPP_CA_PATH | SSLSUPP_CERTINFO | SSLSUPP_PINNEDPUBKEY |
        SSLSUPP_HTTPS_PROXY,
