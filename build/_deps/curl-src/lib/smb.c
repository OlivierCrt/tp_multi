...
static CURLcode smb_disconnect(struct Curl_easy *data, struct connectdata *conn,
                               bool dead);
...
static void conn_state(struct Curl_easy *data, enum smb_conn_state newstate) {
  ...
  static const char *const names[] = {
      "SMB_NOT_CONNECTED", "SMB_CONNECTING", "SMB_NEGOTIATE",
      "SMB_SETUP",         "SMB_CONNECTED",
      /* LAST */
  };

  if (smbc->state != newstate)
    infof(data, "SMB conn %p state change from %s to %s", (void *)smbc,
          names[smbc->state], names[newstate]);
#endif

  smbc->state = newstate;
}

static void request_state(struct Curl_easy *data, enum smb_req_state newstate) {
  struct smb_request *req = data->req.p.smb;
#if defined(DEBUGBUILD) && !defined(CURL_DISABLE_VERBOSE_STRINGS)
  /* For debug purposes */
  static const char *const names[] = {
      "SMB_REQUESTING",      "SMB_TREE_CONNECT", "SMB_OPEN",
      "SMB_DOWNLOAD",        "SMB_UPLOAD",       "SMB_CLOSE",
      "SMB_TREE_DISCONNECT", "SMB_DONE",
      /* LAST */
  };

  if (req->state != newstate)
    infof(data, "SMB request %p state change from %s to %s", (void *)req,
          names[req->state], names[newstate]);
#endif
...
static CURLcode smb_recv_message(struct Curl_easy *data, void **msg) {
  ...
  nbt_size =
      Curl_read16_be((const unsigned char *)(buf + sizeof(unsigned short))) +
      sizeof(unsigned int);
...
static void smb_format_message(struct Curl_easy *data, struct smb_header *h,
  ...
  memset(h, 0, sizeof(*h));
  h->nbt_length =
      htons((unsigned short)(sizeof(*h) - sizeof(unsigned int) + len));
...
static CURLcode smb_send(struct Curl_easy *data, ssize_t len,
  ...
  result =
      Curl_nwrite(data, FIRSTSOCKET, data->state.ulbuf, len, &bytes_written);
  if (result)
    return result;
...
static CURLcode smb_flush(struct Curl_easy *data) {
  ...
  result = Curl_nwrite(data, FIRSTSOCKET, data->state.ulbuf + smbc->sent, len,
                       &bytes_written);
  if (result)
    return result;
...
static CURLcode smb_send_message(struct Curl_easy *data, unsigned char cmd,
  ...
  if (result)
    return result;
  smb_format_message(data, (struct smb_header *)data->state.ulbuf, cmd,
                     msg_len);
  memcpy(data->state.ulbuf + sizeof(struct smb_header), msg, msg_len);
...
static CURLcode smb_parse_url_path(struct Curl_easy *data,
  ...
  /* URL decode the path */
  CURLcode result =
      Curl_urldecode(data->state.up.path, 0, &path, NULL, REJECT_CTRL);
  if (result)
    return result;
