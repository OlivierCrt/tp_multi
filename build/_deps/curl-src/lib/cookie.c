...
static bool cookie_tailmatch(const char *cookie_domain,
                             size_t cookie_domain_len, const char *hostname) {
...
void Curl_cookie_loadfiles(struct Curl_easy *data) {
  ...
  if (list) {
    ...
    while (list) {
      struct CookieInfo *newcookies = Curl_cookie_init(
          data, list->data, data->cookies, data->set.cookiesession);
      if (!newcookies)
        /*
 ...
 */
struct Cookie *
Curl_cookie_add(struct Curl_easy *data, struct CookieInfo *c,
                ...
                bool secure) /* TRUE if connection is over secure origin */
{
  ...
  if (httpheader) {
    ...
    do {
      ...
      if (nlen) {
        ...
        if (done)
        ...
        } else if ((nlen == 6) && strncasecompare("domain", namep, 6) && vlen) {
          bool is_ip;
          ...
          offt = curlx_strtoofft((*maxage == '\"') ? &maxage[1] : &maxage[0],
                                 NULL, 10, &co->expires);
          switch (offt) {
          case CURL_OFFT_FLOW:
    ...
    if (strcasecompare(clist->name, co->name)) {
      ...
      if (matching_domains &&          /* the domains were identical */
        ...
        if (strncasecompare(clist->spath, co->spath, cllen)) {
          infof(data,
                "cookie '%s' for domain '%s' dropped, would "
                "overlay an existing cookie",
                co->name, co->domain);
          freecookie(co);
          return NULL;
        }
  ...
  if (c->running)
    ...
    infof(data,
          ...
          replace_old ? "Replaced" : "Added", co->name, co->value, co->domain,
          co->path, co->expires);
...
static char *get_netscape_format(const struct Cookie *co) {
  return aprintf(
      ...
      co->domain ? co->domain : "unknown", co->tailmatch ? "TRUE" : "FALSE",
      co->path ? co->path : "/", co->secure ? "TRUE" : "FALSE", co->expires,
      co->name, co->value ? co->value : "");
}
 ...
 */
static CURLcode cookie_output(struct Curl_easy *data, struct CookieInfo *c,
                              const char *filename) {
  struct Cookie *co;
  FILE *out = NULL;
