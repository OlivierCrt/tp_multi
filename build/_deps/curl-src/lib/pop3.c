...
static int pop3_getsock(struct Curl_easy *data, struct connectdata *conn,
                        curl_socket_t *socks);
...
static void pop3_state(struct Curl_easy *data, pop3state newstate) {
  ...
  static const char *const names[] = {
      "STOP", "SERVERGREET", "CAPA", "STARTTLS", "UPGRADETLS", "AUTH",
      "APOP", "USER",        "PASS", "COMMAND",  "QUIT",
      /* LAST */
  };

  if (pop3c->state != newstate)
    infof(data, "POP3 %p state change from %s to %s", (void *)pop3c,
          names[pop3c->state], names[newstate]);
#endif
...
static CURLcode pop3_continue_auth(struct Curl_easy *data, const char *mech,
  ...
  return Curl_pp_sendf(data, &pop3c->pp, "%s",
...
static CURLcode pop3_perform_command(struct Curl_easy *data) {
  ...
  if (pop3->id[0] != '\0')
    result = Curl_pp_sendf(
        data, &conn->proto.pop3c.pp, "%s %s",
        (pop3->custom && pop3->custom[0] != '\0' ? pop3->custom : command),
        pop3->id);
  else
    result = Curl_pp_sendf(
        data, &conn->proto.pop3c.pp, "%s",
        (pop3->custom && pop3->custom[0] != '\0' ? pop3->custom : command));
...
static CURLcode pop3_state_auth_resp(struct Curl_easy *data, int pop3code,
                                     pop3state instate) {
  CURLcode result = CURLE_OK;
  struct connectdata *conn = data->conn;
  struct pop3_conn *pop3c = &conn->proto.pop3c;
...
static CURLcode pop3_state_command_resp(struct Curl_easy *data, int pop3code,
                                        pop3state instate) {
  CURLcode result = CURLE_OK;
  struct connectdata *conn = data->conn;
  struct POP3 *pop3 = data->req.p.pop3;
...
static CURLcode pop3_parse_url_options(struct connectdata *conn) {
  ...
  while (!result && ptr && *ptr) {
    ...
    if (strncasecompare(key, "AUTH=", 5)) {
      result =
          Curl_sasl_parse_url_auth_option(&pop3c->sasl, value, ptr - value);
...
CURLcode Curl_pop3_write(struct Curl_easy *data, char *str, size_t nread) {
    ...
    case 0x0d:
      if (pop3c->eob == 0) {
        ...
        if (i) {
          ...
          result =
              Curl_client_write(data, CLIENTWRITE_BODY, &str[last], i - last);
    ...
    if (prev && prev >= pop3c->eob) {
      ...
      if (prev) {
        ...
        if (strip_dot && prev - 1 > 0) {
          ...
          result =
              Curl_client_write(data, CLIENTWRITE_BODY, (char *)POP3_EOB, prev);
        } else {
          result = CURLE_OK;
        }
  ...
  if (nread - last) {
    result =
        Curl_client_write(data, CLIENTWRITE_BODY, &str[last], nread - last);
  }
