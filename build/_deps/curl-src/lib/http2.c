...
static size_t populate_binsettings(uint8_t *binsettings,
  ...
  return nghttp2_pack_settings_payload(binsettings, H2_BINSETTINGS_LEN, iv,
                                       ivlen);
}
...
#define H2_STREAM_CTX(d)                                                       \
  ((struct stream_ctx *)(((d) && (d)->req.p.http)                              \
                             ? ((struct HTTP *)(d)->req.p.http)->h2_ctx        \
                             : NULL))
#define H2_STREAM_LCTX(d) ((struct HTTP *)(d)->req.p.http)->h2_ctx
#define H2_STREAM_ID(d) (H2_STREAM_CTX(d) ? H2_STREAM_CTX(d)->id : -2)

/*
 * Mark this transfer to get "drained".
 */
static void drain_stream(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (data->state.dselect_bits != bits) {
    CURL_TRC_CF(data, cf, "[%d] DRAIN dselect_bits=%x", stream->id, bits);
...
static CURLcode http2_data_setup(struct Curl_cfilter *cf,
  ...
  stream->id = -1;
  Curl_bufq_initp(&stream->sendbuf, &ctx->stream_bufcp, H2_STREAM_SEND_CHUNKS,
                  BUFQ_OPT_NONE);
  Curl_bufq_initp(&stream->recvbuf, &ctx->stream_bufcp, H2_STREAM_RECV_CHUNKS,
                  BUFQ_OPT_SOFT_LIMIT);
...
static void http2_data_done(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (ctx->h2) {
    if (!stream->closed && stream->id > 0) {
      /* RST_STREAM */
      CURL_TRC_CF(data, cf, "[%d] premature DATA_DONE, RST stream", stream->id);
      if (!nghttp2_submit_rst_stream(ctx->h2, NGHTTP2_FLAG_NONE, stream->id,
    ...
    if (nghttp2_session_get_stream_user_data(ctx->h2, stream->id)) {
      int rv = nghttp2_session_set_stream_user_data(ctx->h2, stream->id, 0);
      if (rv) {
...
static ssize_t nw_in_reader(void *reader_ctx, unsigned char *buf, size_t buflen,
                            CURLcode *err) {
...
static ssize_t nw_out_writer(void *writer_ctx, const unsigned char *buf,
                             size_t buflen, CURLcode *err) {
...
static ssize_t send_callback(nghttp2_session *h2, const uint8_t *mem,
                             size_t length, int flags, void *userp);
...
static int on_data_chunk_recv(nghttp2_session *session, uint8_t flags,
                              int32_t stream_id, const uint8_t *mem, size_t len,
                              void *userp);
...
static int on_header(nghttp2_session *session, const nghttp2_frame *frame,
                     ...
                     size_t valuelen, uint8_t flags, void *userp);
 ...
 */
static CURLcode cf_h2_ctx_init(struct Curl_cfilter *cf, struct Curl_easy *data,
                               bool via_h1_upgrade) {
  struct cf_h2_ctx *ctx = cf->ctx;
  struct stream_ctx *stream;
  ...
  if (via_h1_upgrade) {
    ...
    rc = nghttp2_session_set_stream_user_data(ctx->h2, stream->id, data);
    if (rc) {
      infof(data, "http/2: failed to set user_data for stream %u", stream->id);
      DEBUGASSERT(0);
    }
    ...
    ivlen = populate_settings(iv, data);
    rc = nghttp2_submit_settings(ctx->h2, NGHTTP2_FLAG_NONE, iv, ivlen);
    if (rc) {
...
static int h2_process_pending_input(struct Curl_cfilter *cf,
  ...
  while (Curl_bufq_peek(&ctx->inbufq, &buf, &blen)) {
    ...
    if (Curl_bufq_is_empty(&ctx->inbufq)) {
      ...
      CURL_TRC_CF(data, cf,
                  "process_pending_input: %zu bytes left "
  ...
  *input_pending = FALSE;
  ...
  if (*input_pending) {
    ...
    *input_pending = FALSE;
    ...
    if (nread != -1) {
      CURL_TRC_CF(data, cf,
                  "%zd bytes stray data read before trying "
                  "h2 connection",
                  nread);
...
static CURLcode http2_send_ping(struct Curl_cfilter *cf,
  ...
  if (rc) {
    failf(data, "nghttp2_submit_ping() failed: %s(%d)", nghttp2_strerror(rc),
          rc);
    return CURLE_HTTP2;
  }
  ...
  if (rc) {
    failf(data, "nghttp2_session_send() failed: %s(%d)", nghttp2_strerror(rc),
          rc);
...
static CURLcode nw_out_flush(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct cf_h2_ctx *ctx = cf->ctx;
  ssize_t nwritten;
  CURLcode result;
 ...
 */
static ssize_t send_callback(nghttp2_session *h2, const uint8_t *buf,
                             size_t blen, int flags, void *userp) {
  ...
  nwritten = Curl_bufq_write_pass(&ctx->outbufq, buf, blen, nw_out_writer, cf,
                                  &result);
  if (nwritten < 0) {
    if (result == CURLE_AGAIN) {
      ctx->nw_out_blocked = 1;
...
static int push_promise(struct Curl_cfilter *cf, struct Curl_easy *data,
                        const nghttp2_push_promise *frame) {
  struct cf_h2_ctx *ctx = cf->ctx;
  ...
  if (data->multi->push_cb) {
    ...
    rv = data->multi->push_cb(data, newhandle, stream->push_headers_used,
                              &heads, data->multi->push_userp);
    ...
    rv =
        nghttp2_session_set_stream_user_data(ctx->h2, newstream->id, newhandle);
    if (rv) {
      infof(data, "failed to set user_data for stream %u", newstream->id);
      DEBUGASSERT(0);
      rv = CURL_PUSH_DENY;
      goto fail;
    }
...
static CURLcode recvbuf_write_hds(struct Curl_cfilter *cf,
                                  struct Curl_easy *data, const char *buf,
                                  size_t blen) {
  struct stream_ctx *stream = H2_STREAM_CTX(data);
  ssize_t nwritten;
  CURLcode result;
  ...
  (void)cf;
  nwritten = Curl_bufq_write(&stream->recvbuf, (const unsigned char *)buf, blen,
                             &result);
  if (nwritten < 0)
    return result;
...
static CURLcode on_stream_frame(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  case NGHTTP2_DATA:
    ...
    CURL_TRC_CF(
        data, cf, "[%d] DATA, buffered=%zu, window=%d/%d", stream_id, rbuflen,
        nghttp2_session_get_stream_effective_recv_data_length(ctx->h2,
                                                              stream->id),
        nghttp2_session_get_stream_effective_local_window_size(ctx->h2,
                                                               stream->id));
    /* If !body started on this stream, then receiving DATA is illegal. */
    if (!stream->bodystarted) {
      rv = nghttp2_submit_rst_stream(ctx->h2, NGHTTP2_FLAG_NONE, stream_id,
    ...
    if (frame->hd.flags & NGHTTP2_FLAG_END_STREAM) {
      ...
      int32_t wsize =
          nghttp2_session_get_stream_local_window_size(ctx->h2, stream->id);
      if (wsize > 0 && (uint32_t)wsize != stream->local_window_size) {
        ...
        nghttp2_session_set_local_window_size(
            ctx->h2, NGHTTP2_FLAG_NONE, stream->id, stream->local_window_size);
      }
    }
    break;
  ...
  case NGHTTP2_PUSH_PROMISE:
    ...
    if (rv) { /* deny! */
      ...
      if (nghttp2_is_fatal(rv))
        ...
        CURL_TRC_CF(data, cf, "[%d] fail in PUSH_PROMISE received", stream_id);
        return CURLE_RECV_ERROR;
      }
    }
    break;
  ...
  case NGHTTP2_WINDOW_UPDATE:
    if ((data->req.keepon & KEEP_SEND_HOLD) && (data->req.keepon & KEEP_SEND)) {
      data->req.keepon &= ~KEEP_SEND_HOLD;
      drain_stream(cf, data, stream);
      CURL_TRC_CF(data, cf, "[%d] un-holding after win update", stream_id);
    }
    break;
  default:
    break;
...
static int fr_print(const nghttp2_frame *frame, char *buffer, size_t blen) {
  switch (frame->hd.type) {
  case NGHTTP2_DATA: {
    return msnprintf(buffer, blen, "FRAME[DATA, len=%d, eos=%d, padlen=%d]",
                     (int)frame->hd.length,
                     !!(frame->hd.flags & NGHTTP2_FLAG_END_STREAM),
                     (int)frame->data.padlen);
  }
  case NGHTTP2_HEADERS: {
    return msnprintf(buffer, blen, "FRAME[HEADERS, len=%d, hend=%d, eos=%d]",
                     (int)frame->hd.length,
                     !!(frame->hd.flags & NGHTTP2_FLAG_END_HEADERS),
                     !!(frame->hd.flags & NGHTTP2_FLAG_END_STREAM));
  }
  case NGHTTP2_PRIORITY: {
    return msnprintf(buffer, blen, "FRAME[PRIORITY, len=%d, flags=%d]",
                     (int)frame->hd.length, frame->hd.flags);
  }
  case NGHTTP2_RST_STREAM: {
    return msnprintf(
        buffer, blen, "FRAME[RST_STREAM, len=%d, flags=%d, error=%u]",
        (int)frame->hd.length, frame->hd.flags, frame->rst_stream.error_code);
  }
  case NGHTTP2_SETTINGS: {
    if (frame->hd.flags & NGHTTP2_FLAG_ACK) {
      return msnprintf(buffer, blen, "FRAME[SETTINGS, ack=1]");
    }
    return msnprintf(buffer, blen, "FRAME[SETTINGS, len=%d]",
                     (int)frame->hd.length);
  }
  case NGHTTP2_PUSH_PROMISE: {
    return msnprintf(buffer, blen, "FRAME[PUSH_PROMISE, len=%d, hend=%d]",
                     (int)frame->hd.length,
                     !!(frame->hd.flags & NGHTTP2_FLAG_END_HEADERS));
  }
  case NGHTTP2_PING: {
    return msnprintf(buffer, blen, "FRAME[PING, len=%d, ack=%d]",
                     (int)frame->hd.length, frame->hd.flags & NGHTTP2_FLAG_ACK);
  }
  case NGHTTP2_GOAWAY: {
    char scratch[128];
  ...
  case NGHTTP2_WINDOW_UPDATE: {
    return msnprintf(buffer, blen, "FRAME[WINDOW_UPDATE, incr=%d]",
                     frame->window_update.window_size_increment);
  }
  default:
    return msnprintf(buffer, blen, "FRAME[%d, len=%d, flags=%d]",
                     frame->hd.type, (int)frame->hd.length, frame->hd.flags);
  }
}
    ...
    case NGHTTP2_SETTINGS: {
      if (!(frame->hd.flags & NGHTTP2_FLAG_ACK)) {
        ...
        if ((data->req.keepon & KEEP_SEND_HOLD) &&
          ...
          if (stream) {
            drain_stream(cf, data, stream);
            CURL_TRC_CF(data, cf, "[%d] un-holding after SETTINGS", stream_id);
          }
        }
      }
      break;
...
static int on_stream_close(nghttp2_session *session, int32_t stream_id,
  ...
  data_s = stream_id ? nghttp2_session_get_stream_user_data(session, stream_id)
                     : NULL;
  if (!data_s) {
    return 0;
  }
  ...
  if (stream->error)
    CURL_TRC_CF(data_s, cf, "[%d] RESET: %s (err %d)", stream_id,
  ...
  if (rv) {
    infof(data_s, "http/2: failed to clear user_data for stream %u", stream_id);
    DEBUGASSERT(0);
  }
  return 0;
}
...
static int on_header(nghttp2_session *session, const nghttp2_frame *frame,
  ...
  if (frame->hd.type == NGHTTP2_PUSH_PROMISE) {
    ...
    if (!strcmp(HTTP_PSEUDO_AUTHORITY, (const char *)name)) {
      ...
      int rc = 0;
      char *check =
          aprintf("%s:%d", cf->conn->host.name, cf->conn->remote_port);
      if (!check)
        /* no memory */
        return NGHTTP2_ERR_CALLBACK_FAILURE;
    ...
    if (!stream->push_headers) {
      ...
      stream->push_headers =
          malloc(stream->push_headers_alloc * sizeof(char *));
      if (!stream->push_headers)
...
static ssize_t req_body_read_callback(nghttp2_session *session,
  ...
  CURL_TRC_CF(data_s, cf,
              "[%d] req_body_read(len=%zu) left=%" CURL_FORMAT_CURL_OFF_T
              " -> %zd, %d",
...
CURLcode Curl_http2_request_upgrade(struct dynbuf *req,
  ...
  result =
      Curl_base64url_encode((const char *)binsettings, binlen, &base64, &blen);
  if (result) {
    Curl_dyn_free(req);
    return result;
  }
...
static ssize_t http2_handle_stream_close(struct Curl_cfilter *cf,
  ...
  if (stream->error == NGHTTP2_REFUSED_STREAM) {
    CURL_TRC_CF(data, cf,
                "[%d] REFUSED_STREAM, try again on a new "
                "connection",
                stream->id);
    ...
    *err = CURLE_RECV_ERROR; /* trigger Curl_retry_request() later */
    ...
    failf(data, "HTTP/2 stream %u was not closed cleanly: %s (err %u)",
          stream->id, nghttp2_http2_strerror(stream->error), stream->error);
  ...
  if (!stream->bodystarted) {
    failf(data,
          "HTTP/2 stream %u was closed cleanly, but before getting "
    ...
    *err = CURLE_OK;
    ...
    for (i = 0; i < Curl_dynhds_count(&stream->resp_trailers); ++i) {
      ...
      if (!e)
        break;
      Curl_dyn_reset(&dbuf);
      *err = Curl_dyn_addf(&dbuf, "%.*s: %.*s\x0d\x0a", (int)e->namelen,
                           e->name, (int)e->valuelen, e->value);
      if (*err)
        break;
...
static int sweight_wanted(const struct Curl_easy *data) {
  ...
  return data->set.priority.weight ? data->set.priority.weight
                                   : NGHTTP2_DEFAULT_WEIGHT;
}

static int sweight_in_effect(const struct Curl_easy *data) {
  /* 0 weight is not set by user and we take the nghttp2 default one */
  return data->state.priority.weight ? data->state.priority.weight
                                     : NGHTTP2_DEFAULT_WEIGHT;
}
...
static void h2_pri_spec(struct Curl_easy *data,
  ...
  nghttp2_priority_spec_init(pri_spec, depstream_id, sweight_wanted(data),
...
static CURLcode h2_progress_egress(struct Curl_cfilter *cf,
  ...
  if (stream && stream->id > 0 &&
    ...
    rv = nghttp2_submit_priority(ctx->h2, NGHTTP2_FLAG_NONE, stream->id,
                                 &pri_spec);
    if (rv)
      goto out;
  }
...
static ssize_t stream_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
                           struct stream_ctx *stream, char *buf, size_t len,
                           CURLcode *err) {
  struct cf_h2_ctx *ctx = cf->ctx;
  ssize_t nread = -1;
  ...
  *err = CURLE_AGAIN;
  if (!Curl_bufq_is_empty(&stream->recvbuf)) {
    nread = Curl_bufq_read(&stream->recvbuf, (unsigned char *)buf, len, err);
    if (nread < 0)
      goto out;
    DEBUGASSERT(nread > 0);
  }
...
out:
  if (nread < 0 && *err != CURLE_AGAIN)
    CURL_TRC_CF(data, cf, "[%d] stream_recv(len=%zu) -> %zd, %d", stream->id,
                len, nread, *err);
  return nread;
}
...
static ssize_t cf_h2_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (!stream) {
     ...
     * is for such a case. */
    failf(data,
          "[%zd-%zd], http/2 recv on a transfer never opened "
          "or already cleared",
          (ssize_t)data->id, (ssize_t)cf->conn->connection_id);
    *err = CURLE_HTTP2;
    return -1;
  }
...
out:
  ...
  CURL_TRC_CF(data, cf,
              "[%d] cf_recv(len=%zu) -> %zd %d, "
              "buffered=%zu, window=%d/%d, connection %d/%d",
              stream->id, len, nread, *err, Curl_bufq_len(&stream->recvbuf),
              nghttp2_session_get_stream_effective_recv_data_length(ctx->h2,
                                                                    stream->id),
...
static ssize_t h2_submit(struct stream_ctx **pstream, struct Curl_cfilter *cf,
  ...
  *err = Curl_http_req_to_h2(&h2_headers, stream->h1.req, data);
  ...
  default:
    ...
    stream_id =
        nghttp2_submit_request(ctx->h2, &pri_spec, nva, nheader, NULL, data);
  }
...
#define MAX_ACC 60000 /* <64KB to account for some overhead */
  if (Curl_trc_is_verbose(data)) {
    ...
    infof(data, "[HTTP/2] [%d] OPENED stream for %s", stream_id,
          data->state.url);
    for (i = 0; i < nheader; ++i) {
      acc += nva[i].namelen + nva[i].valuelen;

      infof(data, "[HTTP/2] [%d] [%.*s: %.*s]", stream_id, (int)nva[i].namelen,
            nva[i].name, (int)nva[i].valuelen, nva[i].value);
    }

    if (acc > MAX_ACC) {
      infof(data,
            "[HTTP/2] Warning: The cumulative length of all "
...
out:
  CURL_TRC_CF(data, cf, "[%d] submit -> %zd, %d", stream ? stream->id : -1,
              nwritten, *err);
  Curl_safefree(nva);
  *pstream = stream;
...
static ssize_t cf_h2_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (stream && stream->id != -1) {
    if (stream->upload_blocked_len) {
      ...
      if (len < stream->upload_blocked_len) {
        ...
        failf(data, "HTTP/2 send again with decreased length (%zd vs %zd)", len,
              stream->upload_blocked_len);
        *err = CURLE_HTTP2;
        nwritten = -1;
        goto out;
      }
  ...
  if (stream && blocked && nwritten > 0) {
    ...
    CURL_TRC_CF(data, cf,
                "[%d] cf_send(len=%zu) BLOCK: win %u/%zu "
                "blocked_len=%zu",
                stream->id, len,
...
out:
  if (stream) {
    CURL_TRC_CF(
        data, cf,
        "[%d] cf_send(len=%zu) -> %zd, %d, "
        "upload_left=%" CURL_FORMAT_CURL_OFF_T ", "
        "h2 windows %d-%d (stream-conn), "
        "buffers %zu-%zu (stream-conn)",
        stream->id, len, nwritten, *err, stream->upload_left,
        nghttp2_session_get_stream_remote_window_size(ctx->h2, stream->id),
...
static int cf_h2_get_select_socks(struct Curl_cfilter *cf,
                                  struct Curl_easy *data, curl_socket_t *sock) {
  struct cf_h2_ctx *ctx = cf->ctx;
  struct SingleRequest *k = &data->req;
  struct stream_ctx *stream = H2_STREAM_CTX(data);
...
static CURLcode cf_h2_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static CURLcode http2_data_pause(struct Curl_cfilter *cf,
                                 struct Curl_easy *data, bool pause) {
  ...
  if (ctx && ctx->h2 && stream) {
    ...
    int rv = nghttp2_session_set_local_window_size(ctx->h2, NGHTTP2_FLAG_NONE,
                                                   stream->id, window);
    if (rv) {
      failf(data, "nghttp2_session_set_local_window_size() failed: %s(%d)",
            nghttp2_strerror(rv), rv);
    ...
    DEBUGF(infof(data, "Set HTTP/2 window size to %u for stream %u", window,
                 stream->id));
...
#ifdef DEBUGBUILD
    {
      ...
      uint32_t window2 =
          nghttp2_session_get_stream_local_window_size(ctx->h2, stream->id);
      DEBUGF(infof(data, "HTTP/2 window size is now %u for stream %u", window2,
                   stream->id));
    }
#endif
  }
...
static CURLcode cf_h2_cntrl(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static bool cf_h2_is_alive(struct Curl_cfilter *cf, struct Curl_easy *data,
                           bool *input_pending) {
  struct cf_h2_ctx *ctx = cf->ctx;
  CURLcode result;
  struct cf_call_data save;
  ...
  CURL_TRC_CF(data, cf, "conn alive -> %d, input_pending=%d", result,
              *input_pending);
  CF_DATA_RESTORE(cf, save);
  return result;
}
...
static CURLcode cf_h2_query(struct Curl_cfilter *cf, struct Curl_easy *data,
...
bool Curl_http2_may_switch(struct Curl_easy *data, struct connectdata *conn,
                           int sockindex) {
  (void)sockindex;
...
CURLcode Curl_http2_switch(struct Curl_easy *data, struct connectdata *conn,
                           int sockindex) {
...
CURLcode Curl_http2_upgrade(struct Curl_easy *data, struct connectdata *conn,
                            int sockindex, const char *mem, size_t nread) {
  struct Curl_cfilter *cf;
  struct cf_h2_ctx *ctx;
  CURLcode result;
  ...
  if (nread > 0) {
    ...
    copied = Curl_bufq_write(&ctx->inbufq, (const unsigned char *)mem, nread,
                             &result);
    if (copied < 0) {
    ...
    if ((size_t)copied < nread) {
      failf(data,
            "connection buffer size could not take all data "
            "from HTTP Upgrade response header: copied=%zd, datalen=%zu",
            copied, nread);
      return CURLE_HTTP2;
    }
    infof(data,
          "Copied HTTP/2 data in stream buffer to connection buffer"
