 ...
 */
static int check_sspi_err(struct Curl_easy *data, SECURITY_STATUS status,
                          const char *function) {
...
CURLcode Curl_SOCKS5_gssapi_negotiate(struct Curl_cfilter *cf,
  ...
  if (strchr(service, '/')) {
    ...
    service_name =
        malloc(service_length + strlen(conn->socks_proxy.host.name) + 2);
    if (!service_name)
      return CURLE_OUT_OF_MEMORY;
    msnprintf(service_name,
              service_length + strlen(conn->socks_proxy.host.name) + 2, "%s/%s",
  ...
  status = s_pSecFn->AcquireCredentialsHandle(
      NULL, (TCHAR *)TEXT("Kerberos"), SECPKG_CRED_OUTBOUND, NULL, NULL, NULL,
      NULL, &cred_handle, &expiry);
  ...
  for (;;) {
    ...
    status = s_pSecFn->InitializeSecurityContext(
        &cred_handle, context_handle, sname,
        ISC_REQ_MUTUAL_AUTH | ISC_REQ_ALLOCATE_MEMORY |
            ISC_REQ_CONFIDENTIALITY | ISC_REQ_REPLAY_DETECT,
        0, SECURITY_NATIVE_DREP, &input_desc, 0, &sspi_context, &output_desc,
        &sspi_ret_flags, &expiry);
    ...
    if (sspi_send_token.cbBuffer) {
      ...
      written =
          Curl_conn_cf_send(cf->next, data, (char *)sspi_send_token.pvBuffer,
  ...
  if (data->set.socks5_gssapi_nec) {
    ...
    status = s_pSecFn->QueryContextAttributes(&sspi_context, SECPKG_ATTR_SIZES,
                                              &sspi_sizes);
