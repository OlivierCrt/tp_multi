...
static CURLcode cf_haproxy_date_out_set(struct Curl_cfilter *cf,
  ...
  if (cf->conn->unix_domain_socket)
    ...
    result =
        Curl_dyn_addf(&ctx->data_out, "PROXY %s %s %s %i %i\r\n", tcp_version,
                      client_ip, data->info.conn_primary_ip,
                      data->info.conn_local_port, data->info.conn_primary_port);
...
static CURLcode cf_haproxy_connect(struct Curl_cfilter *cf,
  ...
  case HAPROXY_SEND:
    ...
    if (len > 0) {
      ssize_t written = Curl_conn_send(
          data, cf->sockindex, Curl_dyn_ptr(&ctx->data_out), len, &result);
      if (written < 0)
        goto out;
...
static void cf_haproxy_close(struct Curl_cfilter *cf, struct Curl_easy *data) {
