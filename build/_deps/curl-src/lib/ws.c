...
static void ws_dec_info(struct ws_decoder *dec, struct Curl_easy *data,
  ...
  case 0:
    break;
  case 1:
    infof(data, "WS-DEC: %s [%s%s]", msg, ws_frame_name_of_op(dec->head[0]),
  ...
  default:
    if (dec->head_len < dec->head_total) {
      infof(data, "WS-DEC: %s [%s%s](%d/%d)", msg,
            ...
            (dec->head[0] & WSBIT_FIN) ? "" : " NON-FINAL", dec->head_len,
            dec->head_total);
    } else {
      infof(data,
            "WS-DEC: %s [%s%s payload=%" CURL_FORMAT_CURL_OFF_T
            "/%" CURL_FORMAT_CURL_OFF_T "]",
            msg, ws_frame_name_of_op(dec->head[0]),
            (dec->head[0] & WSBIT_FIN) ? "" : " NON-FINAL", dec->payload_offset,
            dec->payload_len);
    }
    break;
  }
...
typedef ssize_t ws_write_payload(const unsigned char *buf, size_t buflen,
                                 ...
                                 curl_off_t payload_len, void *userp,
                                 CURLcode *err);
...
static void ws_dec_init(struct ws_decoder *dec) { ws_dec_reset(dec); }

static CURLcode ws_dec_read_head(struct ws_decoder *dec, struct Curl_easy *data,
                                 struct bufq *inraw) {
  ...
  while (Curl_bufq_peek(inraw, &inbuf, &inlen)) {
    ...
    case 10:
      dec->payload_len =
          ((curl_off_t)dec->head[2] << 56) | (curl_off_t)dec->head[3] << 48 |
          (curl_off_t)dec->head[4] << 40 | (curl_off_t)dec->head[5] << 32 |
          (curl_off_t)dec->head[6] << 24 | (curl_off_t)dec->head[7] << 16 |
          (curl_off_t)dec->head[8] << 8 | dec->head[9];
      break;
    default:
...
static CURLcode ws_dec_pass_payload(struct ws_decoder *dec,
                                    struct Curl_easy *data, struct bufq *inraw,
  ...
  while (remain && Curl_bufq_peek(inraw, &inbuf, &inlen)) {
    ...
    nwritten = write_payload(inbuf, inlen, dec->frame_age, dec->frame_flags,
                             dec->payload_offset, dec->payload_len, write_ctx,
                             &result);
    if (nwritten < 0)
      return result;
...
static CURLcode ws_dec_pass(struct ws_decoder *dec, struct Curl_easy *data,
                            struct bufq *inraw, ws_write_payload *write_payload,
                            void *write_ctx) {
  CURLcode result;
  ...
  case WS_DEC_HEAD:
    ...
    if (dec->payload_len == 0) {
      ...
      nwritten = write_payload(&tmp, 0, dec->frame_age, dec->frame_flags, 0, 0,
                               write_ctx, &result);
      if (nwritten < 0)
        return result;
      dec->state = WS_DEC_INIT;
...
static void update_meta(struct websocket *ws, int frame_age, int frame_flags,
                        curl_off_t payload_offset, curl_off_t payload_len,
                        size_t cur_len) {
...
static void ws_enc_info(struct ws_encoder *enc, struct Curl_easy *data,
                        const char *msg) {
  infof(data,
        "WS-ENC: %s [%s%s%s payload=%" CURL_FORMAT_CURL_OFF_T
        "/%" CURL_FORMAT_CURL_OFF_T "]",
        msg, ws_frame_name_of_op(enc->firstbyte),
        (enc->firstbyte & WSBIT_OPCODE_MASK) == WSBIT_OPCODE_CONT ? " CONT"
                                                                  : "",
...
static void ws_enc_init(struct ws_encoder *enc) { ws_enc_reset(enc); }
...
static ssize_t ws_enc_write_head(struct Curl_easy *data, struct ws_encoder *enc,
                                 unsigned int flags, curl_off_t payload_len,
  ...
  if (enc->payload_remain > 0) {
    ...
    failf(data,
          "WS: starting new frame with %zd bytes from last one"
          "remaining to be sent",
          (ssize_t)enc->payload_remain);
...
#ifdef USE_HYPER
      char field[128];
      msnprintf(field, sizeof(field), "%s %s", heads[i].name, heads[i].val);
 ...
 * 'mem'.
 */
CURLcode Curl_ws_accept(struct Curl_easy *data, const char *mem, size_t nread) {
  struct SingleRequest *k = &data->req;
  struct websocket *ws;
  CURLcode result;
  ...
  */
  ...
  result =
      Curl_rand(data, (unsigned char *)&ws->enc.mask, sizeof(ws->enc.mask));
  if (result)
    return result;
  ...
  if (data->set.connect_only) {
     ...
     * marked as DONE. */
    nwritten = Curl_bufq_write(&ws->recvbuf, (const unsigned char *)mem, nread,
                               &result);
    if (nwritten < 0)
      return result;
...
static ssize_t ws_client_write(const unsigned char *buf, size_t buflen,
  ...
  if ((frame_flags & CURLWS_PING) && !remain) {
    ...
    *err = curl_ws_send(data, buf, buflen, &bytes, 0, CURLWS_PONG);
    ...
    wrote = data->set.fwrite_func((char *)buf, 1, buflen, data->set.out);
...
*/
size_t Curl_ws_writecb(char *buffer, size_t size /* 1 */, size_t nitems,
                       void *userp) {
  struct Curl_easy *data = userp;
  ...
  if (data->set.ws_raw_mode)
    ...
    while (!Curl_bufq_is_empty(&ws->recvbuf)) {
      ...
      result = ws_dec_pass(&ws->dec, data, &ws->recvbuf, ws_client_write, data);
      if (result == CURLE_AGAIN)
...
static ssize_t nw_in_recv(void *reader_ctx, unsigned char *buf, size_t buflen,
                          CURLcode *err) {
...
CURL_EXTERN CURLcode curl_ws_recv(struct Curl_easy *data, void *buffer,
  ...
  *metap = NULL;
  ...
  while (!done) {
    ...
    result = ws_dec_pass(&ws->dec, data, &ws->recvbuf, ws_client_collect, &ctx);
    if (result == CURLE_AGAIN) {
      if (!ctx.written) {
...
CURL_EXTERN CURLcode curl_ws_send(CURL *data, const void *buffer, size_t buflen,
  ...
  *sent = 0;
  ...
  if (data->set.ws_raw_mode) {
    ...
    if (Curl_is_in_callback(data)) {
      result =
          Curl_write(data, data->conn->writesockfd, buffer, buflen, &nwritten);
    } else
      result = Curl_senddata(data, buffer, buflen, &nwritten);

    infof(data, "WS: wanted to send %zu bytes, sent %zu bytes", buflen,
          nwritten);
  ...
  DEBUGF(infof(data, "curl_ws_send(len=%zu), sendbuf len=%zu space %zu", buflen,
               Curl_bufq_len(&ws->sendbuf), space));
  if (space < 14)
    return CURLE_AGAIN;
  ...
  if (flags & CURLWS_OFFSET) {
    if (fragsize) {
      ...
      n = ws_enc_write_head(data, &ws->enc, flags, fragsize, &ws->sendbuf,
                            &result);
      if (n < 0)
        return result;
    } else {
      if ((curl_off_t)buflen > ws->enc.payload_remain) {
        infof(data,
              "WS: unaligned frame size (sending %zu instead of "
              "%" CURL_FORMAT_CURL_OFF_T ")",
  ...
  n = ws_enc_write_payload(&ws->enc, data, buffer, buflen, &ws->sendbuf,
                           &result);
  if (n < 0)
    return result;
...
void Curl_ws_done(struct Curl_easy *data) { (void)data; }

CURLcode Curl_ws_disconnect(struct Curl_easy *data, struct connectdata *conn,
...
CURL_EXTERN CURLcode curl_ws_send(CURL *curl, const void *buffer, size_t buflen,
                                  size_t *sent, curl_off_t fragsize,
                                  unsigned int flags) {
  (void)curl;
  (void)buffer;
