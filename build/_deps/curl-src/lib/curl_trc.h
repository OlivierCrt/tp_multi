...
void Curl_debug(struct Curl_easy *data, curl_infotype type, char *ptr,
                size_t size);
...
#if defined(__GNUC__) && !defined(printf) && defined(__STDC_VERSION__) &&      \
    (__STDC_VERSION__ >= 199901L) && !defined(__MINGW32__)
...
#define Curl_trc_cf_is_verbose(cf, data)                                       \
  ((data) && (data)->set.verbose && (cf) &&                                    \
   (cf)->cft->log_level >= CURL_LOG_LVL_INFO)
 ...
 */
#if defined(HAVE_VARIADIC_MACROS_C99) && defined(__STDC_VERSION__) &&          \
