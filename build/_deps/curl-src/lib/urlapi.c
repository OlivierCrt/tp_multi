 ...
 * will fail.
 */
static CURLUcode urlencode_str(struct dynbuf *o, const char *url, size_t len,
                               bool relative, bool query) {
...
static CURLUcode junkscan(const char *url, size_t *urllen, unsigned int flags) {
  static const char badbytes[] = {
      ...
      0x1c,       0x1d, 0x1e, 0x1f, 0x7f, 0x00 /* null-terminate */
  };
  size_t n = strlen(url);
  size_t nfine;
 ...
 *
 */
static CURLUcode parse_hostname_login(struct Curl_URL *u, const char *login,
                                      ...
                                      size_t *offset) /* to the host name */
{
  ...
  *offset = 0;
  ...
  ccode = Curl_parse_login_details(
      login, ptr - login - 1, &userp, &passwdp,
...
static int ipv4_normalize(struct dynbuf *host) {
  ...
  case 0: /* a -- 32 bits */
    ...
    result = Curl_dyn_addf(host, "%u.%u.%u.%u", parts[0] >> 24,
                           (parts[0] >> 16) & 0xff, (parts[0] >> 8) & 0xff,
                           parts[0] & 0xff);
    break;
  case 1: /* a.b -- 8.24 bits */
    ...
    Curl_dyn_reset(host);
    result =
        Curl_dyn_addf(host, "%u.%u.%u.%u", parts[0], (parts[1] >> 16) & 0xff,
  ...
  case 2: /* a.b.c -- 8.8.16 bits */
    if ((parts[0] > 0xff) || (parts[1] > 0xff) || (parts[2] > 0xffff))
      return HOST_NAME;
    Curl_dyn_reset(host);
    result = Curl_dyn_addf(host, "%u.%u.%u.%u", parts[0], parts[1],
  ...
  case 3: /* a.b.c.d -- 8.8.8.8 bits */
    if ((parts[0] > 0xff) || (parts[1] > 0xff) || (parts[2] > 0xff) ||
      ...
      return HOST_NAME;
    Curl_dyn_reset(host);
    result = Curl_dyn_addf(host, "%u.%u.%u.%u", parts[0], parts[1], parts[2],
                           parts[3]);
    break;
  }
  if (result)
...
static CURLUcode urldecode_host(struct dynbuf *host) {
  ...
  if (!per)
    ...
    size_t dlen;
    char *decoded;
    CURLcode result = Curl_urldecode(hostname, 0, &decoded, &dlen, REJECT_CTRL);
    if (result)
      return CURLUE_BAD_HOSTNAME;
    Curl_dyn_reset(host);
...
static CURLUcode parse_authority(struct Curl_URL *u, const char *auth,
                                 size_t authlen, unsigned int flags,
                                 struct dynbuf *host, bool has_scheme) {
  size_t offset;
  CURLUcode result;
...
CURLUcode Curl_url_set_authority(CURLU *u, const char *authority,
  ...
  result = parse_authority(u, authority, strlen(authority), flags, &host,
                           !!u->scheme);
  if (result)
    Curl_dyn_free(&host);
  else {
...
static CURLUcode parseurl(const char *url, CURLU *u, unsigned int flags) {
  ...
  if (query) {
    size_t qlen =
        fragment ? (size_t)(fragment - query) : pathlen - (query - path);
    pathlen -= qlen;
    if (qlen > 1) {
      if (flags & CURLU_URLENCODE) {
        struct dynbuf enc;
...
/*
 */
CURLU *curl_url(void) { return calloc(sizeof(struct Curl_URL), 1); }
...
CURLUcode curl_url_get(const CURLU *u, CURLUPart what, char **part,
                       unsigned int flags) {
  const char *ptr;
  ...
  *part = NULL;
  ...
  case CURLUPART_URL: {
    ...
    if (u->scheme && strcasecompare("file", u->scheme)) {
      url = aprintf("file://%s%s%s", u->path, u->fragment ? "#" : "",
      ...
      if (u->host[0] == '[') {
        ...
        if (!Curl_is_ASCII_name(u->host)) {
          ...
          if (result)
            return (result == CURLE_OUT_OF_MEMORY) ? CURLUE_OUT_OF_MEMORY
                                                   : CURLUE_BAD_HOSTNAME;
#endif
        }
      } else if (depunyfy) {
      ...
      url = aprintf(
          "%s://%s%s%s%s%s%s%s%s%s%s%s%s%s%s", scheme, u->user ? u->user : "",
          u->password ? ":" : "", u->password ? u->password : "",
          options ? ";" : "", options ? options : "",
          (u->user || u->password || options) ? "@" : "",
          allochost ? allochost : u->host, port ? ":" : "", port ? port : "",
          u->path ? u->path : "/", (u->query && u->query[0]) ? "?" : "",
          (u->query && u->query[0]) ? u->query : "", u->fragment ? "#" : "",
          u->fragment ? u->fragment : "");
      free(allochost);
    }
    if (!url)
  ...
  if (ptr) {
    ...
    *part = Curl_memdup(ptr, partlen + 1);
    ...
    if (urlencode) {
      struct dynbuf enc;
      Curl_dyn_init(&enc, CURL_MAX_INPUT_LENGTH);
      if (urlencode_str(&enc, *part, partlen, TRUE, what == CURLUPART_QUERY))
        return CURLUE_OUT_OF_MEMORY;
      free(*part);
...
CURLUcode curl_url_set(CURLU *u, CURLUPart what, const char *part,
                       unsigned int flags) {
  char **storep = NULL;
  long port = 0;
  bool urlencode = (flags & CURLU_URLENCODE) ? 1 : 0;
    ...
    if (urlencode) {
      ...
      for (i = (const unsigned char *)part; *i; i++) {
        ...
        if ((*i == ' ') && plusencode) {
          ...
          if (result)
            return CURLUE_OUT_OF_MEMORY;
        } else if (ISUNRESERVED(*i) || ((*i == '/') && urlskipslash) ||
                   ((*i == '=') && equalsencode)) {
          if ((*i == '=') && equalsencode)
