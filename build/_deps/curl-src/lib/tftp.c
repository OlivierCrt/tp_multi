...
static CURLcode tftp_done(struct Curl_easy *data, CURLcode, bool premature);
...
static CURLcode tftp_parse_option_ack(struct tftp_state_data *state,
  ...
  while (tmp < ptr + len) {
    ...
    if (checkprefix(TFTP_OPTION_BLKSIZE, option)) {
      ...
      infof(data, "%s (%d) %s (%d)", "blksize parsed from OACK", state->blksize,
            "requested", state->requested_blksize);
...
static CURLcode tftp_send_first(struct tftp_state_data *state,
  ...
  case TFTP_EVENT_TIMEOUT: /* Resend the first packet out */
    ...
    /* As RFC3617 describes the separator slash is not actually part of the
       ...
       string. */
    result = Curl_urldecode(&state->data->state.up.path[1], 0, &filename, NULL,
                            REJECT_ZERO);
    if (result)
      return result;
    ...
    msnprintf((char *)state->spacket.data + 2, state->blksize, "%s%c%s%c",
              filename, '\0', mode, '\0');
    ...
    if (!data->set.tftp_no_options) {
      ...
      result =
          tftp_option_add(state, &sbytes, (char *)state->spacket.data + sbytes,
                          TFTP_OPTION_TSIZE);
      if (result == CURLE_OK)
    ...
    senddata = sendto(
        state->sockfd, (void *)state->spacket.data, (SEND_TYPE_ARG3)sbytes, 0,
        &data->conn->remote_addr->sa_addr, data->conn->remote_addr->addrlen);
...
static CURLcode tftp_rx(struct tftp_state_data *state, tftp_event_t event) {
  ...
  case TFTP_EVENT_DATA:
    ...
    sbytes =
        sendto(state->sockfd, (void *)state->spacket.data, 4, SEND_4TH_ARG,
  ...
  case TFTP_EVENT_TIMEOUT:
    ...
    state->retries++;
    infof(data, "Timeout waiting for block %d ACK.  Retries = %d",
  ...
  case TFTP_EVENT_OACK:
    if (event == TFTP_EVENT_ACK) {
      ...
      if (rblock != state->block &&
        ...
        infof(data, "Received ACK for block %d, expecting %d", rblock,
              state->block);
        state->retries++;
    ...
    sbytes =
        sendto(state->sockfd, (void *)state->spacket.data, 4 + state->sbytes,
               SEND_4TH_ARG, (struct sockaddr *)&state->remote_addr,
               state->remote_addrlen);
    /* Check all sbytes were sent */
    if (sbytes < 0) {
      failf(data, "%s", Curl_strerror(SOCKERRNO, buffer, sizeof(buffer)));
...
static CURLcode tftp_receive_packet(struct Curl_easy *data) {
  ...
  state->rbytes = (int)recvfrom(state->sockfd, (void *)state->rpacket.data,
                                state->blksize + 4, 0,
    ...
    case TFTP_EVENT_DATA:
      ...
      if (state->rbytes > 4 &&
        ...
        if (!result) {
          ...
          result = Curl_pgrsSetDownloadCounter(data, (curl_off_t)k->bytecount);
        }
        if (result) {
...
static timediff_t tftp_state_timeout(struct Curl_easy *data,
  ...
  timeout_ms =
      Curl_timeleft(state->data, NULL, (state->state == TFTP_STATE_START));
  if (timeout_ms < 0) {
    state->error = TFTP_ERR_TIMEOUT;
    state->state = TFTP_STATE_FIN;
    return 0;
