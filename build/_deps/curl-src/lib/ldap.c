...
static int ldap_win_bind(struct Curl_easy *data, LDAP *server, const char *user,
                         const char *passwd) {
  ...
  if (ldap_ssl) {
    ...
    if (conn->ssl_config.verifypeer) {
      ...
      infof(data, "LDAP local: using %s CA cert '%s'",
            (cert_type == LDAPSSL_CERT_FILETYPE_DER ? "DER" : "PEM"), ldap_ca);
    ...
    if (!server) {
      failf(data, "LDAP local: Cannot connect to %s:%u", conn->host.dispname,
            conn->port);
...
*/
  ...
  rc = ldap_search_s(server, ludp->lud_dn, ludp->lud_scope, ludp->lud_filter,
                     ludp->lud_attrs, 0, &ldapmsg);
...
static int _ldap_url_parse2(struct Curl_easy *data,
  ...
  if (!data || !data->state.up.path || data->state.up.path[0] != '/' ||
  ...
  if (*p) {
    ...
    for (i = 0; i < count; i++) {
      ...
      result = Curl_urldecode(attributes[i], 0, &unescaped, NULL, REJECT_ZERO);
      if (result) {
        free(attributes);
