 ...
 */
static size_t convert_lineends(struct Curl_easy *data, char *startPtr,
                               size_t size) {
  char *inPtr, *outPtr;
 ...
 */
CURLcode Curl_nwrite(struct Curl_easy *data, int sockindex, const void *buf,
                     size_t blen, ssize_t *pnwritten) {
  ssize_t nwritten;
 ...
 */
CURLcode Curl_write(struct Curl_easy *data, curl_socket_t sockfd,
...
static CURLcode pausewrite(struct Curl_easy *data,
                           ...
                           bool paused_body, const char *ptr, size_t len) {
 ...
 * callbacks.
 */
static CURLcode chop_write(struct Curl_easy *data, int type,
                           bool skip_body_write, char *optr, size_t olen) {
  ...
  if ((type & (CLIENTWRITE_HEADER | CLIENTWRITE_INFO)) &&
     ...
     */
    writeheader = data->set.fwrite_header ? data->set.fwrite_header
  ...
  if ((conn->handler->protocol & PROTO_FAMILY_HTTP) &&
    ...
    unsigned char htype =
        (unsigned char)(type & CLIENTWRITE_CONNECT
                            ? CURLH_CONNECT
                            : (type & CLIENTWRITE_1XX
                                   ? CURLH_1XX
                                   : (type & CLIENTWRITE_TRAILER
                                          ? CURLH_TRAILER
                                          : CURLH_HEADER)));
  ...
  if (data->state.tempcount) {
    ...
    for (i = 0; i < count; i++) {
      /* even if one function returns error, this loops through and frees
         all buffers */
      if (!result)
        result = chop_write(data, writebuf[i].type, !writebuf[i].paused_body,
...
static CURLcode client_cew_write(struct Curl_easy *data,
                                 struct contenc_writer *writer, const char *buf,
                                 size_t nbytes) {
  (void)writer;
...
static CURLcode init_writer_stack(struct Curl_easy *data) {
  ...
  return Curl_client_create_writer(&data->req.writer_stack, data, &client_cew,
                                   0);
}
