...
WINBASEAPI int WINAPI IdnToAscii(DWORD dwFlags, const WCHAR *lpUnicodeCharStr,
                                 int cchUnicodeChar, WCHAR *lpASCIICharStr,
                                 int cchASCIIChar);
WINBASEAPI int WINAPI IdnToUnicode(DWORD dwFlags, const WCHAR *lpASCIICharStr,
                                   int cchASCIIChar, WCHAR *lpUnicodeCharStr,
                                   int cchUnicodeChar);
#endif
...
static CURLcode win32_idn_to_ascii(const char *in, char **out) {
  ...
  *out = NULL;
  if (in_w) {
    ...
    int chars =
        IdnToAscii(0, in_w, (int)(wcslen(in_w) + 1), punycode, IDN_MAX_LENGTH);
...
static CURLcode win32_ascii_to_idn(const char *in, char **output) {
  ...
  if (in_w) {
    ...
    int chars =
        IdnToUnicode(0, in_w, (int)(wcslen(in_w) + 1), idn, IDN_MAX_LENGTH);
    if (chars) {
...
static CURLcode idn_decode(const char *input, char **output) {
  ...
  if (idn2_check_version(IDN2_VERSION)) {
    int flags = IDN2_NFC_INPUT
                ...
                /* IDN2_NFC_INPUT: Normalize input string using normalization
                   form C. IDN2_NONTRANSITIONAL: Perform Unicode TR46
