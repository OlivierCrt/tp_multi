...
const char *const Curl_wkday[] = {"Mon", "Tue", "Wed", "Thu",
                                  "Fri", "Sat", "Sun"};
const char *const Curl_month[] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun",
                                  "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
#endif

#ifndef CURL_DISABLE_PARSEDATE
static const char *const weekday[] = {"Monday",   "Tuesday", "Wednesday",
...
static int checktz(const char *check, size_t len) {
  ...
  for (i = 0; i < sizeof(tz) / sizeof(tz[0]); i++) {
    ...
    if ((ilen == len) && strncasecompare(check, what->name, len))
 ...
 */
static time_t time2epoch(int sec, int min, int hour, int mday, int mon,
                         int year) {
  ...
  return ((((time_t)(year - 1970) * 365 + leap_days +
            month_days_cumulative[mon] + mday - 1) *
               24 +
           hour) *
              60 +
          min) *
...
static bool match_time(const char *date, int *h, int *m, int *s, char **endp) {
  const char *p;
  int hh, mm, ss = 0;
...
static int parsedate(const char *date, time_t *output) {
  ...
  if ((mdaynum > 31) || (monnum > 11) || (hournum > 23) || (minnum > 59) ||
      (secnum > 60))
