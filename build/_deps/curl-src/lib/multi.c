...
#define GOOD_MULTI_HANDLE(x)                                                   \
  (((x) && (x)->magic == CURL_MULTI_HANDLE) ? TRUE : (DEBUGASSERT(!(x)), FALSE))
#else
...
static CURLMcode add_next_timeout(struct curltime now, struct Curl_multi *multi,
                                  struct Curl_easy *d);
...
static const char *const multi_statename[] = {
    "INIT",       "PENDING",      "CONNECT",      "RESOLVING",
    "CONNECTING", "TUNNELING",    "PROTOCONNECT", "PROTOCONNECTING",
    "DO",         "DOING",        "DOING_MORE",   "DID",
    "PERFORMING", "RATELIMITING", "DONE",         "COMPLETED",
    "MSGSENT",
};
#endif
...
static void mstate(struct Curl_easy *data, CURLMstate state
  ...
  if (data->mstate >= MSTATE_PENDING && data->mstate < MSTATE_COMPLETED) {
    infof(data, "STATE: %s => %s handle %p; line %d", multi_statename[oldstate],
          multi_statename[data->mstate], (void *)data, lineno);
  }
#endif
...
static void trhash_dtor(void *nada) { (void)nada; }
...
static void sh_delentry(struct Curl_sh_entry *entry, struct Curl_hash *sh,
                        curl_socket_t s) {
...
static void sh_init(struct Curl_hash *hash, int hashsize) {
  Curl_hash_init(hash, hashsize, hash_fd, fd_key_compare, sh_freeentry);
}
...
struct Curl_multi *curl_multi_init(void) {
  return Curl_multi_handle(CURL_SOCKET_HASH_TABLE_SIZE,
                           CURL_CONNECTION_HASH_SIZE, CURL_DNS_HASH_SIZE);
}
...
static bool in_main_list(struct Curl_easy *data) {
  return ((data->mstate != MSTATE_PENDING) && (data->mstate != MSTATE_MSGSENT));
}

static void link_easy(struct Curl_multi *multi, struct Curl_easy *data) {
...
static void unlink_easy(struct Curl_multi *multi, struct Curl_easy *data) {
...
static CURLcode multi_done(struct Curl_easy *data,
  ...
  DEBUGF(infof(data, "multi_done[%s]: status: %d prem: %d done: %d",
               multi_statename[data->mstate], (int)status, (int)premature,
               data->state.done));
#else
  DEBUGF(infof(data, "multi_done: status: %d prem: %d done: %d", (int)status,
               (int)premature, data->state.done));
#endif
  ...
  */
  ...
  if ((data->set.reuse_forbid
    ...
    DEBUGF(infof(data,
                 "multi_done, not reusing connection=%" CURL_FORMAT_CURL_OFF_T
                 ", forbid=%d"
                 ", close=%d, premature=%d, conn_multiplex=%d",
                 conn->connection_id, data->set.reuse_forbid, conn->bits.close,
                 premature, Curl_conn_is_multiplex(conn, FIRSTSOCKET)));
...
static int close_connect_only(struct Curl_easy *data, struct connectdata *conn,
                              void *param) {
  (void)param;
...
CURLMcode curl_multi_remove_handle(struct Curl_multi *multi,
  ...
  if (data->conn && data->mstate > MSTATE_DO &&
  ...
  if (data->state.lastconnect_id != -1) {
    ...
    Curl_conncache_foreach(data, data->state.conn_cache, NULL,
                           close_connect_only);
  }
...
static int domore_getsock(struct Curl_easy *data, struct connectdata *conn,
                          curl_socket_t *socks) {
...
static int doing_getsock(struct Curl_easy *data, struct connectdata *conn,
                         curl_socket_t *socks) {
...
static int protocol_getsock(struct Curl_easy *data, struct connectdata *conn,
                            curl_socket_t *socks) {
...
CURLMcode curl_multi_fdset(struct Curl_multi *multi, fd_set *read_fd_set,
 ...
 */
static bool multi_handle_timeout(struct Curl_easy *data, struct curltime *now,
                                 bool *stream_error, CURLcode *result,
  ...
  if (timeout_ms < 0) {
    ...
    if (data->mstate == MSTATE_RESOLVING)
      failf(data,
            "Resolving timed out after %" CURL_FORMAT_TIMEDIFF_T
            " milliseconds",
            Curl_timediff(*now, data->progress.t_startsingle));
    else if (data->mstate == MSTATE_CONNECTING)
      failf(data,
            "Connection timed out after %" CURL_FORMAT_TIMEDIFF_T
            " milliseconds",
            Curl_timediff(*now, data->progress.t_startsingle));
    else {
      struct SingleRequest *k = &data->req;
      if (k->size != -1) {
        failf(data,
              ...
              Curl_timediff(*now, data->progress.t_startsingle), k->bytecount,
              k->size);
      } else {
        failf(data,
              "Operation timed out after %" CURL_FORMAT_TIMEDIFF_T
...
#endif
  if (data->set.postfields || (data->state.httpreq == HTTPREQ_GET) ||
...
static CURLMcode multi_runsingle(struct Curl_multi *multi,
  ...
  do {
    ...
    if (data->conn && (data->mstate >= MSTATE_CONNECT) &&
        (data->mstate < MSTATE_COMPLETED)) {
    ...
    case MSTATE_DO:
      if (data->set.fprereq) {
        ...
        prereq_rc = data->set.fprereq(
            data->set.prereq_userp, data->info.conn_primary_ip,
            data->info.conn_local_ip, data->info.conn_primary_port,
    ...
    case MSTATE_DOING:
      ...
      if (!result) {
        if (dophase_done) {
          /* after DO, go DO_DONE or DO_MORE */
          multistate(data,
                     data->conn->bits.do_more ? MSTATE_DOING_MORE : MSTATE_DID);
          rc = CURLM_CALL_MULTI_PERFORM;
        } /* dophase_done */
      } else {
        /* failure detected */
      ...
      if (result) {
        ...
        if (data->set.max_send_speed)
          send_timeout_ms = Curl_pgrsLimitWaitTime(
              data->progress.uploaded, data->progress.ul_limit_size,
              data->set.max_send_speed, data->progress.ul_limit_start, *nowp);

        recv_timeout_ms = 0;
        if (data->set.max_recv_speed)
          recv_timeout_ms = Curl_pgrsLimitWaitTime(
              data->progress.downloaded, data->progress.dl_limit_size,
              data->set.max_recv_speed, data->progress.dl_limit_start, *nowp);
    ...
    if (data->conn && data->mstate >= MSTATE_CONNECT &&
        data->mstate < MSTATE_DO && rc != CURLM_CALL_MULTI_PERFORM &&
...
CURLMsg *curl_multi_info_read(struct Curl_multi *multi, int *msgs_in_queue) {
  ...
  *msgs_in_queue = 0; /* default to none */
  ...
  if (GOOD_MULTI_HANDLE(multi) && !multi->in_callback &&
...
static CURLMcode singlesocket(struct Curl_multi *multi,
  ...
  for (i = 0; (i < MAX_SOCKSPEREASYHANDLE) && (curraction & GETSOCK_MASK_RW(i));
       i++) {
...
static CURLMcode add_next_timeout(struct curltime now, struct Curl_multi *multi,
  ...
  if (!e) {
    ...
    multi->timetree =
        Curl_splayinsert(*tv, multi->timetree, &d->state.timenode);
  }
  return CURLM_OK;
}

static CURLMcode multi_socket(struct Curl_multi *multi, bool checkall,
...
static CURLMcode multi_addtimeout(struct Curl_easy *data,
                                  struct curltime *stamp, expire_id eid) {
  struct Curl_llist_element *e;
...
void Curl_expire(struct Curl_easy *data, timediff_t milli, expire_id id) {
  ...
  *nowp = set;
  ...
  multi->timetree =
      Curl_splayinsert(*nowp, multi->timetree, &data->state.timenode);
}
...
struct Curl_easy **curl_multi_get_handles(struct Curl_multi *multi) {
  struct Curl_easy **a =
      malloc(sizeof(struct Curl_easy *) * (multi->num_easy + 1));
  if (a) {
    int i = 0;
