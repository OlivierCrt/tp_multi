...
static void h2_tunnel_go_state(struct Curl_cfilter *cf,
  ...
  case H2_TUNNEL_ESTABLISHED:
    CURL_TRC_CF(data, cf, "[%d] new tunnel state 'established'", ts->stream_id);
...
static void drain_tunnel(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (data->state.dselect_bits != bits) {
    CURL_TRC_CF(data, cf, "[%d] DRAIN dselect_bits=%x", tunnel->stream_id,
                bits);
...
static ssize_t proxy_nw_in_reader(void *reader_ctx, unsigned char *buf,
  ...
  if (cf) {
    ...
    CURL_TRC_CF(data, cf, "[0] nw_in_reader(len=%zu) -> %zd, %d", buflen, nread,
                *err);
  } else {
    nread = 0;
  }
...
static ssize_t proxy_h2_nw_out_writer(void *writer_ctx,
  ...
  if (cf) {
    struct Curl_easy *data = CF_DATA_CURRENT(cf);
    nwritten =
        Curl_conn_cf_send(cf->next, data, (const char *)buf, buflen, err);
    CURL_TRC_CF(data, cf, "[0] nw_out_writer(len=%zu) -> %zd, %d", buflen,
                nwritten, *err);
  } else {
    nwritten = 0;
  }
...
static ssize_t on_session_send(nghttp2_session *h2, const uint8_t *buf,
...
static int proxy_h2_on_frame_send(nghttp2_session *session,
                                  const nghttp2_frame *frame, void *userp);
#endif
static int proxy_h2_on_stream_close(nghttp2_session *session, int32_t stream_id,
...
static int tunnel_recv_callback(nghttp2_session *session, uint8_t flags,
                                int32_t stream_id, const uint8_t *mem,
...
static CURLcode proxy_h2_nw_out_flush(struct Curl_cfilter *cf,
  ...
  nwritten = Curl_bufq_pass(&ctx->outbufq, proxy_h2_nw_out_writer, cf, &result);
  if (nwritten < 0) {
...
static int proxy_h2_process_pending_input(struct Curl_cfilter *cf,
  ...
  while (Curl_bufq_peek(&ctx->inbufq, &buf, &blen)) {
    ...
    if (Curl_bufq_is_empty(&ctx->inbufq)) {
      ...
      CURL_TRC_CF(data, cf,
                  "[0] process_pending_input: %zu bytes left "
...
#ifndef CURL_DISABLE_VERBOSE_STRINGS
static int proxy_h2_fr_print(const nghttp2_frame *frame, char *buffer,
                             size_t blen) {
...
static int proxy_h2_on_frame_recv(nghttp2_session *session,
  ...
  case NGHTTP2_HEADERS:
    ...
    CURL_TRC_CF(data, cf, "[%d] got http status: %d", stream_id,
                ctx->tunnel.resp->status);
  ...
  case NGHTTP2_WINDOW_UPDATE:
    if ((data->req.keepon & KEEP_SEND_HOLD) && (data->req.keepon & KEEP_SEND)) {
      data->req.keepon &= ~KEEP_SEND_HOLD;
      Curl_expire(data, 0, EXPIRE_RUN_NOW);
      CURL_TRC_CF(data, cf, "[%d] unpausing after win update", stream_id);
    }
    break;
  default:
    break;
...
static int proxy_h2_on_header(nghttp2_session *session,
  ...
  if (stream_id != ctx->tunnel.stream_id) {
    CURL_TRC_CF(data, cf,
                "[%d] header for non-tunnel stream: "
                "%.*s: %.*s",
  ...
  if (namelen == sizeof(HTTP_PSEUDO_STATUS) - 1 &&
    ...
    result =
        Curl_http_decode_status(&http_status, (const char *)value, valuelen);
    if (result)
      return NGHTTP2_ERR_CALLBACK_FAILURE;
    ...
    CURL_TRC_CF(data, cf, "[%d] status: HTTP/2 %03d", stream_id,
                ctx->tunnel.resp->status);
    return 0;
  }
  ...
  result = Curl_dynhds_add(&ctx->tunnel.resp->headers, (const char *)name,
                           namelen, (const char *)value, valuelen);
  if (result)
    return NGHTTP2_ERR_CALLBACK_FAILURE;

  CURL_TRC_CF(data, cf, "[%d] header: %.*s: %.*s", stream_id, (int)namelen,
              name, (int)valuelen, value);

  return 0; /* 0 is successful */
}

static ssize_t tunnel_send_callback(nghttp2_session *session, int32_t stream_id,
  ...
  CURL_TRC_CF(data, cf, "[%d] tunnel_send_callback -> %zd", ts->stream_id,
              nread);
  return nread;
}
...
static int proxy_h2_on_stream_close(nghttp2_session *session, int32_t stream_id,
  ...
  CURL_TRC_CF(data, cf, "[%d] proxy_h2_on_stream_close, %s (err %d)", stream_id,
...
static CURLcode proxy_h2_submit(int32_t *pstream_id, struct Curl_cfilter *cf,
                                struct Curl_easy *data, nghttp2_session *h2,
                                struct httpreq *req,
  ...
  if (read_callback) {
    ...
    stream_id = nghttp2_submit_request(h2, pri_spec, nva, nheader, &data_prd,
                                       stream_user_data);
  } else {
    stream_id = nghttp2_submit_request(h2, pri_spec, nva, nheader, NULL,
                                       stream_user_data);
  }
...
static CURLcode submit_CONNECT(struct Curl_cfilter *cf, struct Curl_easy *data,
                               struct tunnel_stream *ts) {
  struct cf_h2_proxy_ctx *ctx = cf->ctx;
  CURLcode result;
  ...
  result = proxy_h2_submit(&ts->stream_id, cf, data, ctx->h2, req, NULL, ts,
                           tunnel_send_callback, cf);
  if (result) {
...
static CURLcode inspect_response(struct Curl_cfilter *cf,
  ...
  if (auth_reply) {
    CURL_TRC_CF(data, cf, "[0] CONNECT: fwd auth header '%s'",
                auth_reply->value);
    result =
        Curl_http_input_auth(data, ts->resp->status == 407, auth_reply->value);
    if (result)
      return result;
    if (data->req.newurl) {
...
static CURLcode H2_CONNECT(struct Curl_cfilter *cf, struct Curl_easy *data,
                           struct tunnel_stream *ts) {
  struct cf_h2_proxy_ctx *ctx = cf->ctx;
  CURLcode result = CURLE_OK;
...
static ssize_t tunnel_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  *err = CURLE_AGAIN;
  if (!Curl_bufq_is_empty(&ctx->tunnel.recvbuf)) {
    nread =
        Curl_bufq_read(&ctx->tunnel.recvbuf, (unsigned char *)buf, len, err);
    if (nread < 0)
      goto out;
    DEBUGASSERT(nread > 0);
  }
...
static ssize_t cf_h2_proxy_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
                                char *buf, size_t len, CURLcode *err) {
  struct cf_h2_proxy_ctx *ctx = cf->ctx;
  ssize_t nread = -1;
  struct cf_call_data save;
  ...
  if (nread > 0) {
    CURL_TRC_CF(data, cf, "[%d] increase window by %zd", ctx->tunnel.stream_id,
                nread);
  ...
  if (result == CURLE_AGAIN) {
     ...
     */
    CURL_TRC_CF(data, cf, "[%d] egress blocked, DRAIN", ctx->tunnel.stream_id);
    drain_tunnel(cf, data, &ctx->tunnel);
  } else if (result) {
    *err = result;
    nread = -1;
...
static ssize_t cf_h2_proxy_send(struct Curl_cfilter *cf, struct Curl_easy *data,
...
static bool proxy_h2_connisalive(struct Curl_cfilter *cf,
                                 struct Curl_easy *data, bool *input_pending) {
  struct cf_h2_proxy_ctx *ctx = cf->ctx;
  bool alive = TRUE;
...
static bool cf_h2_proxy_is_alive(struct Curl_cfilter *cf,
  ...
  CURL_TRC_CF(data, cf, "[0] conn alive -> %d, input_pending=%d", result,
              *input_pending);
  CF_DATA_RESTORE(cf, save);
  return result;
}
