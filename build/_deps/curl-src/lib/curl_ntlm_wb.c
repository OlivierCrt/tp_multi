...
static CURLcode ntlm_wb_init(struct Curl_easy *data, struct ntlmdata *ntlm,
  ...
  if (access(ntlm_auth, X_OK) != 0) {
    failf(data, "Could not access ntlm_auth: %s errno %d: %s", ntlm_auth, errno,
          Curl_strerror(errno, buffer, sizeof(buffer)));
    goto done;
  }

  if (Curl_socketpair(AF_UNIX, SOCK_STREAM, 0, sockfds)) {
    failf(data, "Could not open socket pair. errno %d: %s", errno,
          Curl_strerror(errno, buffer, sizeof(buffer)));
    goto done;
  }
  ...
  if (child_pid == -1) {
    sclose(sockfds[0]);
    sclose(sockfds[1]);
    failf(data, "Could not fork. errno %d: %s", errno,
          Curl_strerror(errno, buffer, sizeof(buffer)));
    goto done;
  } else if (!child_pid) {
    /*
    ...
    if (dup2(sockfds[1], STDIN_FILENO) == -1) {
      failf(data, "Could not redirect child stdin. errno %d: %s", errno,
            Curl_strerror(errno, buffer, sizeof(buffer)));
      exit(1);
    }

    if (dup2(sockfds[1], STDOUT_FILENO) == -1) {
      failf(data, "Could not redirect child stdout. errno %d: %s", errno,
            Curl_strerror(errno, buffer, sizeof(buffer)));
      exit(1);
    }

    if (domain)
      execl(ntlm_auth, ntlm_auth, "--helper-protocol", "ntlmssp-client-1",
            "--use-cached-creds", "--username", username, "--domain", domain,
            NULL);
    else
      execl(ntlm_auth, ntlm_auth, "--helper-protocol", "ntlmssp-client-1",
            "--use-cached-creds", "--username", username, NULL);

    sclose_nolog(sockfds[1]);
    failf(data, "Could not execl(). errno %d: %s", errno,
          Curl_strerror(errno, buffer, sizeof(buffer)));
    exit(1);
  }
...
static CURLcode ntlm_wb_response(struct Curl_easy *data, struct ntlmdata *ntlm,
  ...
  if (state == NTLMSTATE_TYPE1 && len_out == 3 && ptr[0] == 'P' &&
      ptr[1] == 'W')
    goto done;
  /* invalid response */
  if (len_out < 4)
...
CURLcode Curl_input_ntlm_wb(struct Curl_easy *data, struct connectdata *conn,
