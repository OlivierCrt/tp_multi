...
static CURLcode file_done(struct Curl_easy *data, CURLcode status,
                          bool premature);
...
static CURLcode file_connect(struct Curl_easy *data, bool *done) {
  ...
  result = Curl_urldecode(data->state.up.path, 0, &real_path, &real_path_len,
                          REJECT_ZERO);
  if (result)
    return result;
  ...
  *done = TRUE; /* unconditionally */
  ...
  if (fstated) {
    ...
    if (expected_size >= 0) {
      ...
      result = Curl_client_write(data, CLIENTWRITE_HEADER, accept_ranges,
    ...
    headerlen =
        msnprintf(header, sizeof(header),
                  ...
                  tm->tm_min, tm->tm_sec, data->req.no_body ? "" : "\r\n");
    result = Curl_client_write(data, CLIENTWRITE_HEADER, header, headerlen);
    if (result)
      return result;
