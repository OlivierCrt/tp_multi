...
#define HMAC_SHA256(k, kl, d, dl, o)                                           \
  do {                                                                         \
    result = Curl_hmacit(Curl_HMAC_SHA256, (unsigned char *)k, kl,             \
                         (unsigned char *)d, dl, o);                           \
    if (result) {                                                              \
      goto fail;                                                               \
    }                                                                          \
  } while (0)
...
static void sha256_to_hex(char *dst, unsigned char *sha) {
  Curl_hexencode(sha, SHA256_DIGEST_LENGTH, (unsigned char *)dst,
                 SHA256_HEX_LENGTH);
}
...
static CURLcode make_headers(struct Curl_easy *data, const char *hostname,
                             char *timestamp, char *provider1,
                             char **date_header, char *content_sha256_header,
  ...
  msnprintf(date_full_hdr, DATE_FULL_HDR_LEN, "x-%s-date:%s", provider1,
            timestamp);
...
static char *parse_content_sha_hdr(struct Curl_easy *data,
                                   const char *provider1, size_t *value_len) {
...
static CURLcode calc_payload_hash(struct Curl_easy *data,
  ...
  result =
      Curl_sha256it(sha_hash, (const unsigned char *)post_data, post_data_len);
  if (!result)
...
static CURLcode calc_s3_payload_hash(struct Curl_easy *data,
  ...
  msnprintf(header, CONTENT_SHA256_HDR_LEN, "x-%s-content-sha256: %s",
            provider1, sha_hex);
...
static CURLcode canon_query(struct Curl_easy *data, const char *query,
                            struct dynbuf *dq) {
...
CURLcode Curl_output_aws_sigv4(struct Curl_easy *data, bool proxy) {
   ...
   */
  arg = data->set.str[STRING_AWS_SIGV4] ? data->set.str[STRING_AWS_SIGV4]
                                        : "aws:amz";
  ...
  */
  ...
  sign_as_s3 =
      (strcasecompare(provider0, "aws") && strcasecompare(service, "s3"));

  payload_hash = parse_content_sha_hdr(data, provider1, &payload_hash_len);

  if (!payload_hash) {
    if (sign_as_s3)
      result = calc_s3_payload_hash(data, httpreq, provider1, sha_hash, sha_hex,
                                    content_sha256_hdr);
    else
  ...
  result =
      make_headers(data, hostname, timestamp, provider1, &date_header,
                   content_sha256_hdr, &canonical_headers, &signed_headers);
  if (result)
    goto fail;
  ...
  canonical_request = curl_maprintf(
      ...
      method, data->state.up.path,
      Curl_dyn_ptr(&canonical_query) ? Curl_dyn_ptr(&canonical_query) : "",
      Curl_dyn_ptr(&canonical_headers), Curl_dyn_ptr(&signed_headers),
  ...
  credential_scope =
      curl_maprintf("%s/%s/%s/%s", date, region, service, request_type);
  if (!credential_scope)
    goto fail;
  ...
  str_to_sign = curl_maprintf("%s4-HMAC-SHA256\n" /* Algorithm */
                              ...
                              provider0, timestamp, credential_scope, sha_hex);
  if (!str_to_sign) {
    goto fail;
  }
  ...
  auth_headers = curl_maprintf(
      ...
      provider0, user, credential_scope, Curl_dyn_ptr(&signed_headers), sha_hex,
      date_header ? date_header : "", content_sha256_hdr);
  if (!auth_headers) {
    goto fail;
  }
