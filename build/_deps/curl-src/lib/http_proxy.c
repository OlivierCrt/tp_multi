...
CURLcode Curl_http_proxy_get_destination(struct Curl_cfilter *cf,
                                         const char **phostname, int *pport,
                                         bool *pipv6_ip) {
  DEBUGASSERT(cf);
  DEBUGASSERT(cf->conn);
...
CURLcode Curl_http_proxy_create_CONNECT(struct httpreq **preq,
  ...
  result = Curl_http_req_make(&req, "CONNECT", sizeof("CONNECT") - 1, NULL, 0,
                              authority, strlen(authority), NULL, 0);
  if (result)
    goto out;
...
connect_sub:
  ...
  *done = FALSE;
  if (!ctx->cf_protocol) {
    ...
    int alpn = Curl_conn_cf_is_ssl(cf->next) ? cf->conn->proxy_alpn
...
void Curl_cf_http_proxy_get_host(struct Curl_cfilter *cf,
                                 struct Curl_easy *data, const char **phost,
                                 const char **pdisplay_host, int *pport) {
  (void)data;
  if (!cf->connected) {
