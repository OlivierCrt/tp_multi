...
static CURLcode send_packet_no_gso(struct Curl_cfilter *cf,
                                   ...
                                   struct cf_quic_ctx *qctx, const uint8_t *pkt,
                                   size_t pktlen, size_t gsolen, size_t *psent);

static CURLcode do_sendmsg(struct Curl_cfilter *cf, struct Curl_easy *data,
                           struct cf_quic_ctx *qctx, const uint8_t *pkt,
                           size_t pktlen, size_t gsolen, size_t *psent) {
#ifdef HAVE_SENDMSG
  ...
  *psent = 0;
  ...
  while ((sent = send(qctx->sockfd, (const char *)pkt, (SEND_TYPE_ARG3)pktlen,
                      0)) == -1 &&
         SOCKERRNO == EINTR)
    ;
...
CURLcode vquic_flush(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  while (Curl_bufq_peek(&qctx->sendbuf, &buf, &blen)) {
    ...
    CURL_TRC_CF(data, cf, "vquic_send(len=%zu, gso=%zu) -> %d, sent=%zu", blen,
                gsolen, result, sent);
    if (result) {
...
static CURLcode recvmmsg_packets(struct Curl_cfilter *cf,
                                 struct Curl_easy *data,
                                 struct cf_quic_ctx *qctx, size_t max_pkts,
  ...
  while (pkts < max_pkts) {
    ...
    if (mcount == -1) {
      ...
      if (!cf->connected && SOCKERRNO == ECONNREFUSED) {
        ...
        int r_port = 0;
        Curl_cf_socket_peek(cf->next, data, NULL, NULL, &r_ip, &r_port, NULL,
                            NULL);
        failf(data, "QUIC: connection to %s port %u refused", r_ip, r_port);
    ...
    for (i = 0; i < mcount; ++i) {
      ...
      result = recv_cb(bufs[i], mmsg[i].msg_len, mmsg[i].msg_hdr.msg_name,
...
out:
  if (total_nread || result)
    CURL_TRC_CF(data, cf, "recvd %zu packets with %zu bytes -> %d", pkts,
                total_nread, result);
  return result;
}

#elif defined(HAVE_SENDMSG)
static CURLcode recvmsg_packets(struct Curl_cfilter *cf, struct Curl_easy *data,
                                struct cf_quic_ctx *qctx, size_t max_pkts,
                                vquic_recv_pkt_cb *recv_cb, void *userp) {
  struct iovec msg_iov;
  struct msghdr msg;
  ...
  for (pkts = 0, total_nread = 0; pkts < max_pkts;) {
    ...
    while ((nread = recvmsg(qctx->sockfd, &msg, 0)) == -1 && SOCKERRNO == EINTR)
      ;
    if (nread == -1) {
      if (SOCKERRNO == EAGAIN || SOCKERRNO == EWOULDBLOCK) {
        goto out;
    ...
    result =
        recv_cb(buf, (size_t)nread, msg.msg_name, msg.msg_namelen, 0, userp);
    if (result)
      goto out;
  }
...
static CURLcode recvfrom_packets(struct Curl_cfilter *cf,
  ...
  for (pkts = 0, total_nread = 0; pkts < max_pkts;) {
    ...
    total_nread += (size_t)nread;
    result =
        recv_cb(buf, (size_t)nread, &remote_addr, remote_addrlen, 0, userp);
    if (result)
      goto out;
  }
...
CURLcode vquic_recv_packets(struct Curl_cfilter *cf, struct Curl_easy *data,
                            struct cf_quic_ctx *qctx, size_t max_pkts,
                            vquic_recv_pkt_cb *recv_cb, void *userp) {
#if defined(HAVE_SENDMMSG)
 ...
 * is != -1.
 */
CURLcode Curl_qlogdir(struct Curl_easy *data, unsigned char *scid,
                      size_t scidlen, int *qlogfdp) {
  ...
  *qlogfdp = -1;
  if (qlog_dir) {
    ...
    if (!result) {
      int qlogfd =
          open(Curl_dyn_ptr(&fname), QLOGMODE, data->set.new_file_perms);
      if (qlogfd != -1)
        *qlogfdp = qlogfd;
    }
    Curl_dyn_free(&fname);
...
CURLcode Curl_cf_quic_create(struct Curl_cfilter **pcf, struct Curl_easy *data,
