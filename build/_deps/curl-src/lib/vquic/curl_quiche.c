...
static CURLcode quic_ssl_setup(struct Curl_cfilter *cf,
                               struct Curl_easy *data) {
  struct cf_quiche_ctx *ctx = cf->ctx;
...
static void stream_send_resume(struct Curl_cfilter *cf,
  ...
  if (stream_send_is_suspended(data)) {
    ...
    --ctx->sends_on_hold;
    if (H3_STREAM_ID(data) >= 0)
      CURL_TRC_CF(data, cf, "[%" PRId64 "] resume sending", H3_STREAM_ID(data));
    else
...
static void check_resumes(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct cf_quiche_ctx *ctx = cf->ctx;
  struct Curl_easy *sdata;

  if (ctx->sends_on_hold) {
    DEBUGASSERT(data->multi);
    for (sdata = data->multi->easyp; sdata && ctx->sends_on_hold;
         sdata = sdata->next) {
...
static int cb_each_header(uint8_t *name, size_t name_len, uint8_t *value,
  ...
  if ((name_len == 7) && !strncmp(HTTP_PSEUDO_STATUS, (char *)name, 7)) {
    CURL_TRC_CF(x->data, x->cf, "[%" PRId64 "] status: %.*s", stream->id,
                (int)value_len, value);
    ...
    CURL_TRC_CF(x->data, x->cf, "[%" PRId64 "] header: %.*s: %.*s", stream->id,
  ...
  if (result) {
    CURL_TRC_CF(x->data, x->cf, "[%" PRId64 "] on header error %d", stream->id,
                result);
  }
  return result;
}

static ssize_t stream_resp_read(void *reader_ctx, unsigned char *buf,
...
static CURLcode cf_recv_body(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct stream_ctx *stream = H3_STREAM_CTX(data);
  ssize_t nwritten;
  ...
  cb_ctx.data = data;
  nwritten =
      Curl_bufq_slurp(&stream->recvbuf, stream_resp_read, &cb_ctx, &result);

  if (nwritten < 0 && result != CURLE_AGAIN) {
    CURL_TRC_CF(data, cf, "[%" PRId64 "] recv_body error %zd", stream->id,
                nwritten);
...
static CURLcode h3_process_event(struct Curl_cfilter *cf,
                                 struct Curl_easy *data, int64_t stream3_id,
                                 quiche_h3_event *ev) {
  struct stream_ctx *stream = H3_STREAM_CTX(data);
  struct cb_ctx cb_ctx;
  CURLcode result = CURLE_OK;
  ...
  default:
    CURL_TRC_CF(data, cf, "[%" PRId64 "] recv, unhandled event %d", stream3_id,
...
static CURLcode cf_poll_events(struct Curl_cfilter *cf,
  ...
  while (ctx->h3c) {
    ...
    if (!sdata) {
      ...
      if (result) {
        CURL_TRC_CF(data, cf,
                    "[%" PRId64 "] error processing event %s "
                    "for [%" PRId64 "] -> %d",
                    stream ? stream->id : -1, cf_ev_name(ev), stream3_id,
                    result);
        if (data == sdata) {
...
static CURLcode recv_pkt(const unsigned char *pkt, size_t pktlen,
  ...
  nread =
      quiche_conn_recv(ctx->qconn, (unsigned char *)pkt, pktlen, &recv_info);
  if (nread < 0) {
...
static CURLcode cf_flush_egress(struct Curl_cfilter *cf,
  ...
  for (;;) {
    ...
    nread =
        Curl_bufq_sipn(&ctx->q.sendbuf, 0, read_pkt_to_send, &readx, &result);
    if (nread < 0) {
      if (result != CURLE_AGAIN)
        return result;
...
static ssize_t recv_closed_stream(struct Curl_cfilter *cf,
  ...
  if (stream->reset) {
    ...
    *err = stream->resp_got_header ? CURLE_PARTIAL_FILE : CURLE_RECV_ERROR;
    CURL_TRC_CF(data, cf, "[%" PRId64 "] cf_recv, was reset -> %d", stream->id,
                *err);
    ...
    *err = CURLE_RECV_ERROR;
    CURL_TRC_CF(data, cf,
                "[%" PRId64 "] cf_recv, closed incomplete"
                " -> %d",
                stream->id, *err);
  } else {
    *err = CURLE_OK;
...
out:
  ...
  CURL_TRC_CF(data, cf,
              "[%" PRId64 "] cf_recv(total=%" CURL_FORMAT_CURL_OFF_T
              ") -> %zd, %d",
...
static ssize_t h3_open_stream(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (stream3_id < 0) {
    if (QUICHE_H3_ERR_STREAM_BLOCKED == stream3_id) {
      ...
      *err = CURLE_AGAIN;
      ...
      CURL_TRC_CF(data, cf, "send_request(%s) -> %" PRId64, data->state.url,
                  stream3_id);
    }
    *err = CURLE_SEND_ERROR;
    nwritten = -1;
  ...
  if (Curl_trc_is_verbose(data)) {
    ...
    for (i = 0; i < nheader; ++i) {
      infof(data, "[HTTP/3] [%" PRId64 "] [%.*s: %.*s]", stream->id,
            (int)nva[i].name_len, nva[i].name, (int)nva[i].value_len,
            nva[i].value);
    }
  }
...
static ssize_t cf_quiche_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  *err = cf_process_ingress(cf, data);
  ...
  if (!stream || stream->id < 0) {
    ...
    bool eof =
        (stream->upload_left >= 0 && (curl_off_t)len >= stream->upload_left);
...
static CURLcode cf_quiche_data_event(struct Curl_cfilter *cf,
  ...
  case CF_CTRL_DATA_DONE_SEND: {
    ...
    if (stream && !stream->send_closed) {
      ...
      CURL_TRC_CF(data, cf, "[%" PRId64 "] DONE_SEND -> %zd, %d", stream->id,
                  sent, result);
    }
    break;
  }
...
static CURLcode cf_connect_start(struct Curl_cfilter *cf,
  ...
  quiche_config_set_max_connection_window(ctx->cfg, 10 * QUIC_MAX_STREAMS *
                                                        H3_STREAM_WINDOW_SIZE);
  quiche_config_set_max_stream_window(ctx->cfg, 10 * H3_STREAM_WINDOW_SIZE);
  quiche_config_set_application_protos(
      ctx->cfg, (uint8_t *)QUICHE_H3_APPLICATION_PROTOCOL,
      sizeof(QUICHE_H3_APPLICATION_PROTOCOL) - 1);
  ...
  ctx->qconn = quiche_conn_new_with_tls(
      (const uint8_t *)ctx->scid, sizeof(ctx->scid), NULL, 0,
      (struct sockaddr *)&ctx->q.local_addr, ctx->q.local_addrlen,
      &sockaddr->sa_addr, sockaddr->addrlen, ctx->cfg, ctx->ssl, false);
  if (!ctx->qconn) {
...
out:
  ...
  if (result && result != CURLE_AGAIN) {
    ...
    infof(data, "connect to %s port %u failed: %s", r_ip, r_port,
...
static CURLcode cf_quiche_query(struct Curl_cfilter *cf, struct Curl_easy *data,
