...
static bool MSH3_CALL msh3_data_received(MSH3_REQUEST *Request, void *IfContext,
...
static void MSH3_CALL msh3_shutdown_complete(MSH3_REQUEST *Request,
                                             void *IfContext);
static void MSH3_CALL msh3_data_sent(MSH3_REQUEST *Request, void *IfContext,
                                     void *SendContext);
...
#define CF_CTX_CALL_DATA(cf) ((struct cf_msh3_ctx *)(cf)->ctx)->call_data
...
static CURLcode h3_data_setup(struct Curl_cfilter *cf, struct Curl_easy *data) {
  ...
  Curl_bufq_init2(&stream->recvbuf, H3_STREAM_CHUNK_SIZE, H3_STREAM_RECV_CHUNKS,
                  BUFQ_OPT_SOFT_LIMIT);
...
static const MSH3_CONNECTION_IF msh3_conn_if = {
    msh3_conn_connected, msh3_conn_shutdown_complete, msh3_conn_new_request};
...
static const MSH3_REQUEST_IF msh3_request_if = {
    msh3_header_received, msh3_data_received, msh3_complete,
    msh3_shutdown_complete, msh3_data_sent};
 ...
 */
static CURLcode write_resp_raw(struct Curl_easy *data, const void *mem,
                               size_t memlen) {
  struct stream_ctx *stream = H3_STREAM_CTX(data);
...
static void MSH3_CALL msh3_header_received(MSH3_REQUEST *Request, void *userp,
...
static bool MSH3_CALL msh3_data_received(MSH3_REQUEST *Request, void *IfContext,
                                         uint32_t *buflen, const uint8_t *buf) {
...
static ssize_t cf_msh3_recv(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  *err = CURLE_OK;
  ...
  if (!Curl_bufq_is_empty(&stream->recvbuf)) {
    ...
    CURL_TRC_CF(data, cf, "read recvbuf(len=%zu) -> %zd, %d", len, nread, *err);
    if (nread < 0)
      goto out;
    if (stream->closed)
...
static ssize_t cf_msh3_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (!stream->req) {
    ...
    *err = Curl_http_req_to_h2(&h2_headers, h1.req, data);
    ...
    stream->req =
        MsH3RequestOpen(ctx->qconn, &msh3_request_if, data, nva, nheader,
                        eos ? MSH3_REQUEST_FLAG_FIN : MSH3_REQUEST_FLAG_NONE);
    if (!stream->req) {
...
static CURLcode cf_msh3_data_event(struct Curl_cfilter *cf,
                                   struct Curl_easy *data, int event, int arg1,
                                   void *arg2) {
  struct stream_ctx *stream = H3_STREAM_CTX(data);
  struct cf_call_data save;
  CURLcode result = CURLE_OK;
  ...
  case CF_CTRL_DATA_DONE_SEND:
    ...
    if (stream) {
      ...
      if (stream->req) {
        char buf[1];
        if (!MsH3RequestSend(stream->req, MSH3_REQUEST_FLAG_FIN, buf, 0,
                             data)) {
...
static CURLcode cf_connect_start(struct Curl_cfilter *cf,
  ...
  CURL_TRC_CF(data, cf, "connecting to %s:%d (verify=%d)", cf->conn->host.name,
              (int)cf->conn->remote_port, verify);
  ...
  ctx->qconn = MsH3ConnectionOpen(ctx->api, &msh3_conn_if, cf,
                                  cf->conn->host.name, &addr, !verify);
  if (!ctx->qconn) {
...
static CURLcode cf_msh3_connect(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  if (ctx) {
    ...
    if (ctx->active) {
      ...
      if (ctx->sock[SP_LOCAL] == cf->conn->sock[cf->sockindex]) {
        ...
        CURL_TRC_CF(data, cf,
                    "cf_socket_close(%d) no longer at "
                    "conn->sock[], discarding",
                    (int)ctx->sock[SP_LOCAL]);
        ctx->sock[SP_LOCAL] = CURL_SOCKET_BAD;
      }
      if (cf->sockindex == FIRSTSOCKET)
...
static CURLcode cf_msh3_query(struct Curl_cfilter *cf, struct Curl_easy *data,
