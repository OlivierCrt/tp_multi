...
#define H3_STREAM_POOL_SPARES (H3_STREAM_WINDOW_SIZE / H3_STREAM_CHUNK_SIZE) / 2
/* Receive and Send max number of chunks just follows from the
 * chunk size and window size */
#define H3_STREAM_RECV_CHUNKS (H3_STREAM_WINDOW_SIZE / H3_STREAM_CHUNK_SIZE)
#define H3_STREAM_SEND_CHUNKS (H3_STREAM_WINDOW_SIZE / H3_STREAM_CHUNK_SIZE)
...
void Curl_ngtcp2_ver(char *p, size_t len) {
  ...
  (void)msnprintf(p, len, "ngtcp2/%s nghttp3/%s", ng2->version_str,
                  ht3->version_str);
}
...
#define H3_STREAM_CTX(d)                                                       \
  ((struct h3_stream_ctx *)(((d) && (d)->req.p.http)                           \
                                ? ((struct HTTP *)(d)->req.p.http)->h3_ctx     \
                                : NULL))
#define H3_STREAM_LCTX(d) ((struct HTTP *)(d)->req.p.http)->h3_ctx
#define H3_STREAM_ID(d) (H3_STREAM_CTX(d) ? H3_STREAM_CTX(d)->id : -2)

static CURLcode h3_data_setup(struct Curl_cfilter *cf, struct Curl_easy *data) {
  ...
  Curl_bufq_initp(&stream->sendbuf, &ctx->stream_bufcp, H3_STREAM_SEND_CHUNKS,
                  BUFQ_OPT_NONE);
  ...
  Curl_bufq_initp(&stream->recvbuf, &ctx->stream_bufcp, H3_STREAM_RECV_CHUNKS,
                  BUFQ_OPT_SOFT_LIMIT);
...
static void pktx_init(struct pkt_io_ctx *pktx, struct Curl_cfilter *cf,
                      struct Curl_easy *data) {
  pktx->cf = cf;
  pktx->data = data;
  pktx->ts = timestamp();
...
static void qlog_callback(void *user_data, uint32_t flags, const void *data,
                          size_t datalen) {
  struct Curl_cfilter *cf = user_data;
  struct cf_ngtcp2_ctx *ctx = cf->ctx;
  (void)flags;
...
static void quic_settings(struct cf_ngtcp2_ctx *ctx, struct Curl_easy *data,
                          struct pkt_io_ctx *pktx) {
...
#ifdef USE_OPENSSL
static CURLcode quic_ssl_ctx(SSL_CTX **pssl_ctx, struct Curl_cfilter *cf,
                             struct Curl_easy *data) {
  struct cf_ngtcp2_ctx *ctx = cf->ctx;
  ...
  {
    const char *curves =
        conn->ssl_config.curves ? conn->ssl_config.curves : QUIC_GROUPS;
  ...
  SSL_CTX_set_verify(
      ssl_ctx, conn->ssl_config.verifypeer ? SSL_VERIFY_PEER : SSL_VERIFY_NONE,
      NULL);
  ...
  if (data->set.ssl.fsslctx) {
    ...
    result = (*data->set.ssl.fsslctx)(data, ssl_ctx, data->set.ssl.fsslctxp);
...
static CURLcode quic_set_client_cert(struct Curl_cfilter *cf,
  ...
  if (ssl_config->primary.clientcert || ssl_config->primary.cert_blob ||
    ...
    return Curl_ossl_set_client_cert(
        ...
        ssl_config->primary.cert_blob, ssl_config->cert_type, ssl_config->key,
        ssl_config->key_blob, ssl_config->key_type, ssl_config->key_passwd);
  }
...
static CURLcode quic_init_ssl(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct cf_ngtcp2_ctx *ctx = cf->ctx;
...
static CURLcode quic_init_ssl(struct Curl_cfilter *cf, struct Curl_easy *data) {
  ...
  gnutls_alpn_set_protocols(ctx->gtls->session, alpn, 2, GNUTLS_ALPN_MANDATORY);
  return CURLE_OK;
}
#elif defined(USE_WOLFSSL)

static CURLcode quic_ssl_ctx(WOLFSSL_CTX **pssl_ctx, struct Curl_cfilter *cf,
                             struct Curl_easy *data) {
  struct connectdata *conn = cf->conn;
  CURLcode result = CURLE_FAILED_INIT;
  ...
  if (conn->ssl_config.verifypeer) {
    ...
    if (ssl_cafile || ssl_capath) {
      ...
      int rc = wolfSSL_CTX_load_verify_locations_ex(
          ssl_ctx, ssl_cafile, ssl_capath, WOLFSSL_LOAD_FLAG_IGNORE_ERR);
      if (SSL_SUCCESS != rc) {
...
static CURLcode quic_init_ssl(struct Curl_cfilter *cf, struct Curl_easy *data) {
  ...
  /* set SNI */
  wolfSSL_UseSNI(ctx->ssl, WOLFSSL_SNI_HOST_NAME, hostname,
...
static void report_consumed_data(struct Curl_cfilter *cf,
                                 struct Curl_easy *data, size_t consumed) {
  struct h3_stream_ctx *stream = H3_STREAM_CTX(data);
  struct cf_ngtcp2_ctx *ctx = cf->ctx;
  ...
  if (consumed > 0) {
    CURL_TRC_CF(data, cf, "[%" PRId64 "] ACK %zu bytes of DATA", stream->id,
                consumed);
    ngtcp2_conn_extend_max_stream_offset(ctx->qconn, stream->id, consumed);
...
static int cb_recv_stream_data(ngtcp2_conn *tconn, uint32_t flags,
  ...
  CURL_TRC_CF(data, cf, "[%" PRId64 "] read_stream(len=%zu) -> %zd", stream_id,
              buflen, nconsumed);
  if (nconsumed < 0) {
    ngtcp2_ccerr_set_application_error(
        &ctx->last_error, nghttp3_err_infer_quic_app_error_code((int)nconsumed),
        NULL, 0);
...
static int cb_stream_close(ngtcp2_conn *tconn, uint32_t flags,
  ...
  rv = nghttp3_conn_close_stream(ctx->h3conn, stream3_id, app_error_code);
  CURL_TRC_CF(data, cf, "[%" PRId64 "] quic close(err=%" PRIu64 ") -> %d",
              stream3_id, app_error_code, rv);
  if (rv) {
    ngtcp2_ccerr_set_application_error(
...
static int cf_ngtcp2_get_select_socks(struct Curl_cfilter *cf,
  ...
  if ((k->keepon & KEEP_SENDBITS) == KEEP_SEND &&
      ...
      ngtcp2_conn_get_max_data_left(ctx->qconn) && stream &&
...
static void h3_drain_stream(struct Curl_cfilter *cf, struct Curl_easy *data) {
  struct h3_stream_ctx *stream = H3_STREAM_CTX(data);
  unsigned char bits;
...
static int cb_h3_stream_close(nghttp3_conn *conn, int64_t stream_id,
  ...
  if (stream->error3 != NGHTTP3_H3_NO_ERROR) {
    ...
    CURL_TRC_CF(data, cf, "[%" PRId64 "] RESET: error %" PRId64, stream->id,
                stream->error3);
  } else {
 ...
 */
static CURLcode write_resp_raw(struct Curl_cfilter *cf, struct Curl_easy *data,
                               const void *mem, size_t memlen, bool flow) {
  struct h3_stream_ctx *stream = H3_STREAM_CTX(data);
...
static int cb_h3_recv_data(nghttp3_conn *conn, int64_t stream3_id,
                           const uint8_t *buf, size_t buflen, void *user_data,
                           void *stream_user_data) {
  struct Curl_cfilter *cf = user_data;
  struct Curl_easy *data = stream_user_data;
  struct h3_stream_ctx *stream = H3_STREAM_CTX(data);
...
static int cb_h3_recv_header(nghttp3_conn *conn, int64_t stream_id,
  ...
  if (token == NGHTTP3_QPACK_TOKEN__STATUS) {
    ...
    if (result)
      return -1;
    ncopy =
        msnprintf(line, sizeof(line), "HTTP/3 %03d \r\n", stream->status_code);
...
static int init_ngh3_conn(struct Curl_cfilter *cf) {
  ...
  rc = nghttp3_conn_client_new(&ctx->h3conn, &ngh3_callbacks, &ctx->h3settings,
                               nghttp3_mem_default(), cf);
  if (rc) {
    result = CURLE_OUT_OF_MEMORY;
    goto fail;
  }
...
static nghttp3_ssize cb_h3_read_req_body(nghttp3_conn *conn, int64_t stream_id,
  ...
  if (stream->sendbuf_len_in_flight < Curl_bufq_len(&stream->sendbuf)) {
    nvecs = 0;
    while (nvecs < veccnt &&
           Curl_bufq_peek_at(&stream->sendbuf, stream->sendbuf_len_in_flight,
  ...
  CURL_TRC_CF(
      data, cf,
      "[%" PRId64 "] read req body -> "
      "%d vecs%s with %zu (buffered=%zu, left=%" CURL_FORMAT_CURL_OFF_T ")",
      stream->id, (int)nvecs, *pflags == NGHTTP3_DATA_FLAG_EOF ? " EOF" : "",
      nwritten, Curl_bufq_len(&stream->sendbuf), stream->upload_left);
...
static ssize_t h3_stream_open(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  *err = Curl_http_req_to_h2(&h2_headers, stream->h1.req, data);
  ...
  rc = nghttp3_conn_submit_request(ctx->h3conn, stream->id, nva, nheader,
                                   preader, data);
  if (rc) {
    switch (rc) {
    case NGHTTP3_ERR_CONN_CLOSING:
      CURL_TRC_CF(data, cf,
                  "h3sid[%" PRId64 "] failed to send, "
  ...
  if (Curl_trc_is_verbose(data)) {
    infof(data, "[HTTP/3] [%" PRId64 "] OPENED stream for %s", stream->id,
          data->state.url);
    for (i = 0; i < nheader; ++i) {
      infof(data, "[HTTP/3] [%" PRId64 "] [%.*s: %.*s]", stream->id,
            (int)nva[i].namelen, nva[i].name, (int)nva[i].valuelen,
            nva[i].value);
    }
  }
...
static ssize_t cf_ngtcp2_send(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  *err = CURLE_OK;
  ...
  if (stream && sent > 0 && stream->sendbuf_len_in_flight) {
    ...
    CURL_TRC_CF(data, cf,
                "[%" PRId64 "] cf_send(len=%zu), "
                "%zu bytes in flight -> EGAIN",
                stream->id, len, stream->sendbuf_len_in_flight);
    *err = CURLE_AGAIN;
    sent = -1;
    data->req.keepon |= KEEP_SEND_HOLD;
  }
...
static CURLcode qng_verify_peer(struct Curl_cfilter *cf,
  ...
  if (cf->conn->ssl_config.verifyhost) {
    ...
    result = Curl_gtls_verifyserver(
        data, ctx->gtls->session, &cf->conn->ssl_config, &data->set.ssl,
        hostname, disp_hostname, data->set.str[STRING_SSL_PINNEDPUBLICKEY]);
    if (result)
      return result;
#elif defined(USE_WOLFSSL)
...
static CURLcode recv_pkt(const unsigned char *pkt, size_t pktlen,
                         ...
                         socklen_t remote_addrlen, int ecn, void *userp) {
    ...
    if (!ctx->last_error.error_code) {
      if (rv == NGTCP2_ERR_CRYPTO) {
        ngtcp2_ccerr_set_tls_alert(
            &ctx->last_error, ngtcp2_conn_get_tls_alert(ctx->qconn), NULL, 0);
      } else {
        ngtcp2_ccerr_set_liberr(&ctx->last_error, rv, NULL, 0);
      }
    }
...
static CURLcode cf_progress_ingress(struct Curl_cfilter *cf,
  ...
  for (i = 0; i < pkts_max; i += pkts_chunk) {
    pktx->pkt_count = 0;
    result = vquic_recv_packets(cf, data, &ctx->q, pkts_chunk, recv_pkt, pktx);
    if (result) /* error */
      break;
 ...
 */
static ssize_t read_pkt_to_send(void *userp, unsigned char *buf, size_t buflen,
                                CURLcode *err) {
  ...
  *err = CURLE_OK;
  for (;;) {
    ...
    n = ngtcp2_conn_writev_stream(ctx->qconn, &x->ps.path, NULL, buf, buflen,
...
static CURLcode cf_progress_egress(struct Curl_cfilter *cf,
  ...
  max_pktcnt =
      CURLMIN(MAX_PKT_BURST, ctx->q.sendbuf.chunk_size / max_payload_size);

  for (;;) {
    /* add the next packet to send, if any, to our buffer */
    nread = Curl_bufq_sipn(&ctx->q.sendbuf, max_payload_size, read_pkt_to_send,
                           pktx, &curlcode);
    if (nread < 0) {
      if (curlcode != CURLE_AGAIN)
        return curlcode;
    ...
    if (pktcnt == 0) {
      ...
      curlcode = vquic_send_tail_split(cf, data, &ctx->q, gsolen, nread, nread);
      if (curlcode) {
        if (curlcode == CURLE_AGAIN) {
          Curl_expire(data, 1, EXPIRE_QUIC);
          return CURLE_OK;
...
static CURLcode h3_data_pause(struct Curl_cfilter *cf, struct Curl_easy *data,
                              bool pause) {
...
static CURLcode cf_ngtcp2_data_event(struct Curl_cfilter *cf,
                                     struct Curl_easy *data, int event,
                                     int arg1, void *arg2) {
  struct cf_ngtcp2_ctx *ctx = cf->ctx;
  CURLcode result = CURLE_OK;
  struct cf_call_data save;
...
static CURLcode cf_connect_start(struct Curl_cfilter *cf,
  ...
  Curl_cf_socket_peek(cf->next, data, &ctx->q.sockfd, &sockaddr, NULL, NULL,
                      NULL, NULL);
  if (!sockaddr)
    return CURLE_QUIC_CONNECT_ERROR;
  ...
  ngtcp2_addr_init(&ctx->connected_path.local,
                   (struct sockaddr *)&ctx->q.local_addr, ctx->q.local_addrlen);
  ngtcp2_addr_init(&ctx->connected_path.remote, &sockaddr->sa_addr,
                   sockaddr->addrlen);

  rc = ngtcp2_conn_client_new(&ctx->qconn, &ctx->dcid, &ctx->scid,
                              &ctx->connected_path, NGTCP2_PROTO_VER_V1,
                              &ng_callbacks, &ctx->settings,
                              &ctx->transport_params, NULL, cf);
  if (rc)
    return CURLE_QUIC_CONNECT_ERROR;
...
static CURLcode cf_ngtcp2_connect(struct Curl_cfilter *cf,
                                  struct Curl_easy *data, bool blocking,
                                  bool *done) {
  struct cf_ngtcp2_ctx *ctx = cf->ctx;
  CURLcode result = CURLE_OK;
  struct cf_call_data save;
...
out:
  ...
  if (result) {
    ...
    Curl_cf_socket_peek(cf->next, data, NULL, NULL, &r_ip, &r_port, NULL, NULL);
    infof(data, "QUIC connect to %s port %u failed: %s", r_ip, r_port,
...
static CURLcode cf_ngtcp2_query(struct Curl_cfilter *cf, struct Curl_easy *data,
  ...
  case CF_QUERY_MAX_CONCURRENT: {
    ...
    if (rp)
      *pres1 = (rp->initial_max_streams_bidi > INT_MAX)
                   ? INT_MAX
                   : (int)rp->initial_max_streams_bidi;
