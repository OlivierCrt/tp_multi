...
#define sftp_libssh2_realpath(s, p, t, m)                                      \
  libssh2_sftp_symlink_ex((s), (p), curlx_uztoui(strlen(p)), (t), (m),         \
...
static CURLcode scp_disconnect(struct Curl_easy *data, struct connectdata *conn,
...
static void state(struct Curl_easy *data, sshstate nowstate) {
  ...
  /* for debug purposes */
  static const char *const names[] = {"SSH_STOP",
                                      "SSH_INIT",
                                      "SSH_S_STARTUP",
                                      "SSH_HOSTKEY",
                                      "SSH_AUTHLIST",
  ...
  if (sshc->state != nowstate) {
    infof(data, "SFTP %p state change from %s to %s", (void *)sshc,
          names[sshc->state], names[nowstate]);
  }
#endif
...
#define SCP_SEND(a, b, c, d)                                                   \
  libssh2_scp_send_ex(a, b, (int)(c), (size_t)d, 0, 0)
#else
#define SCP_SEND(a, b, c, d)                                                   \
  libssh2_scp_send64(a, b, (int)(c), (libssh2_uint64_t)d, 0, 0)
#endif
...
static CURLcode ssh_knownhost(struct Curl_easy *data) {
  ...
  if (data->set.str[STRING_SSH_KNOWNHOSTS]) {
    ...
    const char *remotekey =
        libssh2_session_hostkey(sshc->ssh_session, &keylen, &sshkeytype);
    ...
    if (remotekey) {
      ...
      if (!keybit)
        ...
        keycheck = libssh2_knownhost_checkp(
            sshc->kh, conn->host.name,
            (conn->remote_port != PORT_SSH) ? conn->remote_port : -1, remotekey,
            keylen,
        ...
        keycheck = libssh2_knownhost_check(
            sshc->kh, conn->host.name, remotekey, keylen,
            LIBSSH2_KNOWNHOST_TYPE_PLAIN | LIBSSH2_KNOWNHOST_KEYENC_RAW |
                keybit,
            &host);
#endif
        ...
        infof(data, "SSH host check: %d, key: %s", keycheck,
              (keycheck <= LIBSSH2_KNOWNHOST_CHECK_MISMATCH) ? host->key
                                                             : "<none>");
    ...
    case CURLKHSTAT_FINE_ADD_TO_FILE:
      ...
      if (keycheck != LIBSSH2_KNOWNHOST_CHECK_MATCH) {
        ...
        int addrc = libssh2_knownhost_add(
            sshc->kh, conn->host.name, NULL, remotekey, keylen,
            LIBSSH2_KNOWNHOST_TYPE_PLAIN | LIBSSH2_KNOWNHOST_KEYENC_RAW |
                keybit,
            NULL);
        if (addrc)
          ...
          int wrc = libssh2_knownhost_writefile(
              sshc->kh, data->set.str[STRING_SSH_KNOWNHOSTS],
...
static CURLcode ssh_check_fingerprint(struct Curl_easy *data) {
  ...
  if (pubkey_sha256) {
    ...
    fingerprint =
        libssh2_hostkey_hash(sshc->ssh_session, LIBSSH2_HOSTKEY_HASH_SHA256);
#else
    const char *hostkey;
    size_t len = 0;
    ...
    if (!fingerprint) {
      failf(data, "Denied establishing ssh session: sha256 fingerprint "
                  "not available");
      state(data, SSH_SESSION_FREE);
  ...
  if (pubkey_md5) {
    ...
    fingerprint =
        libssh2_hostkey_hash(sshc->ssh_session, LIBSSH2_HOSTKEY_HASH_MD5);
    ...
    if (!fingerprint || !strcasecompare(md5buffer, pubkey_md5)) {
      if (fingerprint) {
        ...
        failf(data, "Denied establishing ssh session: md5 fingerprint "
                    "not available");
      }
      state(data, SSH_SESSION_FREE);
      sshc->actualcode = CURLE_PEER_FAILED_VERIFICATION;
  ...
  if (!pubkey_md5 && !pubkey_sha256) {
    if (data->set.ssh_hostkeyfunc) {
      ...
      if (remotekey) {
        ...
        Curl_set_in_callback(data, true);
        rc = data->set.ssh_hostkeyfunc(data->set.ssh_hostkeyfunc_userp, keytype,
                                       remotekey, keylen);
        Curl_set_in_callback(data, false);
...
static CURLcode ssh_force_knownhost_key_type(struct Curl_easy *data) {
  ...
  if (sshc->kh && !data->set.str[STRING_SSH_HOST_PUBLIC_KEY_MD5]) {
      ...
      if (store) {
        if (store->name) {
          if (store->name[0] == '[') {
            ...
            if (!kh_name_end) {
              infof(data, "Invalid host pattern %s in %s", store->name,
                    data->set.str[STRING_SSH_KNOWNHOSTS]);
              continue;
            }
            ...
            if (kh_name_end && (port == conn->remote_port)) {
              ...
              if (strncmp(store->name + 1, conn->host.name, kh_name_size) ==
                  0) {
                found = true;
                break;
              }
    ...
    if (found) {
      int rc;
      infof(data, "Found host %s in %s", conn->host.name,
            data->set.str[STRING_SSH_KNOWNHOSTS]);
      ...
      infof(data, "Did not find host %s in %s", conn->host.name,
            data->set.str[STRING_SSH_KNOWNHOSTS]);
    }
  }
  ...
  *block = 0; /* we're not blocking by default */
  ...
  do {
    ...
    case SSH_AUTHLIST:
       ...
       */
      sshc->authlist = libssh2_userauth_list(sshc->ssh_session, conn->user,
    ...
    case SSH_AUTH_PKEY:
      ...
      rc = libssh2_userauth_publickey_fromfile_ex(
          ...
          sshc->rsa_pub, sshc->rsa, sshc->passphrase);
      if (rc == LIBSSH2_ERROR_EAGAIN) {
        break;
      }
      ...
      if (rc == 0) {
        ...
        if (rc == -1) {
          ...
          (void)libssh2_session_last_error(sshc->ssh_session, &err_msg, NULL,
                                           0);
        }
    ...
    case SSH_AUTH_PASS:
      rc = libssh2_userauth_password_ex(
          sshc->ssh_session, conn->user, curlx_uztoui(strlen(conn->user)),
          conn->passwd, curlx_uztoui(strlen(conn->passwd)), NULL);
      if (rc == LIBSSH2_ERROR_EAGAIN) {
        break;
      }
      if (rc == 0) {
    ...
    case SSH_AUTH_AGENT:
      ...
      rc = libssh2_agent_get_identity(sshc->ssh_agent, &sshc->sshagent_identity,
    ...
    case SSH_SFTP_REALPATH: {
       ...
       */
      rc = sftp_libssh2_realpath(sshc->sftp_session, ".", tempHome,
                                 PATH_MAX - 1);
      if (rc == LIBSSH2_ERROR_EAGAIN) {
        break;
      }
      if (rc > 0) {
        ...
        if (sftperr)
          ...
          result = CURLE_SSH;
        sshc->actualcode = result;
        DEBUGF(infof(data, "error = %lu makes libcurl = %d", sftperr,
                     (int)result));
        state(data, SSH_STOP);
        break;
      }
    ...
    case SSH_SFTP_QUOTE:
      /* Send any quote commands */
      {
        ...
        /* if a command starts with an asterisk, which a legal SFTP command
           never can, the command will be allowed to fail without it causing any
        ...
        if (strcasecompare("pwd", cmd)) {
          ...
          /* this sends an FTP-like "header" to the header callback so that the
             ...
             using ordinary FTP. */
          result =
              Curl_client_write(data, CLIENTWRITE_HEADER, tmp, strlen(tmp));
          free(tmp);
          if (result) {
            state(data, SSH_SFTP_CLOSE);
            sshc->nextstate = SSH_NO_STATE;
    ...
    case SSH_SFTP_QUOTE_STAT: {
      ...
      if (!strncasecompare(cmd, "chmod", 5)) {
        ...
        rc = libssh2_sftp_stat_ex(sshc->sftp_session, sshc->quote_path2,
                                  ...
                                  LIBSSH2_SFTP_STAT, &sshp->quote_attrs);
        if (rc == LIBSSH2_ERROR_EAGAIN) {
          break;
        }
    ...
    case SSH_SFTP_QUOTE_SETSTAT:
      rc = libssh2_sftp_stat_ex(sshc->sftp_session, sshc->quote_path2,
                                curlx_uztoui(strlen(sshc->quote_path2)),
                                LIBSSH2_SFTP_SETSTAT, &sshp->quote_attrs);
      if (rc == LIBSSH2_ERROR_EAGAIN) {
        break;
      }
    ...
    case SSH_SFTP_QUOTE_SYMLINK:
      rc = libssh2_sftp_symlink_ex(
          sshc->sftp_session, sshc->quote_path1,
          curlx_uztoui(strlen(sshc->quote_path1)), sshc->quote_path2,
          curlx_uztoui(strlen(sshc->quote_path2)), LIBSSH2_SFTP_SYMLINK);
      if (rc == LIBSSH2_ERROR_EAGAIN) {
        break;
      }
      if (rc && !sshc->acceptfail) {
    ...
    case SSH_SFTP_QUOTE_STATVFS: {
      ...
      if (rc && !sshc->acceptfail) {
        ...
        char *tmp = aprintf("statvfs:\n"
                            ...
                            "f_flag: %llu\n"
                            "f_namemax: %llu\n",
                            statvfs.f_bsize, statvfs.f_frsize, statvfs.f_blocks,
                            statvfs.f_bfree, statvfs.f_bavail, statvfs.f_files,
                            statvfs.f_ffree, statvfs.f_favail, statvfs.f_fsid,
                            statvfs.f_flag, statvfs.f_namemax);
        if (!tmp) {
          result = CURLE_OUT_OF_MEMORY;
          state(data, SSH_SFTP_CLOSE);
          sshc->nextstate = SSH_NO_STATE;
    ...
    case SSH_SFTP_UPLOAD_INIT: {
      ...
      sshc->sftp_handle = libssh2_sftp_open_ex(
          sshc->sftp_session, sshp->path, curlx_uztoui(strlen(sshp->path)),
          flags, data->set.new_file_perms, LIBSSH2_SFTP_OPENFILE);
      ...
      if (!sshc->sftp_handle) {
        ...
        if (sshc->secondCreateDirs) {
          ...
          sshc->actualcode = sftperr != LIBSSH2_FX_OK
                                 ? sftp_libssh2_error_to_CURLE(sftperr)
                                 : CURLE_SSH;
        ...
        if (((sftperr == LIBSSH2_FX_NO_SUCH_FILE) ||
            ...
            (data->set.ftp_create_missing_dirs && (strlen(sshp->path) > 1))) {
        ...
        failf(data, "Upload failed: %s (%lu/%d)",
              sftperr != LIBSSH2_FX_OK ? sftp_libssh2_strerror(sftperr)
                                       : "ssh error",
              sftperr, rc);
        break;
      }
    ...
    case SSH_SFTP_READDIR_INIT:
       ...
       * listing
       */
      sshc->sftp_handle = libssh2_sftp_open_ex(sshc->sftp_session, sshp->path,
                                               curlx_uztoui(strlen(sshp->path)),
    ...
    case SSH_SFTP_READDIR:
      rc = libssh2_sftp_readdir_ex(sshc->sftp_handle, sshp->readdir_filename,
                                   PATH_MAX, sshp->readdir_longentry, PATH_MAX,
                                   &sshp->readdir_attrs);
      if (rc == LIBSSH2_ERROR_EAGAIN) {
        break;
      }
      if (rc > 0) {
        ...
        if (data->set.list_only) {
          result = Curl_client_write(data, CLIENTWRITE_BODY,
                                     sshp->readdir_filename, readdir_len);
          if (!result)
    ...
    case SSH_SFTP_READDIR_LINK:
      rc = libssh2_sftp_symlink_ex(
          sshc->sftp_session, Curl_dyn_ptr(&sshp->readdir_link),
          (int)Curl_dyn_len(&sshp->readdir_link), sshp->readdir_filename,
    ...
    case SSH_SFTP_READDIR_BOTTOM:
      ...
      if (!result) {
        /* output debug output if that is requested */
        Curl_debug(data, CURLINFO_DATA_IN, Curl_dyn_ptr(&sshp->readdir),
                   Curl_dyn_len(&sshp->readdir));
    ...
    case SSH_SFTP_DOWNLOAD_STAT: {
      ...
      if (rc || !(attrs.flags & LIBSSH2_SFTP_ATTR_SIZE) ||
          (attrs.filesize == 0)) {
        /*
        ...
        if (data->state.use_range) {
          ...
          if (from > size) {
            failf(data,
                  ...
                  from, (curl_off_t)attrs.filesize);
            return CURLE_BAD_DOWNLOAD_RESUME;
          }
          if (from > to) {
            from = to;
    ...
    case SSH_SCP_UPLOAD_INIT:
      ...
      if (!sshc->ssh_channel) {
        ...
        ssh_err = (int)(libssh2_session_last_error(sshc->ssh_session, &err_msg,
                                                   NULL, 0));
    ...
    case SSH_SESSION_DISCONNECT:
      ...
      if (sshc->ssh_session) {
        ...
        if (rc < 0) {
          ...
          (void)libssh2_session_last_error(sshc->ssh_session, &err_msg, NULL,
                                           0);
          infof(data, "Failed to disconnect libssh2 session: %d %s", rc,
                err_msg);
        }
      }
    ...
    case SSH_SESSION_FREE:
      ...
      if (sshc->ssh_agent) {
        ...
        if (rc < 0) {
          ...
          (void)libssh2_session_last_error(sshc->ssh_session, &err_msg, NULL,
                                           0);
          infof(data, "Failed to disconnect from libssh2 agent: %d %s", rc,
                err_msg);
        }
...
static CURLcode ssh_block_statemach(struct Curl_easy *data,
                                    struct connectdata *conn, bool disconnect) {
  struct ssh_conn *sshc = &conn->proto.sshc;
  CURLcode result = CURLE_OK;
...
static ssize_t ssh_tls_recv(libssh2_socket_t sock, void *buffer, size_t length,
...
static CURLcode ssh_connect(struct Curl_easy *data, bool *done) {
  ...
  sshc->ssh_session = libssh2_session_init_ex(
      my_libssh2_malloc, my_libssh2_free, my_libssh2_realloc, data);
  ...
  if (conn->http_proxy.proxytype == CURLPROXY_HTTPS) {
    ...
    */
    libssh2_session_callback_set(sshc->ssh_session, LIBSSH2_CALLBACK_RECV,
                                 sshrecv.recvp);
    libssh2_session_callback_set(sshc->ssh_session, LIBSSH2_CALLBACK_SEND,
                                 sshsend.sendp);
...
static CURLcode scp_perform(struct Curl_easy *data, bool *connected,
                            bool *dophase_done) {
  CURLcode result = CURLE_OK;
...
static ssize_t scp_send(struct Curl_easy *data, int sockindex, const void *mem,
...
static ssize_t scp_recv(struct Curl_easy *data, int sockindex, char *mem,
                        size_t len, CURLcode *err) {
  ssize_t nread;
  struct connectdata *conn = data->conn;
  struct ssh_conn *sshc = &conn->proto.sshc;
...
static ssize_t sftp_send(struct Curl_easy *data, int sockindex, const void *mem,
                         size_t len, CURLcode *err) {
  ssize_t nwrite;
  struct connectdata *conn = data->conn;
  struct ssh_conn *sshc = &conn->proto.sshc;
 ...
 * or <0 on error
 */
static ssize_t sftp_recv(struct Curl_easy *data, int sockindex, char *mem,
                         size_t len, CURLcode *err) {
  ssize_t nread;
  struct connectdata *conn = data->conn;
  struct ssh_conn *sshc = &conn->proto.sshc;
