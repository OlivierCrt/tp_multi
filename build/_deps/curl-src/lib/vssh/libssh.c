...
#if defined(__GNUC__) && (LIBSSH_VERSION_MINOR >= 10) ||                       \
...
static CURLcode myssh_multi_statemach(struct Curl_easy *data, bool *done);
static CURLcode myssh_do_it(struct Curl_easy *data, bool *done);

static CURLcode scp_done(struct Curl_easy *data, CURLcode, bool premature);
...
static int myssh_getsock(struct Curl_easy *data, struct connectdata *conn,
                         curl_socket_t *sock);
...
static void mystate(struct Curl_easy *data, sshstate nowstate
  ...
  if (sshc->state != nowstate) {
    infof(data, "SSH %p state change from %s to %s (line %d)", (void *)sshc,
          names[sshc->state], names[nowstate], lineno);
  }
#endif
...
static int myssh_is_known(struct Curl_easy *data) {
  ...
  vstate =
      ssh_session_get_known_hosts_entry(sshc->ssh_session, &knownhostsentry);
  ...
  *block = 0; /* we're not blocking by default */
  ...
  do {
    ...
    case SSH_AUTH_PKEY_INIT:
      ...
      if (data->set.str[STRING_SSH_PRIVATE_KEY]) {
        if (sshc->pubkey && !data->set.ssl.key_passwd) {
          rc =
              ssh_userauth_try_publickey(sshc->ssh_session, NULL, sshc->pubkey);
          if (rc == SSH_AUTH_AGAIN) {
            rc = SSH_AGAIN;
            break;
          }
        ...
        rc = ssh_pki_import_privkey_file(data->set.str[STRING_SSH_PRIVATE_KEY],
                                         data->set.ssl.key_passwd, NULL, NULL,
                                         &sshc->privkey);
        if (rc != SSH_OK) {
    ...
    case SSH_SFTP_QUOTE_RENAME:
      rc =
          sftp_rename(sshc->sftp_session, sshc->quote_path1, sshc->quote_path2);
      if (rc && !sshc->acceptfail) {
        Curl_safefree(sshc->quote_path1);
        Curl_safefree(sshc->quote_path2);
    ...
    case SSH_SFTP_UPLOAD_INIT: {
      ...
      sshc->sftp_file = sftp_open(sshc->sftp_session, protop->path, flags,
    ...
    case SSH_SFTP_READDIR:
      ...
      if (sshc->readdir_attrs) {
        ...
        if (data->set.list_only) {
          ...
          result = Curl_client_write(data, CLIENTWRITE_BODY, tmpLine,
                                     sshc->readdir_len + 1);
          free(tmpLine);
    ...
    case SSH_SFTP_READDIR_LINK:
      ...
      sshc->readdir_link_attrs =
          sftp_lstat(sshc->sftp_session, sshc->readdir_linkPath);
      ...
      if (!sshc->readdir_link_attrs->name) {
        sshc->readdir_tmp =
            sftp_readlink(sshc->sftp_session, sshc->readdir_linkPath);
      ...
      if (Curl_dyn_addf(&sshc->readdir_buf, " -> %s", sshc->readdir_filename)) {
        sshc->actualcode = CURLE_OUT_OF_MEMORY;
        break;
      }
    ...
    case SSH_SFTP_DOWNLOAD_INIT:
      ...
      sshc->sftp_file = sftp_open(sshc->sftp_session, protop->path, O_RDONLY,
                                  (mode_t)data->set.new_file_perms);
      if (!sshc->sftp_file) {
        failf(data, "Could not open remote file for reading: %s",
              ssh_get_error(sshc->ssh_session));
...
static CURLcode myssh_block_statemach(struct Curl_easy *data, bool disconnect) {
  struct connectdata *conn = data->conn;
  struct ssh_conn *sshc = &conn->proto.sshc;
  CURLcode result = CURLE_OK;
...
static CURLcode myssh_connect(struct Curl_easy *data, bool *done) {
  ...
  if (conn->remote_port) {
    rc =
        ssh_options_set(ssh->ssh_session, SSH_OPTIONS_PORT, &conn->remote_port);
    if (rc != SSH_OK) {
...
static ssize_t sftp_recv(struct Curl_easy *data, int sockindex, char *mem,
  ...
  case 1:
    ...
    nread = sftp_async_read(conn->proto.sshc.sftp_file, mem, (uint32_t)len,
                            conn->proto.sshc.sftp_file_index);
...
static void sftp_quote(struct Curl_easy *data) {
  ...
  if (strncasecompare(cmd, "chgrp ", 6) || strncasecompare(cmd, "chmod ", 6) ||
      strncasecompare(cmd, "chown ", 6) || strncasecompare(cmd, "atime ", 6) ||
...
void Curl_ssh_cleanup(void) { (void)ssh_finalize(); }
