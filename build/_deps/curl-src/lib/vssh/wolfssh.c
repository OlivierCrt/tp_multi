...
#endif
static CURLcode wsftp_done(struct Curl_easy *data, CURLcode, bool premature);
static CURLcode wsftp_doing(struct Curl_easy *data, bool *dophase_done);
static CURLcode wsftp_disconnect(struct Curl_easy *data,
                                 struct connectdata *conn, bool dead);
static int wssh_getsock(struct Curl_easy *data, struct connectdata *conn,
                        curl_socket_t *sock);
...
static void state(struct Curl_easy *data, sshstate nowstate) {
  ...
  if (sshc->state != nowstate) {
    infof(data, "wolfssh %p state change from %s to %s", (void *)sshc,
...
static ssize_t wscp_send(struct Curl_easy *data, int sockindex, const void *mem,
...
static ssize_t wscp_recv(struct Curl_easy *data, int sockindex, char *mem,
                         size_t len, CURLcode *err) {
  ssize_t nread = 0;
  (void)data;
  (void)sockindex; /* we only support SCP on the fixed known primary socket */
...
static ssize_t wsftp_send(struct Curl_easy *data, int sockindex,
  ...
  rc = wolfSSH_SFTP_SendWritePacket(sshc->ssh_session, sshc->handle,
                                    sshc->handleSz, &offset[0], (byte *)mem,
                                    (word32)len);
  ...
  infof(data, "sent %zu bytes SFTP from offset %" CURL_FORMAT_CURL_OFF_T, len,
        sshc->offset);
  sshc->offset += len;
  return (ssize_t)rc;
}
 ...
 * or <0 on error
 */
static ssize_t wsftp_recv(struct Curl_easy *data, int sockindex, char *mem,
                          size_t len, CURLcode *err) {
  int rc;
  struct connectdata *conn = data->conn;
  struct ssh_conn *sshc = &conn->proto.sshc;
...
static int userauth(byte authtype, WS_UserAuthData *authdata, void *ctx) {
  ...
  DEBUGF(
      infof(data, "wolfssh callback: type %s",
  ...
  *block = FALSE; /* we're not blocking by default */
  ...
  do {
    ...
    case SSH_SFTP_UPLOAD_INIT: {
      ...
      rc = wolfSSH_SFTP_Open(sshc->ssh_session, sftp_scp->path, flags,
                             &createattrs, sshc->handle, &sshc->handleSz);
      if (rc == WS_FATAL_ERROR)
        rc = wolfSSH_get_error(sshc->ssh_session);
      if (rc == WS_WANT_READ) {
        *block = TRUE;
    ...
    case SSH_SFTP_DOWNLOAD_INIT:
      sshc->handleSz = sizeof(sshc->handle);
      rc =
          wolfSSH_SFTP_Open(sshc->ssh_session, sftp_scp->path, WOLFSSH_FXF_READ,
                            NULL, sshc->handle, &sshc->handleSz);
      if (rc == WS_FATAL_ERROR)
        rc = wolfSSH_get_error(sshc->ssh_session);
      if (rc == WS_WANT_READ) {
        *block = TRUE;
    ...
    case SSH_SFTP_CLOSE:
      if (sshc->handleSz)
        rc =
            wolfSSH_SFTP_Close(sshc->ssh_session, sshc->handle, sshc->handleSz);
      else
    ...
    case SSH_SFTP_READDIR:
      ...
      if (rc == WS_WANT_READ) {
        ...
        *block = TRUE;
        ...
        while (name) {
          char *line =
              aprintf("%s\n", data->set.list_only ? name->fName : name->lName);
          if (!line) {
            state(data, SSH_SFTP_CLOSE);
            sshc->actualcode = CURLE_OUT_OF_MEMORY;
            break;
          ...
          result =
              Curl_client_write(data, CLIENTWRITE_BODY, line, strlen(line));
          free(line);
          if (result) {
            sshc->actualcode = result;
            break;
...
static CURLcode wscp_perform(struct Curl_easy *data, bool *connected,
                             bool *dophase_done) {
  (void)data;
  (void)connected;
  (void)dophase_done;
...
static CURLcode wsftp_perform(struct Curl_easy *data, bool *connected,
                              bool *dophase_done) {
  CURLcode result = CURLE_OK;
...
static CURLcode wssh_block_statemach(struct Curl_easy *data, bool disconnect) {
  struct connectdata *conn = data->conn;
  struct ssh_conn *sshc = &conn->proto.sshc;
  CURLcode result = CURLE_OK;
...
static CURLcode wsftp_done(struct Curl_easy *data, CURLcode code,
                           bool premature) {
  (void)premature;
  state(data, SSH_SFTP_CLOSE);
...
void Curl_ssh_cleanup(void) { (void)wolfSSH_Cleanup(); }
