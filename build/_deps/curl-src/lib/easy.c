...
static CURLcode dupset(struct Curl_easy *dst, struct Curl_easy *src) {
  ...
  if (src->set.postfieldsize && src->set.str[i]) {
    ...
    dst->set.str[i] =
        Curl_memdup(src->set.str[i], curlx_sotouz(src->set.postfieldsize));
    if (!dst->set.str[i])
      return CURLE_OUT_OF_MEMORY;
...
struct Curl_easy *curl_easy_duphandle(struct Curl_easy *data) {
  ...
  if (data->cookies) {
    ...
    outcurl->cookies =
        Curl_cookie_init(data, NULL, outcurl->cookies, data->set.cookiesession);
    if (!outcurl->cookies)
      goto fail;
  }
  ...
  if (data->hsts) {
    ...
    if (outcurl->set.str[STRING_HSTS])
      (void)Curl_hsts_loadfile(outcurl, outcurl->hsts,
                               outcurl->set.str[STRING_HSTS]);
    (void)Curl_hsts_loadcb(outcurl, outcurl->hsts);
  }
#endif
  /* Clone the resolver handle, if present, for the new handle */
  if (Curl_resolver_duphandle(outcurl, &outcurl->state.async.resolver,
 ...
 * Returns always 0.
 */
static int conn_upkeep(struct Curl_easy *data, struct connectdata *conn,
                       void *param) {
...
static CURLcode upkeep(struct conncache *conn_cache, void *data) {
  ...
  Curl_conncache_foreach(data, conn_cache, &now, conn_upkeep);
  return CURLE_OK;
}
