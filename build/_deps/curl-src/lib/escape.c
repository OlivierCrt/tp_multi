...
CURLcode Curl_urldecode(const char *string, size_t length, char **ostring,
  ...
  *ostring = ns;
  ...
  while (alloc) {
    ...
    if (('%' == in) && (alloc > 2) && ISXDIGIT(string[1]) &&
        ISXDIGIT(string[2])) {
 ...
 * 'data' is ignored since 7.82.0.
 */
char *curl_easy_unescape(struct Curl_easy *data, const char *string, int length,
                         int *olen) {
  char *str = NULL;
  (void)data;
  if (length >= 0) {
    ...
    size_t outputlen;
    CURLcode res =
        Curl_urldecode(string, inputlen, &str, &outputlen, REJECT_NADA);
    if (res)
      return NULL;
