...
bool curlx_verify_windows_version(const unsigned int majorVersion,
  ...
  if (GetVersionEx(&osver)) {
    ...
    case VERSION_LESS_THAN:
      if (osver.dwMajorVersion < majorVersion ||
          ...
          (buildVersion != 0 && (osver.dwMajorVersion == majorVersion &&
                                 osver.dwMinorVersion == minorVersion &&
    ...
    case VERSION_LESS_THAN_EQUAL:
      if (osver.dwMajorVersion < majorVersion ||
          ...
          (osver.dwMajorVersion == majorVersion &&
           osver.dwMinorVersion == minorVersion &&
           (buildVersion == 0 || osver.dwBuildNumber <= buildVersion)))
        matched = TRUE;
      break;
  ...
  if (onetime) {
    pRtlVerifyVersionInfo = CURLX_FUNCTION_CAST(
        RTLVERIFYVERSIONINFO_FN,
  ...
  if (matched && buildVersion &&
      (condition == VERSION_EQUAL ||
       ((condition == VERSION_GREATER_THAN_EQUAL ||
        ...
        curlx_verify_windows_version(majorVersion, minorVersion, 0, platform,
                                     VERSION_EQUAL)))) {
    ...
    if (pRtlVerifyVersionInfo)
      ...
      matched =
          !!VerifyVersionInfoW((OSVERSIONINFOEXW *)&osver, dwTypeMask, cm);
  }
