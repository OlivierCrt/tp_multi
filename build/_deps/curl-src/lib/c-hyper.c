...
static int hyper_each_header(void *userdata, const uint8_t *name,
...
static CURLcode status_line(struct Curl_easy *data, struct connectdata *conn,
                            uint16_t http_status, int http_version,
                            const uint8_t *reason, size_t rlen) {
  CURLcode result;
  size_t len;
  const char *vstr;
  int writetype;
  vstr = http_version == HYPER_HTTP_VERSION_1_1
             ? "1.1"
  ...
  if (data->state.hconnect)
    ...
    conn->httpversion = http_version == HYPER_HTTP_VERSION_1_1
                            ? 11
  ...
  result = Curl_dyn_addf(&data->state.headerb, "HTTP/%s %03d %.*s\r\n", vstr,
                         (int)http_status, (int)rlen, reason);
  if (result)
    return result;
  len = Curl_dyn_len(&data->state.headerb);
  Curl_debug(data, CURLINFO_HEADER_IN, Curl_dyn_ptr(&data->state.headerb), len);
...
static CURLcode empty_header(struct Curl_easy *data) {
  ...
  if (!result) {
    result = hyper_each_header(data, NULL, 0, NULL, 0) ? CURLE_WRITE_ERROR
                                                       : CURLE_OK;
    if (result)
...
CURLcode Curl_hyper_stream(struct Curl_easy *data, struct connectdata *conn,
                           int *didwhat, bool *done, int select_res) {
  ...
  *done = FALSE;
  do {
    ...
    *didwhat = KEEP_RECV;
    ...
    result =
        status_line(data, conn, http_status, http_version, reasonp, reason_len);
    if (result)
      break;
    ...
    foreach
      = hyper_body_foreach(resp_body, hyper_body_chunk, data);
    if (!foreach) {
...
static CURLcode debug_request(struct Curl_easy *data, const char *method,
                              const char *path, bool h2) {
  char *req = aprintf("%s %s HTTP/%s\r\n", method, path, h2 ? "2" : "1.1");
  if (!req)
...
CURLcode Curl_hyper_header(struct Curl_easy *data, hyper_headers *headers,
  ...
  do {
    ...
    if (HYPERE_OK !=
        hyper_headers_add(headers, (uint8_t *)n, nlen, (uint8_t *)v, vlen)) {
...
static CURLcode request_target(struct Curl_easy *data, struct connectdata *conn,
                               const char *method, bool h2,
                               hyper_request *req) {
  CURLcode result;
  struct dynbuf r;
  ...
  if (h2 && hyper_request_set_uri_parts(
                req,
                /* scheme */
...
static CURLcode bodysend(struct Curl_easy *data, struct connectdata *conn,
                         hyper_headers *headers, hyper_request *hyperreq,
...
static CURLcode cookies(struct Curl_easy *data, struct connectdata *conn,
...
static void http1xx_cb(void *arg, struct hyper_response *resp) {
  ...
  result = status_line(data, data->conn, http_status, http_version, reasonp,
                       reason_len);
  if (!result) {
    headers = hyper_response_headers(resp);
    if (!headers) {
...
CURLcode Curl_http(struct Curl_easy *data, bool *done) {
  ...
  *done = TRUE;
  ...
  if (!Curl_use_http_1_1plus(data, conn)) {
    if (HYPERE_OK != hyper_request_set_version(req, HYPER_HTTP_VERSION_1_0)) {
  ...
  if (data->set.str[STRING_USERAGENT] && *data->set.str[STRING_USERAGENT] &&
  ...
  if (!Curl_checkheaders(data, STRCONST("Accept-Encoding")) &&
    ...
    if (!data->state.aptr.accept_encoding)
      ...
      result =
          Curl_hyper_header(data, headers, data->state.aptr.accept_encoding);
    if (result)
      goto error;
  } else
    Curl_safefree(data->state.aptr.accept_encoding);
