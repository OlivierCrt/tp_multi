...
static int smtp_getsock(struct Curl_easy *data, struct connectdata *conn,
                        curl_socket_t *socks);
...
const struct Curl_handler Curl_handler_smtps = {
    ...
    CURLPROTO_SMTP,        /* family */
    PROTOPT_CLOSEACTION | PROTOPT_SSL | PROTOPT_NOURLQUERY |
        PROTOPT_URLOPTIONS /* flags */
};
#endif
...
static void smtp_state(struct Curl_easy *data, smtpstate newstate) {
  ...
  static const char *const names[] = {
      "STOP",       "SERVERGREET", "EHLO",    "HELO", "STARTTLS",
      "UPGRADETLS", "AUTH",        "COMMAND", "MAIL", "RCPT",
      "DATA",       "POSTDATA",    "QUIT",
      /* LAST */
  };

  if (smtpc->state != newstate)
    infof(data, "SMTP %p state change from %s to %s", (void *)smtpc,
...
static CURLcode smtp_perform_starttls(struct Curl_easy *data,
  ...
  CURLcode result =
      Curl_pp_sendf(data, &conn->proto.smtpc.pp, "%s", "STARTTLS");
...
static CURLcode smtp_continue_auth(struct Curl_easy *data, const char *mech,
  ...
  return Curl_pp_sendf(data, &smtpc->pp, "%s",
...
static CURLcode smtp_perform_command(struct Curl_easy *data) {
  ...
  if (smtp->rcpt) {
    ...
    if ((!smtp->custom) || (!smtp->custom[0])) {
      ...
      result = smtp_parse_address(smtp->rcpt->data, &address, &host);
      if (result)
        return result;
      ...
      result =
          Curl_pp_sendf(data, &conn->proto.smtpc.pp, "VRFY %s%s%s%s", address,
      ...
      utf8 =
          (conn->proto.smtpc.utf8_supported) && (!strcmp(smtp->custom, "EXPN"));

      /* Send the custom recipient based command such as the EXPN command */
      result =
          Curl_pp_sendf(data, &conn->proto.smtpc.pp, "%s %s%s", smtp->custom,
                        smtp->rcpt->data, utf8 ? " SMTPUTF8" : "");
    }
  } else
    /* Send the non-recipient based command such as HELP */
    result = Curl_pp_sendf(
        data, &conn->proto.smtpc.pp, "%s",
...
static CURLcode smtp_perform_mail(struct Curl_easy *data) {
  ...
  if (data->set.str[STRING_MAIL_FROM]) {
    ...
    result =
        smtp_parse_address(data->set.str[STRING_MAIL_FROM], &address, &host);
    if (result)
      return result;
  ...
  if (data->set.str[STRING_MAIL_AUTH] && conn->proto.smtpc.sasl.authused) {
    if (data->set.str[STRING_MAIL_AUTH][0] != '\0') {
      ...
      /* Parse the AUTH mailbox into the local address and host name parts,
         converting the host name to an IDN A-label if necessary */
      result =
          smtp_parse_address(data->set.str[STRING_MAIL_AUTH], &address, &host);
      if (result) {
        free(from);
        return result;
      }
  ...
  result = Curl_pp_sendf(data, &conn->proto.smtpc.pp, "MAIL FROM:%s%s%s%s%s%s",
                         from,                 /* Mandatory                 */
...
static CURLcode smtp_perform_rcpt_to(struct Curl_easy *data) {
  ...
  if (host.name)
    ...
    /* An invalid mailbox was provided but we'll simply let the server worry
       about that and reply with a 501 error */
    result =
        Curl_pp_sendf(data, &conn->proto.smtpc.pp, "RCPT TO:<%s>", address);
...
static CURLcode smtp_state_starttls_resp(struct Curl_easy *data, int smtpcode,
                                         smtpstate instate) {
  CURLcode result = CURLE_OK;
  (void)instate; /* no use for this yet */
...
static CURLcode smtp_state_auth_resp(struct Curl_easy *data, int smtpcode,
                                     smtpstate instate) {
  CURLcode result = CURLE_OK;
  struct connectdata *conn = data->conn;
  struct smtp_conn *smtpc = &conn->proto.smtpc;
...
static CURLcode smtp_state_postdata_resp(struct Curl_easy *data, int smtpcode,
                                         smtpstate instate) {
  CURLcode result = CURLE_OK;
...
static CURLcode smtp_parse_url_options(struct connectdata *conn) {
  ...
  while (!result && ptr && *ptr) {
    ...
    if (strncasecompare(key, "AUTH=", 5))
      result =
          Curl_sasl_parse_url_auth_option(&smtpc->sasl, value, ptr - value);
    else
