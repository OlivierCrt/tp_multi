...
#if defined(CURLRES_SYNCH) && defined(HAVE_ALARM) && defined(SIGALRM) &&       \
    defined(HAVE_SIGSETJMP) && defined(GLOBAL_INIT_IS_THREADSAFE)
/* alarm-based timeouts can only be used with all the dependencies satisfied */
#define USE_ALARM_TIMEOUT
#endif

#define MAX_HOSTCACHE_LEN                                                      \
  (255 + 7) /* max FQDN + colon + port number + zero                           \
             */
...
static time_t hostcache_prune(struct Curl_hash *hostcache, int cache_timeout,
  ...
  Curl_hash_clean_with_criterium(hostcache, (void *)&user,
...
struct Curl_dns_entry *Curl_cache_addr(struct Curl_easy *data,
  ...
  dns2 =
      Curl_hash_add(data->dns.hostcache, entry_id, entry_len + 1, (void *)dns);
  if (!dns2) {
    free(dns);
    return NULL;
  }
...
enum resolve_t Curl_resolv(struct Curl_easy *data, const char *hostname,
                           int port, bool allowDOH,
  ...
  if (!dns) {
    ...
    if (data->set.resolver_start) {
      ...
      st = data->set.resolver_start(
          ...
          NULL,
#endif
          NULL, data->set.resolver_start_client);
...
enum resolve_t Curl_resolv_timeout(struct Curl_easy *data, const char *hostname,
                                   int port, struct Curl_dns_entry **entry,
...
clean_up:
  ...
  if (prev_alarm) {
    ...
    timediff_t elapsed_secs =
        Curl_timediff(Curl_now(), data->conn->created) / 1000;
...
CURLcode Curl_loadhostpairs(struct Curl_easy *data) {
  ...
  for (hostp = data->state.resolve; hostp; hostp = hostp->next) {
    ...
    if (hostp->data[0] == '-') {
      ...
      /* Create an entry id, based upon the hostname and port */
      entry_len = create_hostcache_id(&hostp->data[1], hlen, (int)num, entry_id,
                                      sizeof(entry_id));
      if (data->share)
        Curl_share_lock(data, CURL_LOCK_DATA_DNS, CURL_LOCK_ACCESS_SINGLE);
    ...
    err:
      ...
      /* Create an entry id, based upon the hostname and port */
      entry_len = create_hostcache_id(host_begin, hlen, port, entry_id,
                                      sizeof(entry_id));
      ...
      if (dns) {
        infof(data, "RESOLVE %.*s:%d - old addresses discarded", (int)hlen,
              host_begin, port);
      ...
      infof(data, "Added %.*s:%d:%s to DNS cache%s", (int)hlen, host_begin,
            port, addresses, permanent ? "" : " (non-permanent)");
