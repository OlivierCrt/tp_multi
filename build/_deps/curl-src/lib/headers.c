...
static void copy_header_external(struct Curl_header_store *hs, size_t index,
                                 size_t amount, struct Curl_llist_element *e,
...
/* public API */
CURLHcode curl_easy_header(CURL *easy, const char *name, size_t nameindex,
                           unsigned int type, int request,
                           struct curl_header **hout) {
  ...
  for (e = data->state.httphdrs.head; e; e = e->next) {
    hs = e->ptr;
    if (strcasecompare(hs->name, name) && (hs->type & type) &&
        (hs->request == request)) {
      amount++;
      pick = hs;
      e_pick = e;
  ...
  if (nameindex == amount - 1)
    ...
    for (e = data->state.httphdrs.head; e; e = e->next) {
      hs = e->ptr;
      if (strcasecompare(hs->name, name) && (hs->type & type) &&
          (hs->request == request) && (match++ == nameindex)) {
        e_pick = e;
        break;
      }
    }
...
/* public API */
struct curl_header *curl_easy_nextheader(CURL *easy, unsigned int type,
                                         int request,
  ...
  for (e = data->state.httphdrs.head; e; e = e->next) {
    ...
    if (strcasecompare(hs->name, check->name) && (check->request == request) &&
        (check->type & type))
      amount++;
    if (e == pick)
      index = amount - 1;
  ...
  copy_header_external(hs, index, amount, pick, &data->state.headerout[1]);
...
CURLcode Curl_headers_push(struct Curl_easy *data, const char *header,
  ...
  /* insert this node into the list of headers */
  Curl_llist_insert_next(&data->state.httphdrs, data->state.httphdrs.tail, hs,
                         &hs->node);
