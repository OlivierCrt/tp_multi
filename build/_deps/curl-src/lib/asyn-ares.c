...
static void sock_state_cb(void *data, ares_socket_t socket_fd, int readable,
                          int writable) {
...
static int waitperform(struct Curl_easy *data, timediff_t timeout_ms) {
  ...
  if (!nfds)
    ...
    for (i = 0; i < num; i++)
      ares_process_fd(
          (ares_channel)data->state.async.resolver,
  ...
  *dns = NULL;
  ...
  if (res &&
    ...
    /* Remember that the EXPIRE_HAPPY_EYEBALLS_DNS timer is no longer
       running. */
    memset(&res->happy_eyeballs_dns_time, 0,
           sizeof(res->happy_eyeballs_dns_time));
...
CURLcode Curl_set_dns_servers(struct Curl_easy *data, char *servers) {
  ...
  ares_result = ares_set_servers_ports_csv(data->state.async.resolver, servers);
#else
