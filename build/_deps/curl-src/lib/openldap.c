...
static void oldap_state(struct Curl_easy *data, ldapstate newstate) {
  ...
  static const char *const names[] = {
      "STOP", "SSL", "STARTTLS", "TLS", "MECHS", "SASL", "BIND", "BINDV2",
      /* LAST */
  };

  if (ldapc->state != newstate)
    infof(data, "LDAP %p state change from %s to %s", (void *)ldapc,
          names[ldapc->state], names[newstate]);
#endif
...
static CURLcode oldap_url_parse(struct Curl_easy *data, LDAPURLDesc **ludp) {
  ...
  static const char *const url_errs[] = {"success",
                                         "out of memory",
                                         "bad parameter",
                                         "unrecognized scheme",
...
static CURLcode oldap_perform_bind(struct Curl_easy *data, ldapstate newstate) {
  ...
  rc = ldap_sasl_bind(li->ld, binddn, LDAP_SASL_SIMPLE, &passwd, NULL, NULL,
                      &li->msgid);
  if (rc == LDAP_SUCCESS)
    oldap_state(data, newstate);
  else
    result =
        oldap_map_error(rc, data->state.aptr.user ? CURLE_LOGIN_DENIED
...
static CURLcode oldap_perform_mechs(struct Curl_easy *data) {
  ...
  rc = ldap_search_ext(li->ld, "", LDAP_SCOPE_BASE, "(objectclass=*)",
                       (char **)supportedSASLMechanisms, 0, NULL, NULL, NULL, 0,
                       &li->msgid);
  if (rc == LDAP_SUCCESS)
...
static CURLcode oldap_connect(struct Curl_easy *data, bool *done) {
  ...
  hosturl =
      aprintf("ldap%s://%s:%d", conn->handler->flags & PROTOPT_SSL ? "s" : "",
  ...
  if (rc) {
    failf(data, "LDAP local: Cannot connect to %s, %s", hosturl,
          ldap_err2string(rc));
    free(hosturl);
...
static CURLcode oldap_state_mechs_resp(struct Curl_easy *data, LDAPMessage *msg,
                                       int code) {
  struct connectdata *conn = data->conn;
  struct ldapconninfo *li = conn->proto.ldapc;
  int rc;
...
static CURLcode oldap_state_sasl_resp(struct Curl_easy *data, LDAPMessage *msg,
                                      int code) {
  struct connectdata *conn = data->conn;
  struct ldapconninfo *li = conn->proto.ldapc;
  CURLcode result = CURLE_OK;
...
static CURLcode oldap_do(struct Curl_easy *data, bool *done) {
  ...
  result = oldap_url_parse(data, &lud);
  if (!result) {
    rc = ldap_search_ext(li->ld, lud->lud_dn, lud->lud_scope, lud->lud_filter,
                         lud->lud_attrs, 0, NULL, NULL, NULL, 0, &msgid);
    ldap_free_urldesc(lud);
    if (rc != LDAP_SUCCESS) {
...
static CURLcode client_write(struct Curl_easy *data, const char *prefix,
                             size_t plen, const char *value, size_t len,
  ...
  *err = result;
  ...
  case LDAP_RES_SEARCH_ENTRY:
    ...
    for (rc = ldap_get_attribute_ber(li->ld, msg, ber, &bv, &bvals);
      ...
      binary =
          bv.bv_len > 7 && !strncmp(bv.bv_val + bv.bv_len - 7, ";binary", 7);
      ...
      for (i = 0; bvals[i].bv_val != NULL; i++) {
        ...
        if (binary || binval) {
          ...
          result = client_write(data, STRCONST(" "), bvals[i].bv_val,
                                bvals[i].bv_len, STRCONST("\n"));
        if (result)
          break;
      }
...
static Sockbuf_IO ldapsb_tls = {ldapsb_tls_setup, ldapsb_tls_remove,
                                ldapsb_tls_ctrl,  ldapsb_tls_read,
