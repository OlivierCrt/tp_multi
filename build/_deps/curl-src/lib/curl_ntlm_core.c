...
static bool encrypt_des(const unsigned char *in, unsigned char *out,
  ...
  /* Perform the encryption */
  err =
      CCCrypt(kCCEncrypt, kCCAlgorithmDES, kCCOptionECBMode, key, kCCKeySizeDES,
              NULL, in, 8 /* inbuflen */, out, 8 /* outbuflen */, &out_len);
...
void Curl_ntlm_core_lm_resp(const unsigned char *keys,
  ...
  DES_ecb_encrypt((DES_cblock *)plaintext, (DES_cblock *)results, DESKEY(ks),
                  DES_ENCRYPT);
...
CURLcode Curl_ntlm_core_mk_lmv2_resp(unsigned char *ntlmv2hash,
  ...
  result =
      Curl_hmacit(Curl_HMAC_MD5, ntlmv2hash, 16, &data[0], 16, hmac_output);
  if (result)
    return result;
