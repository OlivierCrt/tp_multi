...
const char *const urls[] = {"90022", "90023", "90024", "90025", "90026",
                            "90027", "90028", "90029", "90030"};
...
void *create_thread(void *progress_bar) {
  ...
  /* This stops the pulsing if you have it turned on in the progress bar
     section */
  g_source_remove(
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(progress_bar), "pulse_id")));
...
int main(int argc, char **argv) {
  ...
  g_signal_connect(G_OBJECT(top_window), "delete-event", G_CALLBACK(cb_delete),
                   NULL);
